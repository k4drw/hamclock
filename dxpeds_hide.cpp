...
static void addCacheLine(const char* line) {
    ...
    if (!hidden_cache) fatalError("no memory for %d dxpeds hidden cache", n_hidden_cache + 1);
...
static bool cacheLineIsActive(const char* line) {
    long start_t, end_t;
    if (sscanf(line, "%ld,%ld", &start_t, &end_t) != 2) fatalError("bogus DXPeds cache line: %s", line);
    return (end_t > myNow());
}
...
static void loadHiddenCache(void) {
    // reset memory cache
    for (int i = 0; i < n_hidden_cache; i++) free(hidden_cache[i]);
    free(hidden_cache);
    hidden_cache = NULL;
    ...
    if (!fp) {
        if (errno == EACCES) fatalError("%s: %s", hide_fn, strerror(errno));
        return;
    }
    ...
    while (fgets(line, sizeof(line), fp)) {
        chompString(line);
        if (cacheLineIsActive(line)) addCacheLine(line);
    }

    // finished with file
    fclose(fp);

    if (debugLevel(DEBUG_DXPEDS, 1)) Serial.printf("DXP: loaded %d hidden\n", n_hidden_cache);
}
...
static void saveHiddenCache(void) {
    ...
    if (!fp) fatalError("%s: %s", hide_fn, strerror(errno));
    ...
    fclose(fp);
    if (trouble) fatalError("%s: %s", hide_fn, strerror(errno));

    if (debugLevel(DEBUG_DXPEDS, 1)) Serial.printf("DXP: saved %d hidden\n", n_hidden_cache);
}
...
bool isDXPedsHidden(const char* line) {
    // load if first time
    if (!hidden_cache) loadHiddenCache();

    // check against list
    for (int i = 0; i < n_hidden_cache; i++)
        if (strcmp(line, hidden_cache[i]) == 0) return (true);
    return (false);
}
