...
#define X(a, b, c, d, e, f, g, h, i)                                           \
  {a, b, c, d, e,                                                              \
   f, g, h, i}, // expands SPCWX_DATA to each array initialization in {}
...
static bool initSWFit(void) {
  ...
  if (sw_client.connect(backend_host, backend_port)) {
    ...
    for (int n_c = 0; n_c < SPCWX_N;) {
      ...
      int get_i;
      if (sscanf(line, "%d %f %f %f", &get_i, &a[n_c], &b[n_c], &c[n_c]) != 4 ||
          get_i != n_c) {
...
static void sortSpaceWx() {
  ...
  for (int i = 0; i < SPCWX_N; i++) {
    ...
    sw_i.rank = i;
    Serial.printf("RANKSW: %d %12s %8.2g %3d\n", i, plot_names[sw_i.pc],
                  sw_i.value, computeSWRank(&sw_i));
  }
}
...
static void runNCDXFSpcWxMenu(void) {
  ...
  for (int i = 0; i < SPCWX_N; i++)
    mitems[i] = {
        MENU_0OFN, (spcwx_chmask & (1 << i)) != 0, 1, 1, space_wx[i].name, 0};
  mitems[SPCWX_N] = {MENU_BLANK, 0, 0, 0, 0, 0};
  mitems[SPCWX_N + 1] = {
      MENU_TOGGLE, spcwx_chmask == SPCWX_AUTO, 2, 1, "Auto", 0};
    ...
    for (int j = 0; j < SPCWX_N; j++) {
      if (space_wx[j].rank == i) {
        ...
        if (!space_wx[j].value_ok) {
          ...
          case SPCWX_SSN:
            snprintf(values[i], sizeof(values[i]), "%.0f",
          ...
          case SPCWX_KP:
            snprintf(values[i], sizeof(values[i]), "%.1f",
...
bool retrieveSunSpots(SunSpotData &ssn) {
  ...
  if (curlDownload(url, tmp_fn)) {
    ...
    if (fp) {
      ...
      while (fgets(line, sizeof(line), fp)) {
        ...
        if (line[0] == '#' || line[0] == ':' || strlen(line) < 10)
          continue;
  ...
  // set next update
  ssn_cache.next_update = ok ? nextRetrieval(PLOT_CH_SSN, SSN_INTERVAL)
                             : nextWiFiRetry(PLOT_CH_SSN);
  return (ok);
}
...
bool retrieveSolarFlux(SolarFluxData &sf) {
  ...
  // set next update
  sf_cache.next_update = ok ? nextRetrieval(PLOT_CH_FLUX, SFLUX_INTERVAL)
                            : nextWiFiRetry(PLOT_CH_FLUX);
  return (ok);
}
    ...
    while (fgets(line, sizeof(line), fp)) {
      ...
      if (sscanf(line, "%ld %f", &t, &v) == 2) {
        ...
        if (age_sec >= 0 && age_sec < DRAPDATA_PERIOD) {
          int xi =
              DRAPDATA_NPTS * (DRAPDATA_PERIOD - age_sec) / DRAPDATA_PERIOD;
...
bool retrieveDRAP(DRAPData &drap) {
  ...
  drap_cache.next_update = ok ? nextRetrieval(PLOT_CH_DRAP, DRAPDATA_INTERVAL)
...
bool retrieveKp(KpData &kp) {
  ...
  const char *url = "https://services.swpc.noaa.gov/products/"
  ...
  kp_cache.next_update =
      ok ? nextRetrieval(PLOT_CH_KP, KP_INTERVAL) : nextWiFiRetry(PLOT_CH_KP);
  return ok;
}
...
bool retrieveDST(DSTData &dst) {
  ...
  if (dst_client.connect(backend_host, backend_port)) {
    ...
    int dst_i = 0;
    for (dst_i = 0;
         dst_i < DST_NV && getTCPLine(dst_client, line, sizeof(line), NULL);
         dst_i++) {
...
out:
  ...
  // set next update
  dst_cache.next_update = ok ? nextRetrieval(PLOT_CH_DST, DST_INTERVAL)
                             : nextWiFiRetry(PLOT_CH_DST);
...
bool retrieveXRay(XRayData &xray) {
      ...
      if (buf) {
        ...
        if (!err && doc.is<JsonArray>()) {
          ...
          for (int i = 1; i < XRAY_NV; i++) {
            if (xray_cache.l[i] < -8.0 && xray_cache.l[i - 1] > -8.0)
              xray_cache.l[i] = xray_cache.l[i - 1];
            if (xray_cache.s[i] < -8.0 && xray_cache.s[i - 1] > -8.0)
              xray_cache.s[i] = xray_cache.s[i - 1];
          }
  ...
  xray_cache.next_update = ok ? nextRetrieval(PLOT_CH_XRAY, XRAY_INTERVAL)
                              : nextWiFiRetry(PLOT_CH_XRAY);
  return (ok);
}
...
bool retrieveBzBt(BzBtData &bzbt) {
  ...
  bzbt_cache.next_update = ok ? nextRetrieval(PLOT_CH_BZBT, BZBT_INTERVAL)
                              : nextWiFiRetry(PLOT_CH_BZBT);
  return (ok);
}
      ...
      if (buf) {
        ...
        if (!err && doc.is<JsonArray>()) {
          ...
          for (int i = 1; i < n_rows && sw_cache.n_values < SWIND_MAXN; i++) {
            ...
            if ((unixs < start_t || unixs - prev_unixs < SWIND_DT) &&
                sw_cache.n_values != SWIND_MAXN - 1) {
              // accumulate max_y, check next point
              continue;
            }
            prev_unixs = unixs;

            sw_cache.x[sw_cache.n_values] =
                (t0 - unixs) / (-3600.0F); // hours back (neg)
...
bool retrieveNOAASWx(NOAASpaceWxData &noaasw) {
  ...
  // expecting 3 reply lines of the following form, anything else is an error
  // message
  //  R  0 0 0 0
  //  S  0 0 0 0
  //  G  0 0 0 0
...
out:
  ...
  // set next update
  noaasw_cache.next_update =
      ok ? nextRetrieval(PLOT_CH_NOAASPW, NOAASPW_INTERVAL)
...
bool retrieveAurora(AuroraData &aurora) {
  ...
  if (found_any) {
    ...
    for (int i = 0; i < aurora_cache.n_points; i++) {
      aurora_cache.age_hrs[i] =
          -(aurora_cache.n_points - 1 - i) * (AURORA_INTERVAL / 3600.0F);
    }
...
void initSpaceWX(void) {
  ...
  if (spcwx_chmask != SPCWX_AUTO) {
    ...
    for (int i = 0; i < SPCWX_N; i++)
      space_wx[i].rank =
          (spcwx_chmask & (1 << i)) ? rank++ : 99; // 0 is highest rank
  }
}
