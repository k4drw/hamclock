...
/* insure 0 <= *v < r.
 */
static void range(double* v, double r) { *v -= r * floor(*v / r); }
...
static void nutation(double mjd, double* deps, double* dpsi) {
     ...
     */
    *dpsi = (-17.2327 - .01737 * t) * sin(nm) + (-1.2729 - .00013 * t) * sin(tls) + .2088 * sin(tnm) -
            .2037 * sin(tld) + (.1261 - .00031 * t) * sin(ms) + .0675 * sin(md) - (.0497 - .00012 * t) * sin(tls + ms) -
            .0342 * sin(tld - nm) - .0261 * sin(tld + md) + .0214 * sin(tls - ms) - .0149 * sin(tls - tld + md) +
            .0124 * sin(tls - nm) + .0114 * sin(tld - md);
    *deps = (9.21 + .00091 * t) * cos(nm) + (.5522 - .00029 * t) * cos(tls) - .0904 * cos(tnm) + .0884 * cos(tld) +
            .0216 * cos(tls + ms) + .0183 * cos(tld - nm) + .0113 * cos(tld + md) - .0093 * cos(tls - ms) -
            .0066 * cos(tls - nm);
...
static double mjd_day(double jd) { return (floor(jd - 0.5) + 0.5); }

static double mjd_hr(double jd) { return ((jd - mjd_day(jd)) * 24.0); }
...
static double unix2mjd(long unix_time) { return ((unix_time / 86400.0) + 2440587.5 - 2415020.0); }
 ...
 */
static void hadec_aa(double lat, double ha, double dec, double* alt, double* az) { aaha_aux(lat, ha, dec, az, alt); }
 ...
 */
static void riseset(const time_t t0, const LatLong& ll, void (*cir_func)(time_t t0, const LatLong& ll, AstroCir& cir),
                    time_t* riset, time_t* sett) {
...
static void solarCir(time_t t0, const LatLong& ll, AstroCir& cir) {
    ...
    now_lst(mjd, ll.lng, &lst);
    ha = hr2rad(lst) - ra;
    // printf ("Sun @ %ld: lng %g lsn %g lst %g ra %g ha %g\n", nowWO(), ll.lng_d, rad2deg(lsn), hr2deg(lst),
    // rad2deg(ra), rad2deg(ha));
    hadec_aa(ll.lat, ha, dec, &alt, &az);
    range(&az, 2 * M_PI);
    cir.el = alt;
    cir.az = az;
...
int main(int ac, char* av[]) {
    ...
    printf("Sun at %g %g @ %ld = %d %d %d %d %d %d:\n", ll.lat_d, ll.lng_d, t0, tms.tm_year + 1900, tms.tm_mon + 1,
