...
const char* diag_files[N_DIAG_FILES] = {"diagnostic-log.txt", "diagnostic-log-0.txt", "diagnostic-log-1.txt",
...
static void makeDiagFile() {
    // roll previous few
    for (int i = N_DIAG_FILES - 1; i > 0; --i) mvLog(diag_files[i - 1], diag_files[i]);
        ...
        if (atend || atslash) {
            *p = '\0';
            if (mkdir(path_copy, mode) == 0) {
                if (chown(path_copy, getuid(), getgid()) < 0) ok = 0;
...
static int qsStrcmp(const void* v1, const void* v2) { return (strcmp(*(char**)v1, *(char**)v2)); }
...
static void logSys() {
    ...
    if (cwdp) printf("CWD %s\n", cwdp);
    printf("process id %d\n", getpid());
    printf("built as %s %s\n", build_variables, our_make);
    printf("working directory is %s\n", our_dir.c_str());
    printf("ruid %d euid %d\n", getuid(), geteuid());
    if (pw_file) capturePasswords(pw_file);

    // show sorted env w/o changing original list
    extern char** environ;
    int n_env = 0;
    for (char** e = environ; *e != NULL; e++) n_env++;
    char** env_sort = (char**)malloc(n_env * sizeof(char*));
    for (int i = 0; i < n_env; i++) env_sort[i] = environ[i];
    qsort(env_sort, n_env, sizeof(char*), qsStrcmp);
    printf("ENV:\n");
    for (int i = 0; i < n_env; i++) printf("  %s\n", env_sort[i]);
    free(env_sort);

    // host time
    if (system("date -u")) printf("can not run date??\n");

    // disk
    if (system("df -h .")) printf("can not run df??\n");
}
...
static void logOS() {
    ...
    if (fp) {
        char line[100];
        printf("%s:\n", osf);
        while (fgets(line, sizeof(line), fp)) printf("    %s", line);  // line already includes \n
        fclose(fp);
    }

    if ((system("uname -a") >> 8) != 0) printf("uname failed\n");
...
static void prDebugs(void) {
    ...
    for (int i = 0; i < DEBUG_SUBSYS_N; i++) fprintf(stderr, " %s", db_names[i]);
    fprintf(stderr, "\n");
}
...
static void crackArgs(int ac, char* av[]) {
    ...
    while (--ac && **++av == '-') {
        ...
        while (*++s) {
            switch (*s) {
                ...
                case 'b': {
                    if (ac < 2) usage("missing host:port for -b");
                    ...
                    if (myb_colon) {
                        *myb_colon = '\0';  // put EOS after host
                        ...
                        if (backend_port < 1 || backend_port > 65535) usage("-b port must be [1,65355]");
                    } else
                        usage("no : in -b arg");
                    ac--;
                } break;
                case 'd':
                    if (ac < 2) usage("missing directory path for -d");
                    new_appdir = *++av;
                    ac--;
                    break;
                case 'e':
                    if (ac < 2) usage("missing RESTful port number for -e");
                ...
                case '-':  // in case --help
                    ...
                    break;
                case 'i':
                    if (ac < 2) usage("missing IP for -i");
                    init_locip = *++av;
                    ac--;
                    break;
                case 'k':  // fallthru
                case 'K':
                    ...
                    break;
                case 'l':
                    if (ac < 2) usage("missing longitude for -l");
                ...
                case 'm':
                    ...
                    break;
                case 'n':
                    if (ac < 2) usage("missing timeout for -n");
                    liveweb_to = atoi(*++av);
                    if (liveweb_to <= 0) usage("-n timeout must be positive");
                    ac--;
                    break;
                case 'o':
                    diag_to_file = false;
                    ...
                    break;
                case 'p':
                    if (ac < 2) usage("missing file name for -p");
                    pw_file = *++av;
                    ac--;
                    break;
                case 'q':
                ...
                case 'D':
                    ...
                    break;
                case 'r':
                    if (ac < 2) usage("missing R/O web port number for -r");
                    liveweb_ro_port = atoi(*++av);
                    ac--;
                    break;
                case 's':
                    if (ac < 2) usage("missing date/time for -s");
                    setUsrDateTime(*++av);
                    ac--;
                    break;
                case 't':
                    if (ac < 2) usage("missing percentage for -t");
                    max_cpu_usage = atoi(*++av) / 100.0F;
                    if (max_cpu_usage < 0.1F || max_cpu_usage > 1) usage("-t percentage must be [10,100]");
                    ac--;
                    break;
                case 'v':
                    showVersion();
                    ...
                    break;  // lint
                case 'w':
                    if (ac < 2) usage("missing R/W web port number for -w");
                    liveweb_rw_port = atoi(*++av);
                    ac--;
                    break;
                case 'x':
                    if (ac < 2) usage("missing max live web for -x");
                    max_lw = atoi(*++av);
                    if (max_lw < 1 || max_lw > liveweb_maxmax) usage("-x must be [1,%d]", liveweb_maxmax);
                    ac--;
                    break;
                case 'y':
                    want_kbcursor = true;
    ...
    // initial checks
    if (ac > 0) usage("extra args");
    if (init_iploc && init_locip) usage("can not use both -g and -i");
    if (init_iploc && !skip_skip) usage("-g requires -k");
    if (init_locip && !skip_skip) usage("-i requires -k");
    if (cl_set && !skip_skip) usage("-l requires -k");
    ...
    if (restful_port != -1 && (restful_port < 1 || restful_port > 65535)) usage("-e port must be -1 or [1,65355]");
