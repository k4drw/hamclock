...
const char* brb_names[BRB_N] = {BRBMODES};
#undef X
...
static void setDisplayBrightness(bool log) {
    ...
    if (log) Serial.printf("BR: setting bpwm %d\n", bpwm);
    ...
    if (support_dim) {
        ...
        if (!been_here || want_on != was_on) {
            ...
            system(apple_cmd);
            if (log) Serial.printf("BR: bpwm %d cmd: %s\n", bpwm, apple_cmd);

#else

            if (log) Serial.printf("BR: bpwm %d turning %s\n", bpwm, want_on ? "on" : "off");
            ...
            argv_t argvs[] = {{"xset", "dpms", "0", "0", "0", NULL},  // no blanking
            ...
            for (int i = been_here ? 3 : 0; i < NARRAY(argvs); i++) {
                ...
                int child_pid = fork();
                if (child_pid < 0) fatalError("fork() failed: %s", strerror(errno));
                if (child_pid == 0) {
...
static uint16_t readPhot() {
    ...
    if (found_ltr) {
        ...
        if (ltr.readBothChannels(ch0, ch1)) {
            new_phot = ch0;
            if (new_phot > PHOT_MAX) new_phot = PHOT_MAX;
        }
    }
...
static void drawOnOffControls() {
    if (brb_mode != BRB_SHOW_ONOFF || !mainpage_up) return;
    ...
    if (hr_on < 10) tft.print('0');
    tft.print(hr_on);
    tft.print(':');
    if (mn_on < 10) tft.print('0');
    tft.print(mn_on);
    ...
    if (hr_off < 10) tft.print('0');
    tft.print(hr_off);
    tft.print(':');
    if (mn_off < 10) tft.print('0');
    tft.print(mn_off);
}
 ...
 */
static void engageDisplayBrightness(bool log) {
    if (!clock_off) setDisplayBrightness(log);
...
void setupBrightness() {
    ...
    if (!NVReadUInt16(NV_BPWM_BRIGHT, &fast_bpwm_bright) || fast_bpwm_bright > user_max) fast_bpwm_bright = user_max;
    if (!NVReadUInt16(NV_BPWM_DIM, &fast_bpwm_dim) || fast_bpwm_dim < user_min) fast_bpwm_dim = user_min;
...
void followBrightness() {
    ...
    if (!timesUp(&follow_ms, PHOT_DT)) return;

    // always check on/off first
    if (support_onoff) checkOnOffTimers();

    // that's it if there is no light sensor or there is no control ability
    if (!found_phot || (!support_dim && !support_onoff)) return;
    ...
    if (phot_range == 0) phot_range = 1;  // avoid /0
    ...
    if (bpwm < user_min) bpwm = user_min;
    if (bpwm > user_max) bpwm = user_max;
...
static void changeBrightness(const SCoord& s) {
    if (brb_mode == BRB_SHOW_PHOT) {
        ...
        if (s.y < b.y + b.h / 2) {
            ...
            if (phot > fast_phot_dim) fast_phot_bright = phot;

            Serial.printf("BR: set bright:   bpwm: %4d <= %4d <= %4d   phot: %4d <= %4d <= %4d\n", fast_bpwm_dim, bpwm,
                          fast_bpwm_bright, fast_phot_dim, phot, fast_phot_bright);
            ...
            if (phot < fast_phot_bright) fast_phot_dim = phot;
...
static void engageNewBRBMode(void) {
    ...
    if (brb_mode == BRB_SHOW_ONOFF) getPersistentOnOffTimes(DEWeekday(), mins_on, mins_off);
...
static void runNCDXFMenu(void) {
    ...
    static uint8_t mi_brb_order[BRB_N] = {BRB_SHOW_BEACONS, BRB_SHOW_SWSTATS, BRB_SHOW_ONOFF,
                                          BRB_SHOW_PHOT,    BRB_SHOW_BR,      BRB_SHOW_BME76,
                                          BRB_SHOW_BME77,   BRB_SHOW_DXWX,    BRB_SHOW_DEWX};
...
#define _MI_INDENT 2
    MenuItem mitems[BRB_N] = {
        {MENU_AL1OFN, (bool)(brb_rotset & (1 << BRB_SHOW_BEACONS)), 1, _MI_INDENT, brb_names[BRB_SHOW_BEACONS], 0},
        {MENU_AL1OFN, (bool)(brb_rotset & (1 << BRB_SHOW_SWSTATS)), 1, _MI_INDENT, brb_names[BRB_SHOW_SWSTATS], 0},
        {support_onoff ? MENU_AL1OFN : MENU_IGNORE, (bool)(brb_rotset & (1 << BRB_SHOW_ONOFF)), 1, _MI_INDENT,
         brb_names[BRB_SHOW_ONOFF], 0},
        ...
        {support_dim && !found_phot ? MENU_AL1OFN : MENU_IGNORE, (bool)(brb_rotset & (1 << BRB_SHOW_BR)), 1, _MI_INDENT,
         brb_names[BRB_SHOW_BR], 0},
        {getBMEData(BME_76, false) != NULL ? MENU_AL1OFN : MENU_IGNORE, (bool)(brb_rotset & (1 << BRB_SHOW_BME76)), 1,
         _MI_INDENT, brb_names[BRB_SHOW_BME76], 0},
        {getBMEData(BME_77, false) != NULL ? MENU_AL1OFN : MENU_IGNORE, (bool)(brb_rotset & (1 << BRB_SHOW_BME77)), 1,
         _MI_INDENT, brb_names[BRB_SHOW_BME77], 0},
        {dx_anypane ? MENU_IGNORE : MENU_AL1OFN, (bool)(brb_rotset & (1 << BRB_SHOW_DXWX)), 1, _MI_INDENT,
         brb_names[BRB_SHOW_DXWX], 0},
        {de_anypane ? MENU_IGNORE : MENU_AL1OFN, (bool)(brb_rotset & (1 << BRB_SHOW_DEWX)), 1, _MI_INDENT,
         brb_names[BRB_SHOW_DEWX], 0},
    };
    ...
    if (ok) {
        ...
        for (int i = 0; i < BRB_N; i++)
            if (mitems[i].set) brb_rotset |= (1 << mi_brb_order[i]);
...
bool setDisplayOnOffTimes(int dow, uint16_t new_on, uint16_t new_off, int& idle) {
    if (support_onoff) {
        ...
        if (dow >= 1) persistOnOffTimes(dow, new_on, new_off);
