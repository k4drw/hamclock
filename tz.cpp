...
int getFastTZStep(const LatLong& ll) {
    ...
    for (int d_lat = -2; d_lat <= 2; d_lat += 1) {
        ...
        if (ll_step.lat_d < -89 || ll_step.lat_d > 89) continue;
        for (int d_lng = -2; d_lng <= 2; d_lng += 1) {
            ...
            if (tz_chg > 0 && tz_chg < min_step) min_step = tz_chg;
        }
    }
...
int getTZ(TZInfo& tz) {
    ...
    if (here_again) Serial.printf("getTZ: detected recursion\n");
 ...
 */
void setTZAuto(TZInfo& tz) { tz.auto_tz = true; }
