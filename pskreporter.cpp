...
static int
    n_reports; // count of reports used in psk_bands, might be < n_malloced
...
#define TBLROWH                                                                \
  ((PLOTBOX123_H - LISTING_Y0 - SUBHEAD_DYUP) /                                \
   (HAMBAND_N / 2)) // table row height

// handy test and set whether a band is in use
#define SET_PSKBAND(b)                                                         \
  (psk_bands |= (1 << (b))) // record that band b paths are displayed
#define TST_PSKBAND(b)                                                         \
  ((b) != HAMBAND_NONE &&                                                      \
   (psk_bands & (1 << (b))) != 0) // test if band b displayed
...
void drawFarthestPSKSpots() {
  ...
  if (getSpotLabelType() == LBL_NONE ||
      findPaneForChoice(PLOT_CH_PSK) == PANE_NONE)
    return;
...
static void drawPSKPane(const SBox &box) {
  ...
  char where_how[100];
  snprintf(
      where_how, sizeof(where_how), "%s %s - %s %d %s", of_de ? "of" : "by",
      name, (ispsk ? "PSK" : (iswspr ? "WSPR" : "RBN")),
  ...
  for (int i = 0; i < HAMBAND_N; i++) {
    ...
    if (psk_showdist) {
      ...
      if (!showDistKm())
        d *= MI_PER_KM;
      snprintf(report, sizeof(report), "%3sm %5.0f",
               findBandName((HamBandSetting)i), d);
    } else
      snprintf(report, sizeof(report), "%3sm %5d",
               findBandName((HamBandSetting)i), bstats[i].count);
    if (TST_PSKBAND(i)) {
  ...
  // caption
  const char *label =
      psk_showdist ? (showDistKm() ? "Max distance (km)" : "Max distance (mi)")
                   : "Counts";
...
static bool retrievePSK(void) {
  ...
  snprintf(query + qlen, sizeof(query) - qlen, "?%s%s=%s&maxage=%d",
           of_de ? "of" : "by", use_call ? "call" : "grid",
  ...
  if (psk_client.connect(backend_host, backend_port)) {
    ...
    while (getTCPLine(psk_client, line, sizeof(line), NULL)) {
      ...
      // N.B. first grid/call pair is always TX, second always RX; which is DE
      // depends on PSKMB_OFDE
      DXSpot new_sp = {};
      long posting_temp;
      long Hz_temp;
      if (sscanf(line, "%ld,%6[^,],%11[^,],%6[^,],%11[^,],%7[^,],%ld,%f",
                 &posting_temp, new_sp.tx_grid, new_sp.tx_call, new_sp.rx_grid,
                 new_sp.rx_call, new_sp.mode, &Hz_temp, &new_sp.snr) != 8) {
        Serial.printf("PSK: %s\n", line);
        goto out;
      }
      ...
      // RBN does not provide tx_grid but it must be us. N.B. this will be blank
      // from rbndaemon
      if (isrbn)
      ...
      if (!(n_reports < n_malloced)) {
        reports =
            (DXSpot *)realloc(reports, (n_malloced += 100) * sizeof(DXSpot));
        if (!reports)
...
out:
  ...
  Serial.printf("PSK: found %d %s reports %s %s\n", n_reports,
                ...
                of_de ? "of" : "by", use_call ? getCallsign() : de_maid);
...
#define MI_N (HAMBAND_N + 21) // ham_bands + controls
  ...
  mitems[_M_160] = {MENU_AL1OFN, TST_PSKBAND(HAMBAND_160M),  4,
                    SEC_INDENT,  findBandName(HAMBAND_160M), 0};
  mitems[_M_80] = {MENU_AL1OFN, TST_PSKBAND(HAMBAND_80M),  4,
                   SEC_INDENT,  findBandName(HAMBAND_80M), 0};
  mitems[_M_60] = {MENU_AL1OFN, TST_PSKBAND(HAMBAND_60M),  4,
                   SEC_INDENT,  findBandName(HAMBAND_60M), 0};
  mitems[_M_40] = {MENU_AL1OFN, TST_PSKBAND(HAMBAND_40M),  4,
                   SEC_INDENT,  findBandName(HAMBAND_40M), 0};
  ...
  mitems[_M_30] = {MENU_AL1OFN, TST_PSKBAND(HAMBAND_30M),  4,
                   SEC_INDENT,  findBandName(HAMBAND_30M), 0};
  mitems[_M_20] = {MENU_AL1OFN, TST_PSKBAND(HAMBAND_20M),  4,
                   SEC_INDENT,  findBandName(HAMBAND_20M), 0};
  mitems[_M_17] = {MENU_AL1OFN, TST_PSKBAND(HAMBAND_17M),  4,
                   SEC_INDENT,  findBandName(HAMBAND_17M), 0};
  mitems[_M_15] = {MENU_AL1OFN, TST_PSKBAND(HAMBAND_15M),  4,
                   SEC_INDENT,  findBandName(HAMBAND_15M), 0};
  ...
  mitems[_M_12] = {MENU_AL1OFN, TST_PSKBAND(HAMBAND_12M),  4,
                   SEC_INDENT,  findBandName(HAMBAND_12M), 0};
  mitems[_M_10] = {MENU_AL1OFN, TST_PSKBAND(HAMBAND_10M),  4,
                   SEC_INDENT,  findBandName(HAMBAND_10M), 0};
  mitems[_M_6] = {MENU_AL1OFN, TST_PSKBAND(HAMBAND_6M),  4,
                  SEC_INDENT,  findBandName(HAMBAND_6M), 0};
  mitems[_M_2] = {MENU_AL1OFN, TST_PSKBAND(HAMBAND_2M),  4,
                  SEC_INDENT,  findBandName(HAMBAND_2M), 0};
  ...
  if (runMenu(menu)) {
    ...
    if (rbn_set && (!ofDE_set || !call_set)) {
      ...
      // set new ham_bands
      psk_bands = 0;
      if (mitems[_M_160].set)
        SET_PSKBAND(HAMBAND_160M);
      if (mitems[_M_80].set)
        SET_PSKBAND(HAMBAND_80M);
      if (mitems[_M_60].set)
        SET_PSKBAND(HAMBAND_60M);
      if (mitems[_M_40].set)
        SET_PSKBAND(HAMBAND_40M);
      if (mitems[_M_30].set)
        SET_PSKBAND(HAMBAND_30M);
      if (mitems[_M_20].set)
        SET_PSKBAND(HAMBAND_20M);
      if (mitems[_M_17].set)
        SET_PSKBAND(HAMBAND_17M);
      if (mitems[_M_15].set)
        SET_PSKBAND(HAMBAND_15M);
      if (mitems[_M_12].set)
        SET_PSKBAND(HAMBAND_12M);
      if (mitems[_M_10].set)
        SET_PSKBAND(HAMBAND_10M);
      if (mitems[_M_6].set)
        SET_PSKBAND(HAMBAND_6M);
      if (mitems[_M_2].set)
        SET_PSKBAND(HAMBAND_2M);
 ...
 */
bool getPSKBandStats(PSKBandStats stats[HAMBAND_N],
