...
bool crackClusterSpot(char line[], DXSpot& spot) {
    ...
    if (!isdigit(line[70]) || !isdigit(line[71]) || !isdigit(line[72]) || !isdigit(line[73]) || hr < 0 || hr > 59 ||
        mn < 0 || mn > 60) {
    ...
    if (spot.spotted > now) {
        dxcLog("%s %g: correcting future time %02d:%02d %ld > %ld\n", spot.tx_call, spot.kHz, tm.Hour, tm.Minute,
               (long)spot.spotted, (long)(spot.spotted - SECSPERDAY));
    ...
    for (; *xml != '\0'; xml++) {
        switch (state) {
            case XMLS_LOOK4_KEY:
                ...
                break;
            case XMLS_IN_KEY:
                if (key_candidate == NULL) key_candidate = xml;  // first char
                if (*xml == '>') {
            ...
            case XMLS_IN_CONTENT:
                if (key_content == NULL) key_content = xml;  // first char
                if (*xml == '<') {
...
bool crackXMLSpot(const char xml[], DXSpot& spot) {
    ...
    if (!have_spot || !action_add) return (false);
...
bool crackADIFSpot(const char string[], DXSpot& spot) {
    ...
    bool ok = false;
    if (n_good > 0) {
        if (n_good > 1) dxcLog("UDP: using only first of %d ADIF records\n", n_good);
        spot = spots[0];
        ok = true;
    } else {
        if (n_bad > 0) dxcLog("UDP: packet contained %d bad ADIF records\n", n_bad);
    }
    if (spots) free(spots);
    return (ok);
}
