 ...
 * N.B. we assume coords insure marker will be wholy within map boundaries.
 */
void eraseDEAPMarker() { eraseSCircle(deap_c); }

/* return whether to display the DE antipode
 */
static bool showDEAPMarker() { return (map_proj != MAPP_AZIM1 && !dx_info_for_sat && overMap(deap_c.s)); }

/* return whether to display the DX marker:
 *   over map and either not showing sat or showing either DX weather or VOACAP.
 */
bool showDXMarker() {
    return ((!dx_info_for_sat || findPaneChoiceNow(PLOT_CH_DXWX) != PANE_NONE ||
...
void drawDEInfo() {
    ...
    switch (de_time_fmt) {
        case DETIME_INFO:
            ...
            // lat and lon
            char buf[50];
            snprintf(buf, sizeof(buf), "%.0f%c  %.0f%c", roundf(fabsf(de_ll.lat_d)), de_ll.lat_d < 0 ? 'S' : 'N',
...
static void drawMaidGridKey() {
    ...
    if (mapgrid_choice != MAPGRID_MAID || map_proj != MAPP_MERCATOR) return;

    // keep right stripe above RSS and map scale, if on
    uint16_t right_h = map_b.h;
    if (rss_on) right_h = rss_bnr_b.y - map_b.y;
    if (mapScaleIsUp()) right_h = mapscale_b.y - map_b.y;  // drap_b.y already above rss if on
...
static void drawMapPopup(void) {
    ...
    if (runMenu(menu)) {
        ...
        if (mitems[0].set) newDX(map_popup.ll, NULL, NULL);
        if (mitems[1].set) newDE(map_popup.ll, NULL);
...
static void drawAzimGrid() {
    ...
    for (int ti = 0; ti < 360 / THETA_GRID; ti++) {
        ...
        for (float r = 0; r <= M_PIF; r += deg2rad(FINESTEP_GRID)) {
            ...
            if (map_proj == MAPP_AZIM1 && r > max_az1_r) break;
            float ca, B;
            ...
            if (map_proj != MAPP_MERCATOR || (lat > min_pole_lat && lat < max_pole_lat)) {
                ...
                if (segmentSpanOkRaw(s0, s1, lw)) tft.drawLineRaw(s0.x, s0.y, s1.x, s1.y, lw, EARTH_GRIDC);
                s0 = s1;
            } else
                s0.x = 0;
        }
...
static void drawTropicsGrid() {
    ...
    if (map_proj != MAPP_MERCATOR) {
        ...
        for (float lng = -180; lng <= 180; lng += FINESTEP_GRID) {
            ...
            if (segmentSpanOkRaw(s00, s01, lw)) tft.drawLineRaw(s00.x, s00.y, s01.x, s01.y, lw, EARTH_GRIDC);
            s00 = s01;
            if (segmentSpanOkRaw(s10, s11, lw)) tft.drawLineRaw(s10.x, s10.y, s11.x, s11.y, lw, EARTH_GRIDC);
            s10 = s11;
        }
...
#define SEC_INDENT 8
    ...
    if (menu_ok) {
        ...
        map_rotset = 0;
        if (mitems[MI_STY_CTY].set) scheduleNewCoreMap(CM_COUNTRIES);
        if (mitems[MI_STY_TER].set) scheduleNewCoreMap(CM_TERRAIN);
        if (mitems[MI_STY_DRA].set) scheduleNewCoreMap(CM_DRAP);
        if (mitems[MI_STY_MUF].set) scheduleNewCoreMap(CM_MUF_V);
        if (mitems[MI_STY_MRT].set) scheduleNewCoreMap(CM_MUF_RT);
        if (mitems[MI_STY_AUR].set) scheduleNewCoreMap(CM_AURORA);
        if (mitems[MI_STY_WXX].set) scheduleNewCoreMap(CM_WX);
        if (mitems[MI_STY_CLO].set) scheduleNewCoreMap(CM_CLOUDS);
        if (mitems[MI_STY_USR].set) scheduleNewCoreMap(CM_USER);
        if (mitems[MI_STY_TOA].set) scheduleNewCoreMap(CM_PMTOA);
        if (mitems[MI_STY_REL].set) scheduleNewCoreMap(CM_PMREL);
        ...
        if (map_rotset != prev_rotset) {
            ...
            if (!IS_CMROT(core_map)) insureCoreMap();
            saveCoreMaps();
        }
        logMapRotSet();
...
void drawMoreEarth() {
    ...
    // draw next row
    for (moremap_s.x = map_b.x; moremap_s.x <= last_x; moremap_s.x++) drawMapCoord(moremap_s);  // does not draw grid
...
// #define TIME_MAP_DRAW                             // RBF
        ...
        if (tv0.tv_sec != 0) Serial.printf("****** map %ld us\n", TVDELUS(tv0, tv1));
        tv0 = tv1;
#endif  // TIME_MAP_DRAW
    }
}
...
void ll2s(const LatLong& ll, SCoord& s, uint8_t edge) { ll2sScaled(ll, s, edge, 1); }
...
void ll2sRaw(const LatLong& ll, SCoord& s, uint8_t edge) { ll2sScaled(ll, s, edge, tft.SCALESZ); }
...
bool s2ll(const SCoord& s, LatLong& ll) {
    ...
    switch ((MapProjection)map_proj) {
        ...
        case MAPP_MERCATOR: {
            ...
            if (core_map != CM_USER) ll.lng_d += getCenterLng();
            ll.normalize();
...
void drawMapCoord(const SCoord& s) {
    ...
    sr.x = s.x + 1;
    sr.y = s.y;
    if (!s2ll(sr, llr)) llr = lls;
    sd.x = s.x;
    sd.y = s.y + 1;
    if (!s2ll(sd, lld)) lld = lls;
    ...
    tft.plotEarth(s.x, s.y, lls.lat_d, lls.lng_d, llr.lat_d - lls.lat_d, llr.lng_d - lls.lng_d, lld.lat_d - lls.lat_d,
                  lld.lng_d - lls.lng_d, fract_day);
}
...
void drawMoon() {
    ...
    for (int16_t dy = -moon_r; dy <= moon_r; dy++) {
        ...
        for (int16_t dx = -Ryi; dx <= Ryi; dx++) {
            float a = acosf(dx / Ry);
            uint16_t color = (isnan(a) || (phase > 0 && a > phase) || (phase < 0 && a < phase + M_PIF)) ? RA8875_BLACK
                                                                                                        : RA8875_WHITE;
...
void drawDXInfo() {
    ...
    if (dx_info_for_sat || SHOWING_PANE_0()) return;
    ...
    // lat and long
    char buf[50];
    snprintf(buf, sizeof(buf), "%.0f%c  %.0f%c", roundf(fabsf(dx_ll.lat_d)), dx_ll.lat_d < 0 ? 'S' : 'N',
...
bool segmentSpanOk(const SCoord& s0, const SCoord& s1, uint16_t border) {
    if (s0.x == 0 || s1.x == 0) return (false);
    if (s0.x > s1.x ? (s0.x - s1.x > map_b.w / 4) : (s1.x - s0.x > map_b.w / 4)) return (false);  // too wide
    if (s0.y > s1.y ? (s0.y - s1.y > map_b.h / 3) : (s1.y - s0.y > map_b.h / 3)) return (false);  // too hi
    if (map_proj == MAPP_AZIMUTHAL && ((s0.x < map_b.x + map_b.w / 2) != (s1.x < map_b.x + map_b.w / 2)))
        return (false);                                                      // crosses azimuthal hemispheres
    if (overViewBtn(s0, border) || overViewBtn(s1, border)) return (false);  // over the view button
    if (!overMap(s0) || !overMap(s1)) return (false);                        // off the map entirely
    return (true);                                                           // ok!
}
...
bool segmentSpanOkRaw(const SCoord& s0, const SCoord& s1, uint16_t border) {
    ...
    if (s0.x == 0 || s1.x == 0) return (false);
    if (s0.x > s1.x ? (s0.x - s1.x > map_w / 4) : (s1.x - s0.x > map_w / 4)) return (false);  // too wide
    if (s0.y > s1.y ? (s0.y - s1.y > map_h / 3) : (s1.y - s0.y > map_h / 3)) return (false);  // too hi
    ...
    if (!overMap(raw2appSCoord(s0)) || !overMap(raw2appSCoord(s1))) return (false);  // off the map entirely
    return (true);                                                                   // ok!
}
