...
CoreMapInfo cm_info[CM_N] = {COREMAPS};
#undef X
...
static bool downloadZFile(WiFiClient& client, const char* filename, long len) {
    ...
    if (!fp) fatalError("Error creating file %s: %s", filename, strerror(errno));  // never returns
...
static bool installFilePixels(const char* dfile, const char* nfile) {
    ...
    if (day_fp && night_fp) {
        ...
        night_pixels = (char*)mmap(NULL, night_nbytes, PROT_READ, MAP_PRIVATE, fileno(night_fp), 0);
    ...
    if (ok) {
        ...
        if (getGrayDisplay() != GRAY_OFF) {
            ...
            if (!mem_day_pixels || !mem_night_pixels) fatalError("No memory for gray scale image");
 ...
 */
static bool checkDayNightFiles(int yr, int mo, int hr, const char* page, const char* style, const float MHz,
                               char query[], char dfn[], char nfn[], size_t qdn_len) {
    static const char qfmt[] =
        "YEAR=%d&MONTH=%d&UTC=%d&TXLAT=%.3f&TXLNG=%.3f&PATH=%d&WATTS=%d&WIDTH=%d&HEIGHT=%d&MHZ=%.2f&TOA=%.1f&MODE=%d&"
        "TOA=%.1f";

    snprintf(query, qdn_len, qfmt, yr, mo, hr, de_ll.lat_d, de_ll.lng_d, show_lp, bc_power, ZOOM_W, ZOOM_H, MHz, bc_toa,
             bc_modevalue, bc_toa);
 ...
 * return whether ok
 */
static bool installQueryMaps(const char* page, const char* msg, const char* style, const float MHz, long max_age) {
    // fresh start
    invalidatePixels();
...
#define QBUFLEN 200
    ...
    if (!ok) {
        ...
        if (minute(t) < use_prev_minute) {
            ...
            ok = checkDayNightFiles(prev_yr, prev_mo, prev_hr, page, style, MHz, query, q_dfn, q_nfn, QBUFLEN);
            if (!ok) {
    ...
    // check again
    if (!ok) mapMsg(3000, "%s: fail", style);
    ...
    if (!ok) {
        ...
        if (cm != CM_USER) {
            ...
            if (client.connect(backend_host, backend_port)) {
                ...
                if (BUILD_W * pan_zoom.zoom > 4800) mapMsg(0, "%s", title);
                char url[256];
            ...
            if (!fp) mapMsg(1000, "%s: download failed", title);
        }
    }
...
bool installWebMapImages(WiFiClient& client, long content_length, ImageRefit fit, Message& ynot) {
    // time download
    struct timeval tv0;
    if (debugLevel(DEBUG_BMP, 1)) gettimeofday(&tv0, NULL);

    // read contents
    StackMalloc image_mem(content_length);
    char* image = (char*)image_mem.getMem();
    if (!image) fatalError("No memory for User image %ld", content_length);
    long n_read;
 ...
 */
void rmWebMapImages(void) { cleanCache(cm_info[CM_USER].name, CACHE_NONE); }
...
static bool installFileMaps(CoreMaps cm) {
    ...
    if (!CM_ISFILE(cm)) fatalError("installFileMaps(%d) invalid", cm);  // does not return
...
void initCoreMaps() {
    ...
    if (band >= PROPBAND_NONE) RM_CMROT(CM_PMTOA);
    ...
    if (band >= PROPBAND_NONE) RM_CMROT(CM_PMREL);
...
void saveCoreMaps(void) {
    if ((int)core_map >= CM_N) fatalError("saveMapRotSet() core_map %d\n", core_map);
...
void logMapRotSet(void) {
    ...
    for (int i = 0; i < CM_N; i++) {
        ...
        if (IS_CMROT(ci)) ll += snprintf(line + ll, sizeof(line) - ll, "%s ", getCoreMapStyle((CoreMaps)ci, s));
    }
    Serial.printf("%s\n", line);
}
...
float propBand2MHz(PropMapBand band) {
    switch (band) {
        ...
        case PROPBAND_10M:
            return (28.2);
        default:
            fatalError("propBand2MHz MHz %d", band);
    }
...
int propBand2Band(PropMapBand band) {
    switch (band) {
        ...
        case PROPBAND_10M:
            return (10);
        default:
            fatalError("propBand2Band %d", band);
    }
...
void drawMapScale() {
    ...
    static const MapScalePoint d_scale[] = {
        // see fetchDRAP.pl and fetchVOACAP-MUF.pl
        {0, 0x000000, 0},  {4, 0x4E138A, 0},  {9, 0x001EF5, 0},  {15, 0x78FBD6, 1},
        {20, 0x78FA4D, 1}, {27, 0xFEFD54, 1}, {30, 0xEC6F2D, 1}, {35, 0xE93323, 1},
    };

    // CM_AURORA
    static const MapScalePoint a_scale[] = {
        // see fetchAurora.pl
        {0, 0x282828, 0}, {25, 0x00FF00, 1}, {50, 0xFFFF00, 1}, {75, 0xEA6D2D, 1}, {100, 0xFF0000, 1},
    };

    // CM_WX
    static const MapScalePoint w_scale[] = {
        // see fetchWordWx.pl
        // values are degs C
        {-50, 0xD1E7FF, 1}, {-40, 0xB5D5FF, 1}, {-30, 0x88BFFF, 1}, {-20, 0x73AAFF, 1},
        {-10, 0x4078D9, 0}, {0, 0x2060A6, 0},   {10, 0x009EDC, 1},  {20, 0xBEE5B4, 1},
        {30, 0xFF8C24, 1},  {40, 0xEE0051, 1},  {50, 0x5B0023, 1},
    };

    // CM_PMTOA
    static const MapScalePoint t_scale[] = {
        // see fetchVOACAP-TOA.pl
        {0, 0x0000F0, 0},
        {6, 0xF0B060, 1},
        {30, 0xF00000, 1},
    };
    ...
    // CM_PMREL
    static const MapScalePoint r_scale[] = {
        // see fetchVOACAPArea.pl
        {0, 0x666666, 0},  {21, 0xEE6766, 0}, {40, 0xEEEE44, 1},
        {60, 0xEEEE44, 1}, {83, 0x44CC44, 0}, {100, 0x44CC44, 0},
    };
...
#define _MS_PRY (mapscale_b.y + 1U)                               // text y
    ...
    for (int i = 1; i < n_ticks; i++) {  // skip first for title
        ...
        if (ti_x < _MS_X0 || ti_x > _MS_X1 || (marker_x && ti_x >= marker_x - 15 && ti_x <= marker_x + 15)) continue;
...
void insureCoreMap() {
    ...
    if (IS_CMROT(core_map)) return;

    // pick another
    core_map = CM_NONE;
    for (int i = 0; i < CM_N && core_map == CM_NONE; i++)
        if (IS_CMROT(i)) core_map = (CoreMaps)i;

    // if none at all, set a default
    if (core_map == CM_NONE) DO_CMROT(CM_COUNTRIES);
}
...
void rotateNextMap() {
    ...
    if (new_cm < 0) fatalError("Bogus map rotation set: 0x%x\n", map_rotset);
...
void unlinkOurs(const char* filename) {
    ...
    if (unlink(path) < 0) Serial.printf("unlink(%s): %s\n", filename, strerror(errno));
}
