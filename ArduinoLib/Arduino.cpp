...
const char *diag_files[N_DIAG_FILES] = {
    "diagnostic-log.txt", "diagnostic-log-0.txt", "diagnostic-log-1.txt",
...
uint32_t millis(void) {
  ...
  // this is only on FreeBSD but is fully 200x faster than gettimeofday or
  // CLOCK_MONOTONIC as of 14-RELEASE now this one is slow -- use normal
  // gettimeofday
  ...
  if (t0.tv_sec == 0)
    t0 = t;
  uint32_t dt_ms =
      (t.tv_sec - t0.tv_sec) * 1000 + (t.tv_usec - t0.tv_usec) / 1000;
  return (dt_ms);
#endif
}
...
uint16_t analogRead(int pin) {
  ...
  return (
      0); // not supported on Pi, consider https://www.adafruit.com/product/1083
}
...
static void usage(const char *errfmt, ...) {
  if (errfmt) {
    ...
    fprintf(stderr, "Purpose: display time and other information useful to "
    ...
    fprintf(stderr, " -d d : set working directory to d; default is %s\n",
            defaultAppDir().c_str());
    fprintf(stderr,
            " -e p : set RESTful web server port to p or -1 to disable; "
            "default is %d\n",
            RESTFUL_PORT);
    fprintf(
        stderr,
        " -f o : force display full screen initially to \"on\" or \"off\"\n");
    fprintf(stderr, " -g   : init DE using geolocation with current public IP; "
                    "requires -k\n");
    fprintf(stderr, " -h   : print this help summary then exit\n");
    fprintf(stderr,
            " -i i : init DE using geolocation with IP i; requires -k\n");
    fprintf(stderr, " -k   : start in normal mode, ie, don't offer Setup or "
                    "wait for Skips\n");
    fprintf(stderr, " -l l : set Mercator or Robinson center longitude to l "
                    "degrees, +E; requires -k\n");
    ...
    fprintf(stderr, " -o   : write diagnostic log to stdout instead of in %s\n",
            defaultAppDir().c_str());
    fprintf(stderr, " -p f : require passwords in file f formatted as lines of "
                    "\"category password\"\n");
    ...
    fprintf(stderr,
            ...
            LIVEWEB_RW_PORT);
    fprintf(stderr,
            " -x n : set n max live web connections; max %d; default %d\n",
            liveweb_maxmax, liveweb_max);
    fprintf(stderr, " -y   : activate keyboard cursor control "
                    "arrows/hjkl/Return -- beware stuck keys!\n");
  }
...
static void crackArgs(int ac, char *av[]) {
  ...
  while (--ac && **++av == '-') {
    ...
    while (*++s) {
      ...
      case 'a': {
        ...
        char *debug_level =
            strchr(debug_name, '='); // find name=level separator
        if (!debug_level)
          usage("missing name=level for -a");
  ...
  if (liveweb_rw_port > 0 && liveweb_ro_port > 0 &&
      liveweb_rw_port == liveweb_ro_port)
    usage("Live web R/W and R/O ports must not be equal: %d %d",
          liveweb_rw_port, liveweb_ro_port);
  if (liveweb_rw_port > 0 && restful_port > 0 &&
      liveweb_rw_port == restful_port)
    usage("Live web R/W and RESTful ports must not be equal: %d %d",
          liveweb_rw_port, restful_port);
  if (liveweb_ro_port > 0 && restful_port > 0 &&
      liveweb_ro_port == restful_port)
    usage("Live web R/O and RESTful ports must not be equal: %d %d",
          liveweb_ro_port, restful_port);
