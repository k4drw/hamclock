 ...
 */
void ESPhttpUpdate::cleanupDir(const char* tmp) { (void)runCommand(false, 0, 0, 0, "rm -fr %s", tmp); }
...
t_httpUpdate_return ESPhttpUpdate::update(WiFiClient& client, const char* url) {
    ...
    char our_path[1000];
    if (!findFullPath(our_argv[0], our_path, sizeof(our_path))) return (HTTP_UPDATE_FAILED);  // already set err msg
    ...
    if (!runCommand(false, 1, 2, 1, "mkdir %s", tmp_dir)) return (HTTP_UPDATE_FAILED);

    // download url into tmp_dir naming it zip_file
    if (!runCommand(false, 2, 8, 1, "curl --max-time 15 --retry 2 --silent --show-error --output '%s/%s' '%s'", tmp_dir,
                    zip_file, url)) {
        cleanupDir(tmp_dir);
        return (HTTP_UPDATE_FAILED);
    }
    ...
    // explode
    if (!runCommand(false, 8, 12, N_UNZIP_LINES, "cd %s && unzip -q %s", tmp_dir, zip_file)) {
        cleanupDir(tmp_dir);
        return (HTTP_UPDATE_FAILED);
    }
    ...
    if (d) {
        ...
        while ((dir = readdir(d)) != NULL) {
            if (strcmp(dir->d_name, ".") == 0 || strcmp(dir->d_name, "..") == 0 ||
                strcmp(dir->d_name, "__MACOSX") == 0) {
                continue;
            }
            if (dir->d_type == DT_DIR) {
                strncpy(make_dir, dir->d_name, sizeof(make_dir) - 1);
                break;
            }
        }
        closedir(d);
    ...
    if (make_dir[0] == '\0') {
        prError("OTA: Could not find extracted directory in %s\n", tmp_dir);
        cleanupDir(tmp_dir);
        return (HTTP_UPDATE_FAILED);
    }
    printf("OTA: Found extracted dir: %s\n", make_dir);
...
#ifdef _IS_FREEBSD
    if (!runCommand(false, 12, 99, N_MAKE_LINES, "cd %s/%s && gmake -j %d %s %s", tmp_dir, make_dir, j, build_variables,
                    our_make)) {
#else
    if (!runCommand(false, 12, 99, N_MAKE_LINES, "cd %s/%s && make -j %d %s %s", tmp_dir, make_dir, j, build_variables,
                    our_make)) {
#endif
        cleanupDir(tmp_dir);
        return (HTTP_UPDATE_FAILED);
    ...
    if (!runCommand(true, 99, 100, 1, "rm -f %s && mv %s/%s/%s %s", our_path, tmp_dir, make_dir, our_make, our_path)) {
        cleanupDir(tmp_dir);
        return (HTTP_UPDATE_FAILED);
    }
...
void ESPhttpUpdate::onProgress(void (*my_progressCB)(int current, int total)) { progressCB = my_progressCB; }

int ESPhttpUpdate::getLastError(void) { return (1); }
...
void ESPhttpUpdate::prError(const char* fmt, ...) {
    ...
    if (err_lines_q[err_lines_head]) free(err_lines_q[err_lines_head]);
