...
bool ScrollState::checkScrollDownTouch(const SCoord& s, const SBox& b) const {
    return (s.x > b.x + b.w - SCR_DX - SCR_W && s.y > b.y + SCRUP_DY + SCR_H / 2 && s.y < b.y + SCRDW_DY + SCR_H / 2);
}
...
void ScrollState::moveTowardsOlder() {
    ...
    if (top_vis < llimit) top_vis = llimit;
}
...
void ScrollState::moveTowardsNewer() {
    top_vis += max_vis - 1;  // leave one for context
    int ulimit = n_data - 1;
    if (top_vis > ulimit) top_vis = ulimit;
}
 ...
 */
bool ScrollState::okToScrollDown(void) const { return (nMoreBeneath() > 0); }

/* return whether there is more data above the displayed list
 */
bool ScrollState::okToScrollUp(void) const { return (nMoreAbove() > 0); }

/* return whether the displayed rows are at the top most (newest) data entry
 */
bool ScrollState::atNewest(void) const { return (n_data == 0 || top_vis == n_data - 1); }
...
bool ScrollState::scrollT2B(void) const {
    if (dir == DIR_FROMSETUP) return (scrollTopToBottom());
    if (dir == DIR_TOPDOWN) return (true);
    if (dir == DIR_BOTUP) return (false);
