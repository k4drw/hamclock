...
const BCModeSetting bc_modes[N_BCMODES]{{"CW", 19},  {"RTTY", 22}, {"SSB", 38}, {"AM", 49},
                                        {"WSPR", 3}, {"FT8", 13},  {"FT4", 17}};
uint8_t findBCModeValue(const char* name)  // find value give name, else 0
{
    for (int i = 0; i < N_BCMODES; i++)
        if (strcmp(name, bc_modes[i].name) == 0) return (bc_modes[i].value);
    return (0);
}
const char* findBCModeName(uint8_t value)  // find name given value, else NULL
{
    for (int i = 0; i < N_BCMODES; i++)
        if (bc_modes[i].value == value) return (bc_modes[i].name);
    return (NULL);
}
...
static NTPServer ntp_list[] = {
    ...
    {"time.google.com", NTP_TOO_LONG},     {"time.apple.com", NTP_TOO_LONG},    {"pool.ntp.org", NTP_TOO_LONG},
    {"europe.pool.ntp.org", NTP_TOO_LONG}, {"asia.pool.ntp.org", NTP_TOO_LONG}, {"time.nist.gov", NTP_TOO_LONG},
};
...
static time_t nextWiFiRetry(void) {
    ...
    if (next_try > now + WIFI_MAXRETRY) next_try = now + WIFI_MAXRETRY;  // but clamp to WIFI_MAXRETRY
...
static time_t nextPaneUpdate(PlotChoice pc, int interval) {
    ...
    if (pp == PANE_NONE) fatalError("nextPaneUpdate %s PANE_NONE", plot_names[pc]);
    ...
    if (interval > 60) Serial.printf("Pane %d now showing %s updates in %d sec at %d\n", pp, plot_names[pc], dt, at);
...
time_t nextRotation(PlotPane pp) {
    ...
    if (BRBIsRotating() && brb_next_update > latest_rot) latest_rot = brb_next_update;

    // bring up to present if all old
    time_t t0 = myNow();
    if (latest_rot < t0) latest_rot = t0;
...
static void geolocateIP(const char* ip) {
    ...
    // But WiFiClient stream support in ArduinoJson might depend on
    // implementation. ArduinoLib's WiFiClient inherits from nothing? No, it
    // should work if it has read(). But let's stick to buffer for
    // safety/debugging.
    char json_buf[1024];

    strcpy(path, "/json/");
    if (ip) strncat(path, ip, sizeof(path) - strlen(path) - 1);
    ...
    if (iploc_client.connect(host, port)) {
        ...
        // Note: we could use deserializeJson(doc, iploc_client) directly if
        // WiFiClient implemented Stream, but let's be safe and read to buffer
        // first.
        int n = 0;
        ...
        while (millis() - t0 < 3000) {
            if (iploc_client.available()) {
                ...
                if (n < (int)sizeof(json_buf) - 1) json_buf[n++] = c;
        ...
        if (!error) {
            ...
            if (status && strcmp(status, "success") == 0) {
                ...
                tftMsg(true, 0, "  %.2f%c %.2f%c", fabsf(lat), lat < 0 ? 'S' : 'N', fabsf(lon), lon < 0 ? 'W' : 'E');
...
static void initWiFi(bool verbose) {
    ...
    // wait for connection
    if (myssid) tftMsg(verbose, 0, "\r");  // init overwrite
    do {
        if (myssid) tftMsg(verbose, 0, "Connecting to %s %.*s\r", myssid, ndots, dots);
...
void initSys() {
    ...
    if (useGeoIP() || init_iploc || init_locip) {
        ...
        if (getGPSDLatLong(&ll)) {
            ...
            tftMsg(true, 0, "GPSD: %.2f%c %.2f%c", fabsf(de_ll.lat_d), de_ll.lat_d < 0 ? 'S' : 'N', fabsf(de_ll.lng_d),
                   de_ll.lng_d < 0 ? 'W' : 'E');
        ...
        if (getNMEALatLong(ll)) {
            ...
            tftMsg(true, 0, "NMEA: %.2f%c %.2f%c", fabsf(de_ll.lat_d), de_ll.lat_d < 0 ? 'S' : 'N', fabsf(de_ll.lng_d),
                   de_ll.lng_d < 0 ? 'W' : 'E');
        ...
        if (useLocalNTPHost()) {
            ...
            for (int i = 0; i < N_NTP; i++) {
                ...
                if (getNTPUTC(np) == 0)
                    ...
                    if (!best_ntp || np->rsp_time < best_ntp->rsp_time) best_ntp = np;
                }
            ...
            if (!skip_skip) wdDelay(800);  // linger to show last time
            if (best_ntp)
    ...
    if (usr_datetime > 0) setTime(usr_datetime);
...
void checkBGMap(void) {
    ...
    if (next_map != 0 && time_to_refresh && mapIsRotating()) rotateNextMap();
    ...
    if (time_to_refresh || bc_now || tdiff(nowWO(), map_time) >= 3600) {
        ...
        if (bc_up && bc_map) plotBandConditions(plot_b[bc_pp], 1, NULL, NULL);
        ...
        if (bc_up) plotBandConditions(plot_b[bc_pp], ok ? 0 : -1, NULL, NULL);

        time_t dt = next_map - myNow();
        Serial.printf("Next %s map check in %ld s at %ld\n", cm_info[core_map].name, (long)dt,
                      (long)(millis() / 1000 + dt));
    }
}
...
static bool retrieveBandConditions(char* config) {
    ...
    snprintf(query, sizeof(query),
             ...
             bc_page, year(t), month(t), dx_ll.lat_d, dx_ll.lng_d, de_ll.lat_d, de_ll.lng_d, hour(t), show_lp, bc_power,
             bc_modevalue, bc_toa);
    ...
    if (fp) {
        ...
        for (int r = 0; r < BMTRX_ROWS; r++) {
            ...
            int utc_hr;
            if (sscanf(buf, "%d %f,%*f,%f,%f,%f,%f,%f,%f,%f", &utc_hr, &rel[0], &rel[1], &rel[2], &rel[3], &rel[4],
                       &rel[5], &rel[6], &rel[7]) != BMTRX_COLS + 1) {
            ...
            for (int c = 0; c < BMTRX_COLS; c++) bc_matrix.m[utc_hr][c] = (uint8_t)(100 * rel[c]);
        }
...
static void checkBRB(time_t t) {
    ...
    if (t > brb_next_update) {
        ...
        switch (brb_mode) {
            ...
            case BRB_SHOW_BME77:
                // only if new BME
                if (newBME280data()) (void)drawNCDXFBox();
                break;

            case BRB_SHOW_SWSTATS:
                // only if new space stats
                if (checkForNewSpaceWx()) (void)drawNCDXFBox();
                break;
...
void setPlotVisible(PlotChoice pc) {
    ...
    if (findPaneChoiceNow(pc) != PANE_NONE) return;

    // not on display now but maybe in rotation, else just pick one
    PlotPane pp = findPaneForChoice(pc);
    if (pp == PANE_NONE) pp = PANE_3;
...
bool setPlotChoice(PlotPane pp, PlotChoice pc) {
    ...
    if (pp_now != PANE_NONE && pp_now != pp) return (false);
    ...
    switch (pc) {
        case PLOT_CH_DXCLUSTER:
            if (!useDXCluster()) return (false);
            break;
        case PLOT_CH_GIMBAL:
            if (!haveGimbal()) return (false);
            break;
        case PLOT_CH_TEMPERATURE:
            if (getNBMEConnected() == 0) return (false);
        ...
        case PLOT_CH_COUNTDOWN:
            if (getSWEngineState(NULL, NULL) != SWE_COUNTDOWN) return (false);
    ...
    if (findPaneChoiceNow(PLOT_CH_GIMBAL) == PANE_NONE) closeGimbal();
...
void sendUserAgent(WiFiClient& client) {
    ...
    if (logUsageOk() && ready) {
        ...
        int alarms = 0;
        if (a_ds != ALMS_OFF) alarms += 1;
        if (a_os != ALMS_OFF) alarms += 2;
        ...
        int brb = brb_mode;
        if (BRBIsRotating()) brb += 100;

        // GPSD
        int gpsd = 0;
        if (useGPSDTime()) gpsd |= 1;
        if (useGPSDLoc()) gpsd |= 2;
        ...
        for (int i = 0; i < N_CSPR; i++)
            if (getPathDashed((ColorSelection)i)) n_dashed++;
        ...
        if (want_kbcursor) crc |= (1 << 15);  // max old _debug_ was 1<<14
        ...
        for (int i = 0; i < WLID_N; i++) {
            if (getWatchListState((WatchListId)i, NULL) != WLA_OFF) wl |= (1 << i);
        }
        ...
        snprintf(ua, sizeof(ua),
                 ...
                 "\r\n",
                 platform, hc_version, ESP.getChipId(), (long long)getUptime(NULL, NULL, NULL, NULL), crc, map_style,
                 main_page, mapgrid_choice, plotops[PANE_1], plotops[PANE_2], plotops[PANE_3], de_time_fmt, brb,
                 dx_info_for_sat, rss_code, units, getNBMEConnected(), gpio, io, getBMETempCorr(BME_76),
                 getBMEPresCorr(BME_76), desrss, dxsrss, BUILD_W, dpy_mode,

                 // new for LV5:
                 alarms, getCenterLng(), (int)auxtime /* getDoy() before 2.80 */, names_on, getDemoMode(),
                 (int)getSWEngineState(NULL, NULL), (int)getBigClockBits(), utcOffset(), gpsd, rss_interval, dayf,
                 rr_score,
                 ...
                 showNewDXDEWx(), getPaneRotationPeriod(), pw_file != NULL, n_roweb > 0, pz, plotops[PANE_0],
                 screenIsLocked(), showPIP(), (int)getGrayDisplay(), wl, aup_hr, 0);
...
bool httpSkipHeader(WiFiClient& client, const char* header, char* value, int value_len) {
    ...
    do {
        if (!getTCPLine(client, line, sizeof(line), NULL)) return (false);
        // Serial.println (line);

        if (header && value && (hdr = strstr(line, header)) != NULL) snprintf(value, value_len, "%s", hdr + hdr_len);
...
static bool updateDRAP(const SBox& box) {
    ...
    if (ok) {
        if (!drap.data_ok) {
            ...
            // plot
            plotXY(box, drap.x, drap.y, DRAPDATA_NPTS, "Hours", "DRAP, max MHz", DRAPPLOT_COLOR, 0, 0,
                   drap.y[DRAPDATA_NPTS - 1]);
        }

        // update NCDXF box too either way
        if (brb_mode == BRB_SHOW_SWSTATS) drawNCDXFSpcWxStats(RA8875_BLACK);
...
static bool updateSolarFlux(const SBox& box) {
    ...
    if (ok) {
        if (!sf.data_ok) {
            ...
            plotXY(box, sf.x, sf.sflux, SFLUX_NV, "Days", "10.7 cm Solar flux", SFLUX_COLOR, 0, 0,
                   sf.sflux[SFLUX_NV - 10]);
        }
...
static bool updateBzBt(const SBox& box) {
    ...
    if (ok) {
        if (!bzbt.data_ok) {
            ...
            plotXYstr(box, bzbt.x + f25, bzbt.bz + f25, BZBT_NV - f25, "Hours", "Solar Bz and Bt, nT", BZBT_BZCOLOR,
                      min_bzbt, max_bzbt, NULL) &&
                plotXYstr(box, bzbt.x + f25, bzbt.bt + f25, BZBT_NV - f25, NULL, NULL, BZBT_BTCOLOR, min_bzbt, max_bzbt,
                          bz_label);
        }
...
static bool updateAurora(const SBox& box) {
    ...
    if (ok) {
        if (!a.data_ok) {
            ...
            plotXYstr(box, a.age_hrs, a.percent, a.n_points, "Hours", "Aurora Chances, max %", AURORA_COLOR, 0, 100,
                      aurora_label);
        }
...
bool checkBCTouch(const SCoord& s, const SBox& b) {
    ...
    if (!inBox(s, b) || s.y < b.y + PANETITLE_H || !bc_matrix.ok) return (false);
    ...
    if (inBox(s, power_b)) {
        ...
        if (power_changed) {
            bc_power = new_power;
            NVWriteUInt16(NV_BCPOWER, bc_power);
            if (CM_PMACTIVE()) scheduleNewCoreMap(core_map);
        }
        ...
        if (tap_band >= 0 && tap_band < PROPBAND_N) {
            ...
            // update
            if (refresh_map) scheduleNewCoreMap(core_map);
            plotBandConditions(b, 0, NULL, NULL);
...
bool getTCPLine(WiFiClient& client, char line[], uint16_t line_len, uint16_t* ll) {
    ...
    while (true) {
        char c;
        if (!getTCPChar(client, &c)) return (false);
        if (c == '\r') continue;
        if (c == '\n') {
            line[i] = '\0';
            if (ll) *ll = i;
...
void scheduleNewCoreMap(CoreMaps cm) {
    if (cm == CM_NONE) fatalError("Bug! setting no core map");
...
PlotPane ignorePaneTouch() {
    if (myNow() < revert_time) return (revert_pane);
    return (PANE_NONE);
}
