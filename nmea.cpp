...
static bool crackRMC(const char* rmc) {
    ...
    for (int i = 0; i < 13; i++) {
        ...
        if (debugLevel(DEBUG_NMEA, 2)) Serial.printf("NMEA: field %d %s\n", i, field);
...
static bool setNMEAtty(int fd, Message& ynot) {
    ...
    if (debugLevel(DEBUG_NMEA, 1)) Serial.printf("NMEA: %s set to %s\n", getNMEAFile(), baud_str);
...
static void* theNMEAThread(void* unused) {
    ...
    if (!expandENV(fn, path, sizeof(path))) fatalError("NMEA: %s env expansion failed", fn);
    int fd = open(path, O_RDONLY);
    if (fd < 0) fatalError("NMEA device %s: %s", path, strerror(errno));
    ...
    while (fgets(msg, sizeof(msg), fp)) {
        ...
        if (tty_cps) {                                        // tty can correct start based on baud
            ...
            if (debugLevel(DEBUG_NMEA, 1)) Serial.printf("NMEA: fgets %4u ms: %s\n", fgets_ms, msg);
        }
...
static void checkNMEAThread(void) {
    ...
    if (e) fatalError("NMEA: pthread_create %s", strerror(e));
    ...
    if (!running_ok) fatalError("NMEA: no valid sentences found\n");
}
...
time_t getNMEAUTC(void) {
    ...
    if (nmea_ok) {
        ...
        if (ms_late > 0) {
            ...
            usleep(us_sleep);
            if (debugLevel(DEBUG_NMEA, 1)) Serial.printf("NMEA: sync sleep %d ms\n", us_sleep / 1000);
            t += 1;
        }
    }
