...
static void drawMPElPlot(time_t t0, time_t& t_start, time_t& t_end) {
    ...
    for (time_t t = t0; t <= t0 + MP_DUR; t += MP_DT) {
        ...
        if (!prev_both_up && both_up_now) {
            ...
            if (x + width > MP_X0 + MP_PW) width = MP_X0 + MP_PW - x;
...
static void drawMPBothUpTable(time_t t0, time_t t_start, time_t t_end) {
    ...
    int detz = getTZ(de_tz);
    if (better_start == t0) {
        snprintf(buf, sizeof(buf), "DE    now    %02d:%02d   %3.0f    %3.0f", hour(better_end + detz),
                 minute(better_end + detz), rad2deg(de_ac.az), rad2deg(de_ac.el));
    } else {
        snprintf(buf, sizeof(buf), "DE   %02d:%02d   %02d:%02d   %3.0f    %3.0f", hour(better_start + detz),
                 minute(better_start + detz), hour(better_end + detz), minute(better_end + detz), rad2deg(de_ac.az),
                 rad2deg(de_ac.el));
    }
    tft.setTextColor(DE_COLOR);
    ...
    int dxtz = getTZ(dx_tz);
    if (better_start == t0) {
        snprintf(buf, sizeof(buf), "DX    now    %02d:%02d   %3.0f    %3.0f", hour(better_end + dxtz),
                 minute(better_end + dxtz), rad2deg(dx_ac.az), rad2deg(dx_ac.el));
    } else {
        snprintf(buf, sizeof(buf), "DX   %02d:%02d   %02d:%02d   %3.0f    %3.0f", hour(better_start + dxtz),
                 minute(better_start + dxtz), hour(better_end + dxtz), minute(better_end + dxtz), rad2deg(dx_ac.az),
                 rad2deg(dx_ac.el));
    }
    tft.setTextColor(DX_COLOR);
    ...
    // UTC rows
    if (better_start == t0) {
        snprintf(buf, sizeof(buf), "UTC   now    %02d:%02d", hour(better_end), minute(better_end));
    } else {
        snprintf(buf, sizeof(buf), "UTC  %02d:%02d   %02d:%02d", hour(better_start), minute(better_start),
                 hour(better_end), minute(better_end));
    }
    tft.setTextColor(RA8875_WHITE);
...
static void drawMPPopup(const time_t t, const SBox& popup_b) {
    ...
    int detz = getTZ(de_tz);
    snprintf(buf, sizeof(buf), "DE  %02d:%02d  %3.0f %4.0f", hour(t + detz), minute(t + detz), rad2deg(de_ac.az),
             rad2deg(de_ac.el));
    ...
    int dxtz = getTZ(dx_tz);
    snprintf(buf, sizeof(buf), "DX  %02d:%02d  %3.0f %4.0f", hour(t + dxtz), minute(t + dxtz), rad2deg(dx_ac.az),
             rad2deg(dx_ac.el));
 ...
 */
static void freshEMETool(const SBox& exit_b, const char* exit_name, time_t& t0, time_t& t_start, time_t& t_end) {
    // start now
    t0 = nowWO();
...
void drawEMETool() {
    ...
    UserInput ui = {screen_b, UI_UFuncNone, UF_UNUSED, MP_TO, UF_NOCLOCKS, {0, 0}, TT_NONE, '\0', false, false};

    // repeat until actively does something to exit
    for (;;) {
        if (waitForUser(ui)) {
            // done if return, esc or tap Exit button or tap outside box
            if (ui.kb_char == CHAR_CR || ui.kb_char == CHAR_NL || ui.kb_char == CHAR_ESC || inBox(ui.tap, exit_b))
                break;
            ...
            if (ui.tap.x > MP_X0 && ui.tap.x < MP_X0 + MP_PW && ui.tap.y > MP_E2Y(M_PI_2F) &&
                ...
                if (popup_b.x + popup_b.w > MP_X0 + MP_PW) popup_b.x = MP_X0 + MP_PW - popup_b.w;
                if (popup_b.y + popup_b.h > MP_E2Y(-M_PI_2F) - MP_MT) popup_b.y = MP_E2Y(-M_PI_2F) - MP_MT - popup_b.h;
