...
static ColorName cnames[] = {
    {0, 255, 255, "aqua"},       {127, 255, 212, "aquamarine"}, {240, 255, 255, "azure"},
    {245, 245, 220, "beige"},    {255, 228, 196, "bisque"},     {0, 0, 0, "black"},
    {0, 0, 255, "blue"},         {165, 42, 42, "brown"},        {222, 184, 135, "burlywood"},
    {127, 255, 0, "chartreuse"}, {210, 105, 30, "chocolate"},   {255, 127, 80, "coral"},
    {255, 248, 220, "cornsilk"}, {220, 20, 60, "crimson"},      {0, 255, 255, "cyan"},
    {178, 34, 34, "firebrick"},  {255, 0, 255, "fuchsia"},      {220, 220, 220, "gainsboro"},
    {255, 215, 0, "gold"},       {218, 165, 32, "goldenrod"},   {0, 255, 0, "green"},
    {240, 255, 240, "honeydew"}, {75, 0, 130, "indigo"},        {255, 255, 240, "ivory"},
    {240, 230, 140, "khaki"},    {230, 230, 250, "lavender"},   {0, 255, 0, "lime"},
    {250, 240, 230, "linen"},    {255, 0, 255, "magenta"},      {176, 48, 96, "maroon"},
    {255, 228, 181, "moccasin"}, {0, 0, 128, "navy"},           {128, 128, 0, "olive"},
    {255, 165, 0, "orange"},     {218, 112, 214, "orchid"},     {205, 133, 63, "peru"},
    {255, 192, 203, "pink"},     {221, 160, 221, "plum"},       {160, 32, 240, "purple"},
    {255, 0, 0, "red"},          {250, 128, 114, "salmon"},     {255, 245, 238, "seashell"},
    {160, 82, 45, "sienna"},     {192, 192, 192, "silver"},     {255, 250, 250, "snow"},
    {210, 180, 140, "tan"},      {0, 128, 128, "teal"},         {216, 191, 216, "thistle"},
    {255, 99, 71, "tomato"},     {64, 224, 208, "turquoise"},   {238, 130, 238, "violet"},
    {245, 222, 179, "wheat"},    {255, 255, 255, "white"},      {255, 255, 0, "yellow"},
};
...
static void printColorNames(WiFiClient& client) {
    for (int i = 0; i < NARRAY(cnames); i++) client.println(cnames[i].name);
}

static bool rgbOk(int r, int g, int b) { return (r >= 0 && r <= 255 && g >= 0 && g <= 255 && b >= 0 && b <= 255); }
        ...
        switch (state) {
            case PWC_LOOKING_4_NAME:
                if (*line == '=') {
                    ...
                    *line = '\0';  // terminate name
                    if (!setWCValue(wa, strTrimAll(name), NULL, line0, line_len)) return (false);
                    name = line + 1;  // start next name
                    value = NULL;     // no value yet
                } else if (*line == '\0') {
                    if (name[0] == '\0') return (true);  // no name at all is ok
            ...
            case PWC_LOOKING_4_VALUE:
                if (*line == '&') {
                    *line = '\0';  // terminate value
                    if (!setWCValue(wa, strTrimAll(name), strTrimAll(value), line0, line_len)) return (false);
                    name = line + 1;             // start next name
                    value = NULL;                // no value yet
...
static void getStopwatchReport(char buf[], size_t bufl) {
    ...
    switch (sws) {
        ...
        case SWE_LAP:
            ...
            break;
        case SWE_COUNTDOWN:
            state_name = "Countdown";
            break;
    }

    // format report
    size_t bl = snprintf(buf, bufl, "%s %02d:%02d:%02d.%02d", state_name, sw_hr, sw_mn, sw_sc, (int)(sw / 10));
    if (sws == SWE_COUNTDOWN) bl += snprintf(buf + bl, bufl - bl, " / %02d:%02d:%02d", cd_hr, cd_mn, cd_sc);
    buf[bl++] = '\n';
    buf[bl] = '\0';
}
...
static void spotsHelper(WiFiClient& client, const DXSpot* spots, int nspots, char* buf, int buf_len) {
    ...
    for (uint8_t i = 0; i < nspots; i++) {
        ...
        snprintf(buf + bufl, buf_len - bufl, " %-*s %02d%02d %7s %6s %7.2f %7.2f   %6.0f   %4.0f %s\n",
                 MAX_SPOTCALL_LEN - 1, spot.tx_call, hour(spot.spotted), minute(spot.spotted), spot.mode, spot.tx_grid,
                 spot.tx_ll.lat_d, spot.tx_ll.lng_d, dist, bear, bear_ismag ? "magnetic" : "true");
...
static bool getWiFiGPIO(WiFiClient& client, char line[], size_t line_len) {
    ...
    // parse
    if (!parseWebCommand(wa, line, line_len)) return (false);
...
static bool getWiFiConfig(WiFiClient& client, char* unused_line, size_t line_len) {
    ...
    if (!night_on) client.print(" without night");
    client.println();
    ...
    // report panes
    for (int pp = PANE_0; pp < PANE_N; pp++) reportPaneChoices(client, (PlotPane)pp);
    ...
    if (findPaneForChoice(PLOT_CH_PSK) != PANE_NONE) {
        ...
        snprintf(buf, sizeof(buf), "%s DE %s using %s\n", psk_mask & PSKMB_OFDE ? "of" : "by",
                 psk_mask & PSKMB_CALL ? "call" : "grid", (ispsk ? "PSK" : (iswspr ? "WSPR" : "RBN")));
    } else {
        strcpy(buf, "off\n");
    }
    client.print(buf);
    ...
    if (getDisplayInfo(pcon, t_idle, t_idle_left)) {
        ...
        for (int dow = 1; dow <= DAYSPERWEEK; dow++) {
            ...
            if (getDisplayOnOffTimes(dow, on_mins, off_mins)) {
                snprintf(buf, sizeof(buf), "OnOff_%s %02d:%02d %02d:%02d\n", dayShortStr(dow), on_mins / 60,
                         on_mins % 60, off_mins / 60, off_mins % 60);
                client.print(buf);
            } else
                break;
        }
    ...
    if (useGPSDTime() || useGPSDLoc()) {
        client.print("get");
        if (useGPSDTime()) client.print(" time");
        if (useGPSDLoc()) client.print(" location");
        client.print(" from ");
    ...
    if (at_all) {
        ...
        if (gconn && vis_now) {
            bufl += snprintf(buf + bufl, sizeof(buf) - bufl, "%s, %s @ ", gstop ? "stopped" : "active",
                             gauto ? "auto" : "manual");
            if (has_el)
    ...
    if (!dx_info_for_sat) {
        nbuf += snprintf(buf + nbuf, sizeof(buf) - nbuf, "TZ=UTC%+g %s\n", getTZ(dx_tz) / 3600.0,
                         dxsrss == DXSRSS_INAGO ? "RSInAgo" : (dxsrss == DXSRSS_ATAT ? "RSAtAt" : "Prefix"));
    }
    client.print(buf);
    ...
    for (int i = 0; i < MAX_N_BME; i++) {
        ...
        if (dp)
            nbuf += snprintf(buf + nbuf, sizeof(buf) - nbuf, "dTemp@%x= %g dPres@%x= %g ", dp->i2c, getBMETempCorr(i),
                             dp->i2c, getBMEPresCorr(i));
    }
    if (nbuf > 0)
        client.println(buf);
    else
    ...
    for (int i = 0; i < N_CSPR; i++) {
        ...
        snprintf(buf, sizeof(buf), "%-16s%d,%d,%d\n", uscore_name, RGB565_R(color), RGB565_G(color), RGB565_B(color));
        client.print(buf);
    }
...
static bool getWiFiSatellite(WiFiClient& client, char* line, size_t ll) {
    ...
    float *razs, *sazs;
    int n_times;
    if (sn.raz == SAT_NOAZ || sn.saz == SAT_NOAZ || (n_times = nextSatRSEvents(&rises, &razs, &sets, &sazs)) == 0) {
        client.print("No rise or set\n");
    } else {
        // next events
        if (sn.rdt > 0) {
            snprintf(line, ll, "NextRiseIn    %02dh%02d\n", (int)sn.rdt, (int)(60 * (sn.rdt - (int)sn.rdt) + 0.5F));
            client.print(line);
        }
        snprintf(line, ll, "NextSetIn     %02dh%02d\n", (int)sn.sdt, (int)(60 * (sn.sdt - (int)sn.sdt) + 0.5F));
        client.print(line);
        ...
        for (int i = 0; i < n_times; i++) {
            ...
            int l = snprintf(line, ll, "%.3s  %02dh%02d  %3.0f", dayShortStr(rt_wd), hour(rt), minute(rt), razs[i]);

            // if set time is tomorrow start new line with set day and blank rise
            if (rt_wd != st_wd) l += snprintf(line + l, ll - l, "\n%s            ", dayShortStr(st_wd));
    ...
    for (int i = 0; i < MAX_N_BME; i++) {
        ...
        if (dp) {
            ...
            for (int j = 0; j < N_BME_READINGS; j++) {
                ...
                if (u) {
                    char buf[100];
                    snprintf(buf, sizeof(buf), "%4d-%02d-%02dT%02d:%02d:%02dZ %ld  %02x %7.2f %7.2f %7.2f %7.2f\n",
                             year(u), month(u), day(u), hour(u), minute(u), second(u), u, dp->i2c, dp->t[qj], dp->p[qj],
                             dp->h[qj], dewPoint(dp->t[qj], dp->h[qj]));
                    client.print(buf);
                }
            }
            client.print("\n");
...
static bool getWiFiSpaceWx(WiFiClient& client, char* unused_line, size_t line_len) {
    ...
    if (retrieveNOAASWx(noaasw) && noaasw.data_ok) {
        for (int i = 0; i < N_NOAASW_C; i++) {
            ...
            for (int j = 0; j < N_NOAASW_V; j++) bl += snprintf(buf + bl, sizeof(buf) - bl, " %d", noaasw.val[i][j]);
            client.println(buf);
        }
    }
...
static bool getWiFiSys(WiFiClient& client, char* unused_line, size_t line_len) {
    ...
    client.println(hc_version);
    snprintf(buf, sizeof(buf), "BuildSz  %d x %d\n", BUILD_W, BUILD_H);
    client.print(buf);
    client.print("MaxStack ");
    client.println(worst_stack);
    client.print("Platform ");
    client.println(platform);
    client.print("BEHost   ");
    client.println(backend_host);
    client.print("BEPort   ");
    client.println(backend_port);
    client.print("MACaddr  ");
    client.println(WiFi.macAddress().c_str());
    client.print("S/N      ");
...
static bool getWiFiTime(WiFiClient& client, char* unused_line, size_t line_len) {
    ...
    int mn = minute(t);
    int sc = second(t);
    int bl = snprintf(buf, sizeof(buf) - 10, "Clock_UTC %d-%02d-%02dT%02d:%02d:%02d ", yr, mo, dy, hr, mn, sc);
    ...
    if (off == 0) {
        ...
        if (dy_os != 0) bl += snprintf(buf + bl, sizeof(buf) - bl, "%dd", dy_os);
...
static bool setWiFiDEformat(WiFiClient& client, char line[], size_t line_len) {
    ...
    if (new_fmt < 0) {
        ...
        for (int i = 1; i < DETIME_N; i++) ll += snprintf(line + ll, line_len - ll, ", %s", detime_names[i]);
        return (false);
    }
...
static bool setWiFiAuxTime(WiFiClient& client, char line[], size_t line_len) {
    ...
    if (!found) {
        ...
        for (int i = 1; i < AUXT_N; i++) n += snprintf(line + n, line_len - n, ",%s", auxtime_names[i]);
        return (false);
    }
...
static bool setWiFiDisplayTimes(WiFiClient& client, char line[], size_t line_len) {
    if (brOnOffOk()) {
        ...
        bool found_onoff = wa.found[0] && on && wa.found[1] && off && sscanf(on, "%d:%d", &on_hr, &on_mn) == 2 &&
...
static bool setWiFiMapColor(WiFiClient& client, char line[], size_t line_len) {
    ...
    if (brb_mode == BRB_SHOW_BEACONS) (void)drawNCDXFBox();
...
static bool setWiFiMapCenter(WiFiClient& client, char line[], size_t line_len) {
    ...
    if (map_proj == MAPP_MERCATOR || map_proj == MAPP_ROB) initEarthMap();
...
static bool setWiFiMapView(WiFiClient& client, char line[], size_t line_len) {
    ...
    if (S) {
        ...
        if (my_cm == CM_NONE || my_cm == CM_PMTOA || my_cm == CM_PMREL) {
            ...
            for (int i = 0; i < CM_N; i++)
                if (i != CM_PMTOA && i != CM_PMREL) ll += snprintf(line + ll, line_len - ll, " %s", cm_info[i].name);
            return (false);
        }
    }
    ...
    if (G) {
        ...
        if (my_llg < 0) {
            ...
            for (int i = 1; i < MAPGRID_N; i++) ll += snprintf(line + ll, line_len - ll, ", %s", grid_styles[i]);
            return (false);
        }
    }
    ...
    if (P) {
        ...
        if (my_proj < 0) {
            ...
            for (int i = 1; i < MAPP_N; i++) ll += snprintf(line + ll, line_len - ll, ", %s", map_projnames[i]);
            return (false);
        }
    }
...
static bool setWiFiloadBMP(WiFiClient& client, char line[], size_t line_len) {
    // handy wa indices
    enum { LBMP_PANE, LBMP_FIT, LBMP_OFF, LBMP_N };
...
static bool setWiFiRSS(WiFiClient& client, char line[], size_t line_len) {
    ...
    if (wa.found[3] && wa.value[3] == NULL) {
        ...
        char line[150];
        while ((!content_length || (nr += ll + 1) < content_length) && getTCPLine(client, line, sizeof(line), &ll))
...
static bool setWiFiPane(WiFiClient& client, char line[], size_t line_len) {
    ...
    if (strcmp(line, "Pane0=off") == 0) {
        ...
        for (int i = 0; i < n_pc; i++) new_rotset |= (1 << pc[i]);
...
static bool setWiFiSatName(WiFiClient& client, char line[], size_t line_len) {
    // do it
    if (setSatFromName(line)) return (getWiFiSatellite(client, line, line_len));
...
static bool setWiFiSatTLE(WiFiClient& client, char line[], size_t line_len) {
    ...
    // try to install
    if (setSatFromTLE(name, t1, t2)) return (getWiFiSatellite(client, line, line_len));
...
static bool setWiFiTouch(WiFiClient& client, char line[], size_t line_len) {
    ...
    int x, y;
    if (!atoiOnly(wa.value[0], &x) || x < 0 || x >= tft.width() || !atoiOnly(wa.value[1], &y) || y < 0 ||
        y >= tft.height()) {
...
static bool setWiFiGPIO(WiFiClient& client, char line[], size_t line_len) {
    enum { SETG_PIN, SETG_LEVEL, SETG_BLINK };
...
static bool setWiFiVOACAP(WiFiClient& client, char line[], size_t line_len) {
    ...
    // see if what changed
    if (new_power != bc_power || new_modevalue != bc_modevalue || new_toa != bc_toa || new_map != core_map ||
...
static bool setWiFiLiveSpots(WiFiClient& client, char line[], size_t line_len) {
    ...
    enum { _LS_SPOT, _LS_WHAT, _LS_DATA, _LS_BANDS, _LS_AGE, _LS_SHOW, _LS_PATH };
    ...
    const char* usage = "spot=of|by&what=call|grid&show=maxdist|counts&data=psk|wspr|rbn&age=mins&bands=all|160,...";
    ...
    if (wa.found[_LS_BANDS]) {
        ...
        if (strcmp(bands, "all") == 0) {
            ...
            while ((token = strsep(&string, ",")) != NULL) {
                switch (atoi(token)) {
                    case 160:
                        new_bands |= (1 << HAMBAND_160M);
                        break;
                    case 80:
                        new_bands |= (1 << HAMBAND_80M);
                        break;
                    case 60:
                        new_bands |= (1 << HAMBAND_60M);
                        break;
                    case 40:
                        new_bands |= (1 << HAMBAND_40M);
                        break;
                    case 30:
                        new_bands |= (1 << HAMBAND_30M);
                        break;
                    case 20:
                        new_bands |= (1 << HAMBAND_20M);
                        break;
                    case 17:
                        new_bands |= (1 << HAMBAND_17M);
                        break;
                    case 15:
                        new_bands |= (1 << HAMBAND_15M);
                        break;
                    case 12:
                        new_bands |= (1 << HAMBAND_12M);
                        break;
                    case 10:
                        new_bands |= (1 << HAMBAND_10M);
                        break;
                    case 6:
                        new_bands |= (1 << HAMBAND_6M);
                        break;
                    case 2:
                        new_bands |= (1 << HAMBAND_2M);
                        break;
                    default:
    ...
    // skip if no changes
    if (psk_mask == new_mask && psk_bands == new_bands && psk_maxage_mins == new_age && psk_showdist == new_dist &&
...
static bool roCommandOk(const char* cmd) {
    return (strncmp(cmd, "get_", 4) == 0 || strncmp(cmd, "set_daily_alarm", 15) == 0 ||
            strncmp(cmd, "set_once_alarm", 14) == 0 || strncmp(cmd, "set_stopwatch", 13) == 0 ||
            strncmp(cmd, "set_touch", 9) == 0 || strncmp(cmd, "set_screenlock", 14) == 0 ||
            strncmp(cmd, "restart", 7) == 0 || strncmp(cmd, "postDiags", 9) == 0 || strncmp(cmd, "exit", 4) == 0);
}
    ...
    if (!ro || roCommandOk(command)) {
        for (int i = 0; i < N_CMDTABLE; i++) {
            ...
            if (strncmp(command, ctp->command, cmd_len) == 0) {
                ...
                if (replaceEncoding(params)) Serial.printf("Decoded: %s\n", params);  // print decoded version
...
static bool isPOST(const char* line) {
    return (strncmp(line, "POST /set_rss?", 14) == 0 || strncmp(line, "POST /set_adif?", 15) == 0 ||
            strncmp(line, "POST /set_bmp?", 14) == 0);
}

/* return whether the given line is a valid GET command
 */
static bool isGET(const char* line) { return (strncmp(line, "GET /", 5) == 0); }
...
static void serveRemote(WiFiClient& client, bool ro) {
    ...
    if (content_length) Serial.printf("Content-Length: %ld\n", content_length);

    // find beginning just after first -- we aleady know there is a /
    char* cmd_start = strchr(line, '/') + 1;

    // run command
    if (runWebserverCommand(client, ro, cmd_start, line + line_mem.getSize() - cmd_start)) return;
    ...
    for (uint8_t i = 0; i < N_CMDTABLE - N_UNDOC_CMD; i++) {
        ...
        if (ro && !roCommandOk(ctp->command)) continue;
        ...
        if (funp == setWiFiPane) {
            ...
            for (int i = 0; i < PLOT_CH_N; i++) {
                if (plotChoiceIsAvailable((PlotChoice)i)) {
                    if (ll == 0) ll = snprintf(line, line_mem.getSize(), "%s", indent);
...
void initWebServer() {
    ...
    char ynot[100];
    if (!restful_server->begin(ynot)) fatalError("Failed to start RESTful server on port %d: %s", restful_port, ynot);
...
void runNextDemoCommand() {
    ...
    time_t t0 = nowWO();
    int t060 = t0 % 60;
    if (t0 == prev_t0 || !(t060 == 15 || t060 == 45)) return;
    prev_t0 = t0;
    ...
    if (prev_choice == DEMO_N) {
        unsigned sum = 0;
        for (int i = 0; i < DEMO_N; i++) sum += item_probs[i];
        if (sum != 100) fatalError("demo probs sum %u != 100\n", sum);
    }
...
static bool runDemoChoice(DemoChoice choice, bool& slow, char msg[], size_t msg_len) {
    ...
    switch (choice) {
        ...
        case DEMO_NEWDX: {
            ...
            newDX(dll, NULL, NULL);
            if (++demo_ll_i == NARRAY(demo_ll)) demo_ll_i = 0;
            ok = true;
        ...
        case DEMO_MAPPROJ:
            ...
            NVWriteUInt8(NV_MAPPROJ, map_proj);
            initEarthMap();
            if (map_proj != MAPP_MERCATOR) slow = true;
            ok = true;
        ...
        case DEMO_NCDXF:
            ...
            switch (brb_mode) {
                case BRB_SHOW_SWSTATS:
                    brb_mode = BRB_SHOW_DXWX;
                    break;
                case BRB_SHOW_DXWX:
                    brb_mode = BRB_SHOW_DEWX;
                    break;
                case BRB_SHOW_DEWX:
        ...
        case DEMO_SAT: {
            if (isSatDefined()) {
                ...
                static const char* sats[] = {"ISS", "SO-50", "FOX-1B"};
                static int next_sat;
        ...
        case DEMO_PSKMASK:
            psk_mask = 0;
            if (random(100) > 50) psk_mask |= PSKMB_OFDE;
            if (random(100) > 50) psk_mask |= PSKMB_CALL;
            if (random(100) > 50) psk_mask |= PSKMB_PSK;
            ok = true;
