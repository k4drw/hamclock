...
static float RADIANS(float deg) { return deg * M_PIF / 180.; }

static float DEGREES(float rad) { return rad * 180. / M_PIF; }
...
static long fnday(int y, uint8_t m, uint8_t d) {
    if (m < 3) {
        m += 12;
        y--;
    }
...
static void fndate(int& y, uint8_t& m, uint8_t& d, long dt) {
    dt += 428L;
...
// overload <
bool DateTime::operator<(const DateTime& rhs) const { return (rhs - *this > 0); }
...
void DateTime::gettime(int& year, uint8_t& month, uint8_t& day, uint8_t& h, uint8_t& m, uint8_t& s) {
...
void DateTime::settime(int year, uint8_t month, uint8_t day, uint8_t h, uint8_t m, uint8_t s) {
...
Observer::Observer(float lat, float lng, float hgt) {
    ...
    float XX = RE * RE;
    float ZZ = RP * RP;
    float D = sqrt(XX * cosf(LA) * cosf(LA) + ZZ * sinf(LA) * sinf(LA));
    float Rx = XX / D + HT;
    float Rz = ZZ / D + HT;
...
static float getfloat(const char* c, int i0, int i1) {
    char buf[20];
    int i;
    for (i = 0; i0 + i < i1; i++) buf[i] = c[i0 + i];
    buf[i] = '\0';
...
Satellite::Satellite(const char* l1, const char* l2) { tle(l1, l2); }

Satellite::~Satellite() {}

void Satellite::tle(const char* l1, const char* l2) {
 ...
 */
void Satellite::topo(const Observer* obs, float& alt, float& az, float& range, float& range_rate) {
    Vec3 R;
    R[0] = S[0] - obs->O[0];
    R[1] = S[1] - obs->O[1];
    R[2] = S[2] - obs->O[2];
    ...
    alt +=
        (1000.0F / 1010.0F) * (283.0F / (273.0F + 10.0F)) * 1.02F / tanf(RADIANS(alt + 10.3F / (alt + 5.11))) / 60.0F;
}

// subsat location
void Satellite::geo(float& lat, float& lng) {
...
// celestial coords
void Satellite::celest(float& lat, float& lng) {
...
float Satellite::viewingRadius(float alt) {
    float h = sqrtf(S[0] * S[0] + S[1] * S[1] + S[2] * S[2]);
    return (acosf(RE / h * cosf(alt)) - alt);
}

bool Satellite::eclipsed(Sun* sp) {
...
void Sun::predict(const DateTime& dt) {
    long DN = dt.DN;
    float TN = dt.TN;
