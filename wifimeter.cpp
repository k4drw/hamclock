...
bool readWiFiRSSI(int& rssi, bool& dbm) {
    ...
    time_t t0 = myNow();
    if (!t0) return (false);
    if (!WiFi.RSSI(rssi, dbm)) return (false);
    ...
    if (!wp_table) fatalError("no memory for %d wifi history", n_wp_table + 1);
...
void plotWiFiHistory() {
    ...
    for (int i = 0; i < n_wp_table; i++) {
        ...
        if (!x_data || !y_data) fatalError("no memory to plot %d wifi", n_data + 1);
    ...
    if (n_data >= 2) plotMapData(title, x_label, x_data, y_data, n_data);
