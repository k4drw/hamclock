 ...
 */
static bool drawBoxText(bool anyway, const SBox& box, const char* str, uint16_t color, uint16_t l1_dy, uint16_t l12_dy,
                        uint16_t l22_dy, bool shadow) {
    // try as one line
    ...
    if (l12_dy == 0 || l22_dy == 0) return (false);
...
static uint16_t getNextColor(uint16_t current, uint16_t contrast) {
    static uint16_t colors[] = {RA8875_RED,    RA8875_GREEN, RA8875_BLUE,  RA8875_CYAN, RA8875_MAGENTA,
                                RA8875_YELLOW, RA8875_WHITE, RA8875_BLACK, DE_COLOR};
#define NCOLORS NARRAY(colors)

    // find index of current color, ok if not found
    unsigned current_i;
    for (current_i = 0; current_i < NCOLORS; current_i++)
        if (colors[current_i] == current) break;
    ...
    for (unsigned cdiff_i = 1; cdiff_i < NCOLORS; cdiff_i++) {
        ...
        if (next_color == contrast) continue;

        // continue scanning if bad combination
        switch (next_color) {
            case RA8875_RED:
                if (contrast == RA8875_MAGENTA || contrast == DE_COLOR) continue;
                break;
            case RA8875_GREEN:  // fallthru
            case RA8875_BLUE:
                if (contrast == RA8875_CYAN) continue;
                break;
            case RA8875_CYAN:
                if (contrast == RA8875_GREEN || contrast == RA8875_BLUE) continue;
                break;
            case RA8875_MAGENTA:
                if (contrast == RA8875_RED || contrast == DE_COLOR) continue;
                break;
            case RA8875_YELLOW:
                if (contrast == RA8875_WHITE) continue;
                break;
            case RA8875_WHITE:
                if (contrast == RA8875_YELLOW) continue;
                break;
            case RA8875_BLACK:
                // black goes with anything
                break;
            case DE_COLOR:
                if (contrast == RA8875_RED || contrast == RA8875_MAGENTA) continue;
                break;
        }
    ...
    if (!drawBoxText(false, box, slash0, fg_c, box.h / 2 + 20, 0, 0, rainbow)) {
        ...
        if (!drawBoxText(false, box, text, fg_c, box.h / 2 + 10, 0, 0, rainbow)) {
            ...
            if (!drawBoxText(false, box, text, fg_c, box.h / 2 + 10, 0, 0, rainbow)) {
                ...
                for (char* z = uc; *z != '\0'; z++) *z = toupper(*z);
...
static void runCallsignMenu(void) {
    ...
    if (runMenu(menu)) {
        ...
        if (cs_info.now_showing != CT_ONAIR) {
            ...
            if (cs_info.ct_prefer != CT_BOTH) cs_info.now_showing = cs_info.ct_prefer;
        }
...
void updateCallsign(bool force_draw) {
    ...
    // draw if desired
    if (force_draw) drawCallsign(true);
}
...
void setCallsignInfo(Call_t t, const char* text, uint16_t* fg, uint16_t* bg, uint8_t* rainbow) {
    switch (t) {
        case CT_CALL:
            ...
            if (cs_info.now_showing != CT_ONAIR) cs_info.now_showing = CT_CALL;
        ...
        case CT_TITLE:
            ...
            if (cs_info.now_showing != CT_ONAIR) cs_info.now_showing = CT_TITLE;
...
static void cycleFGColor(NV_Name fg_e, CallColors_t& cu) {
    ...
    if (fg_e != NV_NONE) NVWriteUInt16(fg_e, cu.fg);
}
...
static void cycleBGColor(NV_Name bg_e, NV_Name rainbow_e, CallColors_t& cu) {
    ...
    if (bg_e != NV_NONE) NVWriteUInt16(bg_e, cu.bg);
    if (rainbow_e != NV_NONE) NVWriteUInt8(rainbow_e, cu.rainbow);
}
