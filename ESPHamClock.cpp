...
const char* grid_styles[MAPGRID_N] = {
    "None", "Tropics", "Lat/Long", "Maidenhead", "Azimuthal", "CQ Zones", "ITU Zones",
};

// map projections
uint8_t map_proj;   // one of MapProjection
#define X(a, b) b,  // expands MAPPROJS to name plus comma
const char* map_projnames[MAPP_N] = {MAPPROJS};
#undef X
...
// de and dx sun rise/set boxes, dxsrss_b also used for DX prefix depending on
// dxsrss
...
// set up TFT display controller RA8875 instance on hardware SPI plus reset and
// chip select
...
static bool dx_prefix_use_override;  // whether to use dx_override_prefixp[] or
                                     // ll2Prefix()
...
const char* detime_names[DETIME_N] = {DETIMES};
#undef X
...
// #define _GFX_TEXTSZ                          // RBF
    ...
    // start WiFi, maps and set de_ll.lat_d/de_ll.lng_d from geolocation or gpsd
    // as desired -- uses tftMsg()
    initSys();
    ...
    if (!skip_skip) {
        ...
        if (new_avail && askOTAupdate(new_version, true, false)) {
            if (askPasswd("upgrade", false)) doOTAupdate(new_version);
            eraseScreen();
        }
    }
#endif  // !NO_UPGRADE
    ...
    if (!NVReadFloat(NV_DX_LAT, &dx_ll.lat_d) || !NVReadFloat(NV_DX_LNG, &dx_ll.lng_d) || !NVReadTZ(NV_DX_TZ, dx_tz)) {
...
void loop() {
    ...
    if (!runStopwatch()) {
        ...
        // update sat pass (this is just the pass; the path is recomputed before
        // each map sweep)
        updateSatPass();
 ...
 * called once at startup and after each time returning from other full-screen
 * options. The overall layout is establihed by setting the various SBox values.
 * Some are initialized statically in setup() some are then set relative to
 * these.
 */
void initScreen() {
    // erase entire screen
    ...
    // set protected region, which requires explicit call to tft.drawPR() to
    // update
    ...
    tft.drawLine(0, tft.height() - 1, tft.width() - 1, tft.height() - 1,
                 GRAY);                                       // bottom
    tft.drawLine(0, map_b.y - 1, 0, tft.height() - 1, GRAY);  // left
    tft.drawLine(tft.width() - 1, map_b.y - 1, tft.width() - 1, tft.height() - 1,
                 GRAY);  // right
    tft.drawLine(map_b.x - 1, map_b.y - 1, map_b.x - 1, tft.height() - 1,
                 GRAY);  // left of map
...
static void checkTouch() {
    ...
    if (wifi_tt != TT_NONE) {
        ...
        // check tap
        tt = readCalTouch(s);
        if (tt == TT_NONE) tt = checkKBWarp(s);
        if (tt == TT_NONE) return;
    ...
    // check lock
    if (inBox(s, lkscrn_b) || inBox(s, demo_b)) runShutdownMenu();
    if (screenIsLocked()) return;

    drainTouch();

    // check all touch locations, ones that can be over map checked first and
    // beware showing PANE_0
    LatLong ll;
    if (inBox(s, view_btn_b)) {
        ...
        // tapped map: set flag to run popup after map finishes or set newDX here if
        // special-tap
        if (names_on) (void)getNearestCity(ll, ll, NULL);  // ll is unchanged if no city found
        ...
        new_avail = newVersionIsAvailable(new_version, sizeof(new_version));
        if (new_avail) {
            if (askOTAupdate(new_version, true, false) && askPasswd("upgrade", false)) doOTAupdate(new_version);
        } else {
...
void newDX(LatLong& ll, const char grid[MAID_CHARLEN], const char* ovprefix) {
    ...
    if (!askPasswd("newdx", true)) return;
...
void newDE(LatLong& ll, const char grid[MAID_CHARLEN]) {
    // require password if set
    if (!askPasswd("newde", true)) return;
    ...
    if (setNewSatCircumstance()) drawSatPass();
...
/* find long- or short-path angular distance and east-of-north bearing from_ll
 * to_ll given helper values for sin and cos of from lat. all values in radians
 * in range 0..2pi.
 */
void propPath(bool long_path, const LatLong& from_ll, float sflat, float cflat, const LatLong& to_ll, float* distp,
              float* bearp) {
    // cdist will be cos of short-path anglar separation in radians, so acos is
    // 0..pi *bearp will be short-path from to ll east-to-north in radians,
    // -pi..pi
    float cdist;
 ...
 * return whether desired units are magnetic, so no change in bear if return
 * false.
 */
    ...
    for (float b = 0; b < 2 * M_PIF; b += deg2rad(PATH_SEGLEN)) {
        ...
        if (s0.x) {
            if (segmentSpanOkRaw(s0, s1, tft.SCALESZ)) {
                if (show_seg) tft.drawLineRaw(s0.x, s0.y, s1.x, s1.y, sp ? sp_wid : lp_wid, sp ? sp_col : lp_col);
            } else {
                s1.x = 0;
            }
        }
 ...
 */
bool waiting4DXPath() { return (dxpath_time > 0 && millis() - dxpath_time < DXPATH_LINGER); }
...
// handy
void fillSBox(const SBox& box, uint16_t color) { tft.fillRect(box.x, box.y, box.w, box.h, color); }
void drawSBox(const SBox& box, uint16_t color) { tft.drawRect(box.x, box.y, box.w, box.h, color); }

/* draw the given string in the given color anchored at x0,y0 with optional
 * black shadow.
 */
 ...
 */
static bool weAreBeta(void) { return (strchr(hc_version, 'b') != NULL); }
#endif  // !NO_UPGRADE
...
static void drawVersion(bool draw) {
    ...
    if (new_avail) {
        ...
        if (autoUpgrade(update_hour)) {
            ...
            // N.B. may or may not allow upgrade this time if currently within the
            // appointed hour
            static time_t next_check;
            if (next_check == 0) {
                // init using local then convert back to utc, insuring first test is
                // always in the future
                int tz = getTZ(de_tz);
                time_t local = now + tz;
                next_check = local - (local % (24 * 3600)) + 3600 * update_hour + random(3600) - tz;
                while (now > next_check) next_check += 24 * 3600;
            ...
            if (now > next_check) {
                ...
                Serial.printf("auto upgrading to %s %ld > %ld hour %d\n", new_version, now, next_check,
                              hour(now + getTZ(de_tz)));
    ...
    if (draw) {
        ...
        tft.setCursor(version_b.x + version_b.w - vw,
                      version_b.y + 1);  // right justify
...
static void drawRotatingMessage() {
    ...
    if (!timesUp(&prev_ms, 5000)) return;

    // default color, cases might change
    tft.setTextColor(GRAY);

    // rotate through the possibilities until one works, ie, puts something into
    // str[] N.B. we assume at least one case will do this!
        ...
        switch (rot_msg) {
            ...
            case ROTM_FSUSE: {
                ...
                if (getFSSize(cap, used)) {
                    ...
                    if (really_is_full) fatalError("Disk is full");
                }
            }
...
time_t getUptime(uint16_t* days, uint8_t* hrs, uint8_t* mins, uint8_t* secs) {
    ...
    if (start_s == 0 || now_s < start_s) start_s = now_s;
...
static void drawUptime(bool force) {
    ...
    static uint32_t prev_ms;
    if (!timesUp(&prev_ms, 1000)) return;
 ...
 */
bool overMapScale(const SCoord& s) { return (mapScaleIsUp() && inBox(s, mapscale_b)); }
...
void drawAllSymbols() {
    ...
    if (mapScaleIsUp()) drawMapScale();
    if (overMap(sun_c.s)) drawSun();
    if (overMap(moon_c.s)) drawMoon();
    updateBeacons(true);
    if (!overRSS(deap_c.s)) drawDEAPMarker();
 ...
 */
bool overRSS(const SCoord& s) { return (rss_on && inBox(s, rss_bnr_b)); }

/* return whether box b is over an active RSS banner
 */
bool overRSS(const SBox& b) { return (rss_on && boxesOverlap(b, rss_bnr_b)); }

/* log for sure and write another line to the initial screen if verbose.
 * verbose messages are arranged in two columns. screen row is advanced
 * afterwards unless this and previous line ended with \r, and fmt == NULL
 * forces return to normal row advancing for next call.
 */
...
#define MSG_ROW1_Y 100        // coordinate with _initial_ cs_info
    ...
    if (hold && prev_hold) tft.fillRect(x, y - (MSG_ROWH - 9), tft.width() / 2, MSG_ROWH, RA8875_BLACK);
...
void drawScreenLock() {
    ...
    if (!screenIsLocked()) tft.fillRect(lkscrn_b.x + hw + 2, lkscrn_b.y, hw, hh, RA8875_BLACK);
}
...
#define N_DEFMT_CORE 8
    ...
    MenuItem mitems[N_DEFMT_CORE + N_PANE_0_CH] = {
        ...
        // outer menu is whether to display the DE pane or use both DE+DX for a
        // pane choice
    ...
    for (int i = 0; i < N_PANE_0_CH; i++) {
        ...
        int plot_n = 0;
        for (uint32_t bits = pane_0_bits; bits >>= 1; plot_n++) continue;
    ...
    if (pane_0_bits != 0) fatalError("drawDEFormatMenu() %d %d 0x%x\n", pane_0_bits, N_PANE_0_CH, PANE_0_CH_MASK);
    ...
    if (runMenu(menu)) {
        ...
        if (mitems[0].set) {
            ...
            // paranoid
            if (new_fmt < 0) fatalError("drawDEFormatMenu: No de fmt");
            ...
            for (int i = 0; i < N_PANE_0_CH; i++) {
                if (mitems[N_DEFMT_CORE + i].set) new_rotset |= (1 << menu_ch[i]);
            }
    ...
    // redraw if normal
    if (!SHOWING_PANE_0()) restoreNormPANE0();
}

/* resume using nearestPrefix
 */
static void unsetDXPrefixOverride() { dx_prefix_use_override = false; }

/* set an override prefix for getDXPrefix() to use instead of using
 * nearestPrefix()
 */
static void setDXPrefixOverride(const char* ovprefix) {
    // extract
...
/* this function positions a box to contain short text beneath a symbol at
 * location s that has radius r, where s is assumed to be over map. the
 * preferred position is centered below s, but it may be moved above or to
 * either side to avoid going off the map. N.B. r == 0 is a special case to mean
 * center the text exactly at s, not beneath it. N.B. coordinate with
 * drawMapTag()
 */
...
/* draw a string within the given box set by setMapTagBox, using the optional
 * color (default white). actually this is no longer a box but text with a
 * shifted background -- more moxy! still, knowing the box has been positioned
 * well allows for this masking. N.B. coordinate with setMapTagBox()
 */
void drawMapTag(const char* tag, const SBox& box, uint16_t txt_color, uint16_t bg_color) {
    ...
    for (unsigned i = 0; i < strlen(tag); i++) tft.fillRect(x0 + i * 6, y0, 5, 7, bg_color);
 ...
 */
bool inBox(const SCoord& s, const SBox& b) { return (s.x >= b.x && s.x < b.x + b.w && s.y >= b.y && s.y < b.y + b.h); }
...
void eraseSCircle(const SCircle& c) {
    ...
    for (int16_t dy = -2 * c.r; dy <= 2 * c.r; dy += 2) {
        for (int16_t dx = -2 * c.r; dx <= 2 * c.r; dx += 2) {
            ...
            if (xy2 <= radius2) drawMapCoord(c.s.x + dx / 2, c.s.y + dy / 2);
        }
    }
}
...
bool postDiags(void) {
    ...
    char fn[300];
    snprintf(fn, sizeof(fn), "/ham/HamClock/diagnostic-logs/dl-%lld-%s-%u.txt", (long long)myNow(), remote_addr,
             ESP.getChipId());
    ...
    for (int i = 0; i < N_DIAG_FILES; i++) {
        ...
        if (stat(dp.c_str(), &s) == 0) cl += s.st_size;
    }

    // add eeprom file
    if (stat(EEPROM.getFilename(), &s) == 0) cl += s.st_size;

    if (pd_client.connect(backend_host, backend_port)) {
        char buf[4096];
        int buf_l = 0;

        // hand-crafted POST header, move to its own func if ever used for something
        // else
        ...
        for (int i = 0; i <= N_DIAG_FILES; i++) {  // 1 more for eeprom
            ...
            if (fp) {
                int n_r;
                while ((n_r = fread(buf, 1, sizeof(buf), fp)) > 0) pd_client.write((uint8_t*)buf, n_r);
                fclose(fp);
            }
        }
 ...
 * N.B. tempting to just assume the args are already a list on the stack but
 * we'd rather not.
 */
...
static bool RUSure(SBox& box, const char* q) {
    ...
    MenuItem mitems[] = {
        {MENU_BLANK, false, 0, 0, NULL, 0}, {MENU_LABEL, false, 0, q_indent, q, 0},
        {MENU_BLANK, false, 0, 0, NULL, 0}, {MENU_LABEL, false, 0, r_indent, r, 0},
...
static void runShutdownMenu(void) {
    ...
    if (runMenu(menu)) {
        ...
        if (mitems[0].set) {
            ...
            if (askPasswd("unlock", true)) setScreenLock(false);
        }
        ...
        if (mitems[2].set) {
            if (askPasswd("configurations", true)) runConfigManagement();
        }
...
void doExit() {
    ...
    defaultState();
#if defined(_USE_FB0)
    // X11 calls doExit on window close, so drawing would be recursive back to
    // that thread
    eraseScreen();
#endif
    _exit(0);
    ...
    if (tft.displayReady()) {
        ...
        for (char* mp = msg; *mp; mp++) {
            if (tft.getCursorX() > tft.width() - char_w || *mp == '\n') {
                ...
                if ((y += line_h) > tft.height() - 3 * line_h) y = line_h;
                tft.setCursor(0, y);
        ...
        UserInput ui = {screen_b, UI_UFuncNone, UF_UNUSED, UI_NOTIMEOUT, UF_NOCLOCKS,
...
void printFreeHeap(const __FlashStringHelper* label) {
    ...
    // record worst
    if (free_heap < worst_heap) worst_heap = free_heap;
    if (stack_used > worst_stack) worst_stack = stack_used;
}
