...
bool checkFSFull(bool& really) {
    ...
    if (timesUp(&check_ms, CHECKFULL_MS)) {
        DSZ_t cap, used;
        while (getFSSize(cap, used) && cap - used < MINFS_FREE && (rmDiagFile() || cleanCache(CLEAN_NAME, CLEAN_AGE)))
            continue;
...
FS_Info* getConfigDirInfo(int* n_info, char** fs_name, DSZ_t* fs_size, DSZ_t* fs_used) {
    // get basic fs info
    DSZ_t cap, used;
    if (!getFSSize(cap, used)) return (NULL);
    ...
    *fs_used = used;
    ...
    int n_fs = 0;
    DIR* dirp = opendir(wd);
    if (!dirp) fatalError("can not open %s: %s", wd, strerror(errno));

    struct dirent* dp;
    while ((dp = readdir(dirp)) != NULL) {
        // extend array
        fs_array = (FS_Info*)realloc(fs_array, (n_fs + 1) * sizeof(FS_Info));
        if (!fs_array) fatalError("alloc file name %d failed", n_fs);
