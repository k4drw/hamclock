...
static void formatTimeLine(const SBox& box, time_t t1, time_t t2, char str[], size_t str_l) {
    if (show_detz) {
        ...
        if (tm1.tm_wday == tm2.tm_wday) {
            ...
            if (strcmp(m1, m2) == 0) {
                // both AM or both PM
                snprintf(str, str_l, "%s %d:%02d - %d:%02d %s", dayShortStr(tm1.tm_wday + 1), h1_12, tm1.tm_min, h2_12,
                         tm2.tm_min, m1);
            } else {
                // different AM PM
                snprintf(str, str_l, "%s %d:%02d %s - %d:%02d %s", dayShortStr(tm1.tm_wday + 1), h1_12, tm1.tm_min, m1,
                         h2_12, tm2.tm_min, m2);
            }
        } else {
            ...
            snprintf(str, str_l, "%s %d:%02d %s - %s %d:%02d %s", wd1, h1_12, tm1.tm_min, m1, wd2, h2_12, tm2.tm_min,
                     m2);
        ...
        if (tm1.tm_wday == tm2.tm_wday) {
            ...
            snprintf(str, str_l, "%s %02d:%02d - %02d:%02dZ", dayShortStr(tm1.tm_wday + 1), tm1.tm_hour, tm1.tm_min,
            ...
            strcpy(wd1, dayShortStr(tm1.tm_wday + 1));
            strcpy(wd2, dayShortStr(tm2.tm_wday + 1));
            snprintf(str, str_l, "%s %02d:%02d - %s %02d:%02dZ", wd1, tm1.tm_hour, tm1.tm_min, wd2, tm2.tm_hour,
                     tm2.tm_min);
        }
    }
...
static bool runContestMenu(const SCoord& s, const SBox& box) {
    ...
    if (s.y >= box.y + START_DY) {
        ...
        if (show_date) display_i /= 2;
        int data_i;
        if (cts_ss.findDataIndex(display_i, data_i)) cep = &contests[data_i];
        // printf ("****************** %d %d %d %d %s\n", s.y, box.y, display_i,
    ...
    // build menu
    MenuItem mitems[] = {
        {cname_mft, false, 0, indent, cname, 0},           {dates_mft, show_date, 1, indent, "Show dates", 0},
        {detz_mft, show_detz, 2, indent, "Use DE TZ", 0},  {alarm_mft, alarm_is_set, 3, indent, "Set alarm", 0},
        {cpage_mft, false, 4, indent, "Show web page", 0}, {weekpage_mft, false, 5, indent, "Show 8-day page", 0},
    };
    ...
    if (runMenu(menu)) {
        ...
        if (cep) {
            ...
            if (mitems[CM_SET_ALARM].set != alarm_is_set)
                setOneTimeAlarmState(mitems[CM_SET_ALARM].set ? ALMS_ARMED : ALMS_OFF, !show_detz, cep->start_t, cname);

            // open contest web page if desired
            if (mitems[CM_SHOW_CTST_PAGE].set) openURL(cep->url);

        } else {
            // open 8-day web page if desired
            if (mitems[CM_SHOW_WEEK_PAGE].set) openURL(week_url);
        }
    }
...
static void scrubContestTitleLine(char* line, const SBox& box) {
    ...
    char* phrase;
    if ((phrase = strstr(line, "Parks on the Air")) != NULL) strcpy(phrase, "POTA");
    ...
    if (ll > 0 && ispunct(line[ll - 1])) line[--ll] = '\0';

    // well just hack off if still too long
    while (getTextWidth(line) >= box.w) line[--ll] = '\0';
}
...
static bool retrieveContests(const SBox& box) {
    ...
    if (ok) qsort(contests, cts_ss.n_data, sizeof(ContestEntry), qsContestStart);
...
static bool checkActiveContests(void) {
    ...
    if (any_past) cts_ss.scrollToNewest();
...
bool checkContestsTouch(const SCoord& s, const SBox& box) {
    if (s.y < box.y + PANETITLE_H) {
        ...
        if (runContestMenu(s, box)) scheduleNewPlot(PLOT_CH_CONTESTS);
