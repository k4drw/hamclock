...
typedef enum { ADIF_SORTS ADIF_SORTS_N } ADIFSorts;  // enum of each sort function
#undef X
 ...
 */
static void drawAllVisADIFSpots(const SBox& box) { drawVisibleSpots(WLID_ADIF, adif_spots, adif_ss, box, ADIF_COLOR); }
...
void drawADIFPane(const SBox& box, const char* filename) {
    ...
    if (info_l > max_chw) info_l = snprintf(info, max_chw, "%s", fn_basename);
 ...
 */
static bool testADIFFilename(struct _menu_text* tfp, Message& ynot) { return (checkADIFFilename(tfp->text, ynot)); }
...
void freshenADIFFile(void) {
    ...
    if (showing_set_adif) return;

    // get full filename
    const char* fn = getADIFilename();
    if (!fn) return;  // not interested
    char fn_exp[1000];
    if (!expandENV(fn, fn_exp, sizeof(fn_exp))) fatalError("ADIF %s failed ENV expansion", fn);

    // check for new/changed file
    if (fsig.fileChanged(fn_exp)) newfile_pending = true;

    // read file if newer and not scrolled away
    if (newfile_pending && adif_ss.atNewest()) {
        // open
        FILE* fp = fopen(fn_exp, "r");
        if (!fp) fatalError("ADIF %s: %s", fn_exp, strerror(errno));  // never returns
        ...
        if (pp != PANE_NONE) drawADIFPane(plot_b[pp], getADIFilename());
...
bool checkADIFTouch(const SCoord& s, const SBox& box) {
    if (s.y < box.y + PANETITLE_H) {
        ...
        // on hold?
        if (ROTHOLD_TST(PLOT_CH_ADIF)) return (true);
        ...
        int data_i;
        if (adif_ss.findDataIndex(row_i, data_i)) newDX(adif_spots[data_i].tx_ll, NULL, adif_spots[data_i].tx_call);
...
bool onADIFList(const DXSpot& spot, bool chk_dxcc, bool chk_grid, bool chk_pref, bool chk_band) {
    ...
    for (int i = 0; i < adif_ss.n_data; i++) {
        ...
        if ((!chk_pref || pref_match) && (!chk_dxcc || spot.tx_dxcc == adif_spot.tx_dxcc) &&
