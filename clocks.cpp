...
const char* auxtime_names[AUXT_N] = {AUXTIMES};
#undef X
...
static void drawUTCButton() {
    ...
    tft.setCursor(x, clock_b.y + vgap + 1);
    tft.print(msg[0]);
    tft.setCursor(x, clock_b.y + 2 * vgap + FFONT_H + 1);
    tft.print(msg[1]);
    tft.setCursor(x, clock_b.y + 3 * vgap + 2 * FFONT_H + 1);
    tft.print(msg[2]);
}
    ...
    if (auxtime == AUXT_DATE) {
        if (all || day != prev_day) {
            ...
            if (getDateFormat() == DF_DMY) {
                ...
                snprintf(buf + buf_l, sizeof(buf) - buf_l, "%2d %s %d", tm_wo.Day, monthShortStr(tm_wo.Month), year);
                uint16_t bw = getTextWidth(buf);
                ...
                int buf_l = snprintf(buf, sizeof(buf), "%s  ", dayShortStr(tm_wo.Wday));
                snprintf(buf + buf_l, sizeof(buf) - buf_l, "%s %2d, %d", monthShortStr(tm_wo.Month), tm_wo.Day, year);
                uint16_t bw = getTextWidth(buf);
                int16_t x = auxtime_b.x + (auxtime_b.w - bw) / 2;
                if (x < 0) x = 0;
                tft.setCursor(x, y);
                ...
                int buf_l = snprintf(buf, sizeof(buf), "%s,  ", dayShortStr(tm_wo.Wday));
                snprintf(buf + buf_l, sizeof(buf) - buf_l, "%d %s %2d", year, monthShortStr(tm_wo.Month), tm_wo.Day);
                uint16_t bw = getTextWidth(buf);
                int16_t x = auxtime_b.x + (auxtime_b.w - bw) / 2;
                if (x < 0) x = 0;
                tft.setCursor(x, y);
...
#define _JDNFRAC 5                 // n fractional digits
        ...
        if (auxtime == AUXT_MJD) d -= 2400000.5;  // MJD
...
#define CAL_BW 4
    ...
    if (!force && prev_Year == tm.Year && prev_Month == tm.Month && prev_Day == tm.Day) return;
    prev_Year = tm.Year;
    prev_Month = tm.Month;
    prev_Day = tm.Day;
 ...
 */
void hideClocks() { hide_clocks = true; }
 ...
 */
time_t nowWO() { return (myNow() + utc_offset); }
...
time_t myNow() {
    ...
    if (t < prev_t) {
        // backwards ?!?!
        if (!time_running_bw) Serial.printf("time: running backwards: %ld -> %ld\n", (long)prev_t, (long)t);
        ...
        // normal advance
        if (time_is_stuck) Serial.printf("time: unstuck at %ld after %d ms\n", (long)t, m - m_ok);
        time_is_stuck = false;
        m_ok = m;
        if (time_running_bw) Serial.printf("time: running forwards now: %ld -> %ld\n", (long)prev_t, (long)t);
...
bool clockTimeOk() {
    ...
    if (!time_ok) startSyncProvider(false);
    return (time_ok);
}

/* return current user offset from UTC
 */
int utcOffset() { return (utc_offset); }
...
void updateClocks(bool all) {
    ...
    if (all || tm_wo.Minute != prev_mn || tm_wo.Hour != prev_hr) {
        ...
        if (prev_hr != tm_wo.Hour) {
            ...
            if (bc_pane != PANE_NONE && bc_matrix.ok) plotBandConditions(plot_b[bc_pane], 0, NULL, NULL);
        }
    ...
    if (utc_offset != 0 || !clockTimeOk()) drawUTCButton();
...
void drawDXSunRiseSetInfo() {
    if (dxsrss == DXSRSS_PREFIX) return;
...
#define _CT_INDENT2 10
            ...
            menu_b.x += 20;
            SBox ok_b;
            MenuInfo menu = {menu_b, ok_b, UF_NOCLOCKS, M_CANCELOK, 1, _CT_N, mitems};
