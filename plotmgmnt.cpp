...
const char* plot_names[PLOT_CH_N] = {PLOTNAMES};
#undef X
...
bool plotChoiceIsAvailable(PlotChoice pc) {
    switch (pc) {
        ...
        case PLOT_CH_COUNTDOWN:
            return (getSWEngineState(NULL, NULL) == SWE_COUNTDOWN);
        case PLOT_CH_DEWX:
            return ((brb_rotset & (1 << BRB_SHOW_DEWX)) == 0);
        case PLOT_CH_DXWX:
            return ((brb_rotset & (1 << BRB_SHOW_DXWX)) == 0);
        case PLOT_CH_ADIF:
            return (getADIFilename() != NULL);
...
void logPaneRotSet(PlotPane pp, PlotChoice pc) {
    ...
    for (int i = 0; i < PLOT_CH_N; i++)
        if (plot_rotset[pp] & (1 << i)) Serial.printf("    %c%s\n", i == pc ? '*' : ' ', plot_names[i]);
}
...
void logBRBRotSet() {
    ...
    for (int i = 0; i < BRB_N; i++)
        if (brb_rotset & (1 << i)) Serial.printf("    %c%s\n", i == brb_mode ? '*' : ' ', brb_names[i]);
...
static PlotChoice askPaneChoice(PlotPane pp) {
    ...
    if (pp == PANE_0) fatalError("askPaneChoice called with pane 0");
...
#define ASKP_SHOWALL 0  // RBF
    ...
    for (int i = 0; i < PLOT_CH_N; i++) {
        ...
        if ((pp_ch == PANE_NONE && plotChoiceIsAvailable(pc)) || pp_ch == pp || ASKP_SHOWALL) {
            ...
            if (!mitems) fatalError("pane alloc: %d", n_mitems);
...
PlotPane findPaneChoiceNow(PlotChoice pc) {
    for (int i = PANE_0; i < PANE_N; i++)
        if (plot_ch[i] == pc) return ((PlotPane)i);
    return (PANE_NONE);
}
...
PlotPane findPaneForChoice(PlotChoice pc) {
    for (int i = PANE_0; i < PANE_N; i++)
        if ((plot_rotset[i] & (1 << pc))) return ((PlotPane)i);
    return (PANE_NONE);
}
...
PlotChoice getNextRotationChoice(PlotPane pp, PlotChoice pc) {
    if (isPaneRotating(pp)) {
        for (int i = 1; i < PLOT_CH_N; i++) {
            ...
            if (plot_rotset[pp] & (1 << j)) return ((PlotChoice)j);
        }
    } else
        return (pc);
...
PlotChoice getAnyAvailablePane0Choice() {
    ...
    for (int pc = 0; pc < PLOT_CH_N; pc++) {
        if (((1 << pc) & PANE_0_CH_MASK) && plotChoiceIsAvailable((PlotChoice)pc) &&
            findPaneForChoice((PlotChoice)pc) == PANE_NONE) {
            ...
            if (plot_rotset[i] & (1 << PLOT_CH_COUNTDOWN)) {
                ...
                if (plot_ch[i] == PLOT_CH_COUNTDOWN) {
                    ...
                    if (!setPlotChoice((PlotPane)i, plot_ch[i])) {
                        fatalError("can not replace Countdown pain %d with %s", i, plot_names[plot_ch[i]]);
                    }
                }
            }
        }
...
bool checkPlotTouch(TouchType tt, const SCoord& s, PlotPane pp) {
    ...
    if (pp == ignorePaneTouch()) return (false);
    ...
    switch (plot_ch[pp]) {
        case PLOT_CH_DXCLUSTER:
            if (checkDXClusterTouch(s, box)) return (true);
            in_top = true;
            break;
        case PLOT_CH_BC:
            if (checkBCTouch(s, box)) return (true);
            in_top = true;
            break;
        case PLOT_CH_CONTESTS:
            if (checkContestsTouch(s, box)) return (true);
            in_top = true;
            break;
        case PLOT_CH_SDO:
            if (checkSDOTouch(s, box)) return (true);
            in_top = true;
            break;
        case PLOT_CH_GIMBAL:
            if (checkGimbalTouch(s, box)) return (true);
            in_top = true;
            break;
        case PLOT_CH_COUNTDOWN:
            if (!in_top) {
            ...
            break;
        case PLOT_CH_MOON:
            if (checkMoonTouch(s, box)) return (true);
            in_top = true;
            break;
        case PLOT_CH_SSN:
            if (!in_top) {
        ...
        case PLOT_CH_FLUX:
            ...
            break;
        case PLOT_CH_PSK:
            if (checkPSKTouch(s, box)) return (true);
            in_top = true;
            break;
        case PLOT_CH_ONTA:
            if (checkOnTheAirTouch(s, box)) return (true);
            in_top = true;
            break;
        case PLOT_CH_ADIF:
            if (checkADIFTouch(s, box)) return (true);
            in_top = true;
            break;
        case PLOT_CH_DXPEDS:
            if (checkDXPedsTouch(s, box)) return (true);
            in_top = true;
            break;
        ...
        case PLOT_CH_TEMPERATURE:
            if (!in_top) {
                if (setPlotChoice(pp, PLOT_CH_HUMIDITY) || setPlotChoice(pp, PLOT_CH_DEWPOINT) ||
        ...
        case PLOT_CH_PRESSURE:
            if (!in_top) {
                if (setPlotChoice(pp, PLOT_CH_TEMPERATURE) || setPlotChoice(pp, PLOT_CH_HUMIDITY) ||
        ...
        case PLOT_CH_HUMIDITY:
            if (!in_top) {
                if (setPlotChoice(pp, PLOT_CH_DEWPOINT) || setPlotChoice(pp, PLOT_CH_PRESSURE) ||
        ...
        case PLOT_CH_DEWPOINT:
            if (!in_top) {
                if (setPlotChoice(pp, PLOT_CH_PRESSURE) || setPlotChoice(pp, PLOT_CH_TEMPERATURE) ||
...
void initPlotPanes() {
    ...
    for (int i = PANE_0; i < PANE_N; i++)
        if (plot_ch[i] != PLOT_CH_NONE) plot_rotset[i] |= (1 << plot_ch[i]);

    // log and save final arrangement
    for (int i = PANE_0; i < PANE_N; i++) logPaneRotSet((PlotPane)i, plot_ch[i]);
    savePlotOps();
}
    ...
    for (int pp = 0; pp < PANE_N; pp++) {
        if (ROTHOLD_TST(plot_ch[pp])) {
            ...
            if (getPaneRotationPeriod() > min_rot)
                c = ((nextPaneRotation((PlotPane)pp) > t0 + PLOT_ROTWARN_DT) || (t0 & 1) == 1) ? RA8875_WHITE : GRAY;
...
#define NFACTOR NARRAY(factor)
    ...
    if (minscale == 0) {
        ...
        for (n = 0; n < numdiv; n++) ticks[n] = min - 1.0 + n * 2.0 / numdiv;
        return (numdiv);
    }
    ...
    for (n = 0; n < (int)NFACTOR; n++) {
        ...
        if ((scale = (powf(10.0F, ceilf(log10f(x))) * factor[n])) < minscale) minscale = scale;
    }
    delta = minscale;

    lo = floor(min / delta);
    for (n = 0; (v = delta * (lo + n)) < max + delta;) ticks[n++] = v;
