...
static void drawDXPedsMarker(const LatLong& ll, uint16_t color) {
    ...
    if (tft.SCALESZ > 1) ll2sRaw(ll, s, MARKER_R1);
...
static void drawDXCStatusBox(void) {
    ...
    if (want_cl) color = cl_ok ? RA8875_GREEN : RA8875_RED;
...
static void drawDXPedsPane(const SBox& box) {
    ...
    char credit[100];
    int c_l = 0;
    for (int i = 0; i < n_credits; i++) c_l += snprintf(credit + c_l, sizeof(credit) - c_l, "%s ", credits[i].name);
    selectFontStyle(LIGHT_FONT, FAST_FONT);
    tft.setTextColor(DXPEDS_COLOR);
    ...
    if (dxp_ss.getVisDataIndices(min_i, max_i) > 0) {
        for (int i = min_i; i <= max_i; i++) {
            ...
            if (show_date) {
                ...
                tft.drawLine(box.x + 1, y + 2 * DXPEDS_DY - 3, box.x + box.w - 2, y + 2 * DXPEDS_DY - 3, 2,
                             DXPEDS_COLOR);
...
static void formatDXPed(DXPedEntry& de, const char* loc_line, const char* call_line, const SBox& box) {
    ...
    if (loc_l > max_loc_l) {
        ...
        if (and_ptr) loc_l = and_ptr - loc_line;
        if (loc_l > max_loc_l) {
...
static bool runDXPedPaneMenu(const SBox& box) {
    ...
    if (!mitems) fatalError("No memory for %d DXP menu items", n_mi);
#define _MAX_CIL 50
    typedef char CredItem_t[_MAX_CIL];
    CredItem_t* citems = (CredItem_t*)calloc(n_credits, sizeof(CredItem_t));  // N.B. free!
    if (!citems) fatalError("No memory for %d DXP credit items", n_credits);
    ...
    if (runMenu(menu)) {
        ...
        // open a web page?
        for (int i = 0; i < n_credits; i++) {
            if (mitems[DEM_N + i].set) openURL(credits[i].url);
        }

        // save any persistent state changes
        if (full_redo) saveDXPedNV();
    }
...
static bool runOneDXPedMenu(const SCoord& s, const SBox& box, DXPedEntry* dep) {
    ...
    if (runMenu(menu)) {
        ...
        if (mitems[DEX_ALARM].set != alarm_is_set)
            setOneTimeAlarmState(mitems[DEX_ALARM].set ? ALMS_ARMED : ALMS_OFF, true, dep->start_t, dep->loc);
        ...
        if (mitems[DEX_PAGE].set) openURL(dep->url);
...
static bool retrieveDXPCredits(FILE* fp) {
    ...
    if (n_credits > 0) {
        ...
        if (!credits) fatalError("No memory for %d DXPeds credits", n_credits);
    } else
    ...
    for (int i = 0; i < n_credits; i++) {
        ...
        if (debugLevel(DEBUG_DXPEDS, 1)) Serial.printf("DXP: credit[%d] %s %s\n", i, credits[i].name, credits[i].url);
    }
...
static bool retrieveDXPeds(const SBox& box) {
    ...
    if (fp) {
        ...
        if (show_date) dxp_ss.max_vis /= 2;

        // first line is number of credits, followed by one line for name and for url
        if (!retrieveDXPCredits(fp)) goto out;  // already logged why
        ...
        while (fgets(line, sizeof(line), fp)) {
            ...
            if (debugLevel(DEBUG_DXPEDS, 2)) Serial.printf("DXP: datum %d: %s\n", dxp_ss.n_data, line);

            // skip if want to hide
            if (!show_hidden && isDXPedsHidden(line)) {
                if (debugLevel(DEBUG_DXPEDS, 2)) Serial.printf("DXP: hiding %s\n", line);
                continue;
            }
            ...
            if (show_current && (now < start_t || now > end_t)) {
                if (debugLevel(DEBUG_DXPEDS, 2)) Serial.printf("DXP: skipping not active: %s\n", call_f);
                continue;
            }
            ...
            if (!dxpeds) fatalError("No memory for %d dxpeds", dxp_ss.n_data + 1);
...
out:
    ...
    if (ok) {
        ...
        Serial.printf("DXP: %s: %d listing, %d found, %d broken\n", dxpeds_fn, dxp_ss.n_data, found_n, found_n_broken);
    }
...
static bool checkActiveDXPeds(void) {
    ...
    if (any_past) dxp_ss.scrollToNewest();
...
bool updateDXPeds(const SBox& box, bool fresh) {
    ...
    if (ok) {
        ...
        if (checkActiveDXPeds() || fresh) drawDXPedsPane(box);
...
void drawDXPedsOnMap(void) {
    ...
    if (pp == PANE_NONE) {
        ...
        for (int i = 0; i < dxp_ss.n_data; i++) {
            ...
            if (color != SPOT_COLOR) drawDXPedsMarker(de.ll, color);
        }
        for (int i = 0; i < dxp_ss.n_data; i++) {
            const DXPedEntry& de = dxpeds[i];
            uint16_t color = renderDXPedColor(de);
...
bool checkDXPedsTouch(const SCoord& s, const SBox& box) {
    if (s.y < box.y + TITLE_DY) {
        ...
        if (runDXPedPaneMenu(box)) scheduleNewPlot(PLOT_CH_DXPEDS);
        ...
        if (show_date) item /= 2;
        int index;
        if (dxp_ss.findDataIndex(item, index)) dep = &dxpeds[index];

        // redo if needed
        if (dep && runOneDXPedMenu(s, box, dep)) scheduleNewPlot(PLOT_CH_DXPEDS);
...
bool getPaneDXPed(const SCoord& s, DXPedEntry*& dxp) {
    ...
    if (!inBox(s, list_b)) return (false);
...
void tellDXPedsSpotChanged(void) {
    if (findPaneForChoice(PLOT_CH_DXPEDS) != PANE_NONE && dxp_ss.atNewest()) scheduleNewPlot(PLOT_CH_DXPEDS);
}
...
static int qsWorkedCmp(const void* v1, const void* v2) {
    ...
    if (diff == 0) diff = strcasecmp(w1->mode, w2->mode);
    return (diff);
}
...
int findDXPedsWorked(const DXPedEntry* dxp, DXPedsWorked*& worked) {
    ...
    for (int i = 0; i < n_adif_worked; i++) {
        ...
        if (wl.dxcc == dxp->dxcc || strcasecmp(wl.prefix, dxp->prefix) == 0) {
            ...
            if (!worked) fatalError("no memory for %d DXPedsWorked", n_worked + 1);
...
bool findDXPedsCall(const DXSpot* sp) {
    for (int i = 0; i < dxp_ss.n_data; i++)
        if (strcasecmp(sp->tx_call, dxpeds[i].call) == 0) return (true);
    return (false);
}
