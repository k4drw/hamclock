...
static KD3Node* find_median(KD3Node* start, KD3Node* end, int level) {
    ...
    while (1) {
        ...
        if (store->s[level] == md->s[level]) return md;
...
void freeKD3NodeTree(KD3Node* a, int n_a) {
    for (int i = 0; i < n_a; i++) free(a[i].data);
    free(a);
}
 ...
 */
float nearestKD3Dist2Miles(float d) { return (ERAD_M * sqrtf(d)); }
...
int main(int ac, char* av[]) {
    ...
    printf(
        ...
        testNode.s[0], testNode.s[1], testNode.s[2], found->s[0], found->s[1], found->s[2], (char*)found->data,
    ...
    for (i = 0; i < test_runs; i++) {
        ...
        if (best_dist > worst_dist) worst_dist = best_dist;
    }
    gettimeofday(&tv1, NULL);
    printf(
        ">> Million tree %d times\n"
