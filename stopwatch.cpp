 ...
 */
static void setAlarmPin(bool set) { mcp.digitalWrite(SW_ALARMOUT_PIN, set); }
...
static void saveSWNV(void) {
    ...
    if (alarm_daily.state != ALMS_OFF) acode += ALMD_NVARMED;
    if (alarm_daily.utc) acode += ALMD_NVUTC;
    NVWriteUInt16(NV_DAILYALARM, acode);

    uint8_t once_mask = 0;
    if (alarm_once.state != ALMS_OFF) once_mask |= ALMO_ARMED_BIT;
    if (alarm_once.utc) once_mask |= ALMO_UTC_BIT;
...
static void loadSWNV(void) {
    ...
    // beware past alarms
    if (once_time < (uint32_t)nowWO()) alarm_once.state = ALMS_OFF;
}
...
static uint32_t getCountdownLeft() {
    if (sws_engine == SWE_COUNTDOWN) {
        ...
        if (since_start < countdown_period) return (countdown_period - since_start);
    }
    return (0);
}
 ...
 */
static void setSWColor() { sw_col = HSV_2_RGB565(sw_hue, SW_HSV_S, SW_HSV_V); }
...
static void drawSWDigit(uint8_t position, uint8_t digit) {
    // check for no change
    if (swdigits[position] == digit) return;
...
static void drawSWTime(uint32_t t) {
    ...
    if (ndig != SW_ND) fatalError("stopwatch %d != %d", ndig, SW_ND);
}
...
static void drawSatIndicator(bool force) {
    ...
    SatNow sn;
    if (!(bc_bits & SW_BCSATBIT) || !getSatNow(sn)) return;
...
static void eraseContestTitle(void) {
    if (sws_display == SWD_MAIN) tft.fillRect(ALMO_TX0, ALMO_TITY, ALMO_TW, 14, RA8875_BLACK);
}
...
static void drawAlarmIndicators(bool label_too) {
    ...
    if (sws_display == SWD_MAIN) {
        ...
        if (!alarm_once.utc) t += getTZ(de_tz);
        breakTime(t, tm);
        snprintf(buf, sizeof(buf), "%s %04d-%02d-%02d %02d:%02d", alarm_once.utc ? "UTC" : "DE  ", tm.Year + 1970,
        ...
        if (label_too) {
            ...
            switch (alarm_once.state) {
                case ALMS_OFF:
                    lbl = "One-Time Alarm off";
                    break;
                case ALMS_ARMED:
                    lbl = "One-Time Alarm armed";
                    break;
                case ALMS_RINGING:
                    lbl = "One-Time Alarm!";
                    break;
        ...
        if (alarm_once.state == ALMS_OFF) {
            ...
            if (!alarm_once.utc) t += getTZ(de_tz);
            breakTime(t, tm);
            snprintf(buf, sizeof(buf), "A:%04d-%02d-%02d %02d:%02d", tm.Year + 1970, tm.Month, tm.Day, tm.Hour,
                     tm.Minute);
...
static void drawCDTimeRemaining(bool force) {
    ...
    if (sws_engine != SWE_COUNTDOWN) return;

    // not crazy fast unless force
    static uint32_t gate;
    if (!force && !timesUp(&gate, 31)) return;
    ...
    if (sws_display == SWD_MAIN) {
        ...
        if (color == prev_color && sc == prev_sc && !force) return;
...
static void drawBCSpaceWxInfo(bool all) {
    if (checkForNewSpaceWx() || all) drawNCDXFSpcWxStats(sw_col);
}
...
static void drawDigitalBigClock(bool all) {
    ...
    if (bc_bits & SW_LSTBIT) {
        ...
        if (!(bc_bits & SW_UTCBIT)) t0 += getTZ(de_tz);  // now local
        hr = hour(t0);
        mn = minute(t0);
        sc = second(t0);
    }
    ...
    if (want_secs) b.x -= (BDC_HMGAP + (2 * BDC_HMW + BDC_HMGAP) / BCD_SSZRED) / 2;  // see below

    // restart if changing whether we have a leading hour digit
    if (leadhr != prev_leadhr) all = true;
    prev_leadhr = leadhr;
    ...
    if (all || ((bc_bits & SW_BCDATEBIT) && (am != prev_am || dy != prev_dy || mo != prev_mo))) {
        ...
        if (bc_bits & SW_BCDATEBIT) drawBCDateInfo(hr, dy, wd, mo);
        prev_am = am;
        prev_dy = dy;
        prev_mo = mo;
    }
    ...
    if (all || hr != prev_hr) {
        ...
        if (hrten >= 0) b.x += BDC_HMW + BDC_HMGAP;
        b.x += BDC_HMW;
    }
    ...
    if (bc_bits & SW_BCSPWXBIT) drawBCSpaceWxInfo(all);
}
...
static void drawAnalogDigital(bool all, int hr, int mn, int sc) {
    ...
    if (all || hr != prev_hr || mn != prev_mn) {
        ...
        if (debugLevel(DEBUG_BC, 1)) tft.drawRect(x0, BACD_DY - 45, 140, 50, RA8875_RED);
    ...
    if (want_seconds && (all || sc != prev_sc)) {
        ...
        if (all || new_tens) {
            ...
            if (debugLevel(DEBUG_BC, 1)) tft.drawRect(prev_stx, BACD_DY - 45, 60, 50, RA8875_RED);
            tft.setCursor(prev_stx, BACD_DY);
            tft.print(sc / 10);
            prev_sux = tft.getCursorX();
        } else {
            tft.fillRect(prev_sux, BACD_DY - 45, 30, 50, RA8875_BLACK);
            if (debugLevel(DEBUG_BC, 1)) tft.drawRect(prev_sux, BACD_DY - 45, 30, 50, RA8875_RED);
        }
...
static void drawAnalogBigClock(bool all) {
    ...
    if (all || ((bc_bits & SW_BCDATEBIT) && (am != prev_am || dy != prev_dy || mo != prev_mo))) {
        ...
        for (uint16_t r = bac_fr + 1; r <= bac_bezr; r++) tft.drawCircle(BAC_X0, bac_y0, r, BAC_BEZCOL);
#else
        ...
        // minute ticks
        for (int i = 0; i < 60; i++) {
            if ((i % 5) == 0) continue;  // don't overdraw hour marks
    ...
    if (hr_moved) {
        ...
        hand[0].x = BAC_X0 + prev_hrdx1;
        hand[0].y = bac_y0 - prev_hrdy1;
        hand[1].x = BAC_X0;
        hand[1].y = bac_y0;
        hand[2].x = BAC_X0 + prev_hrdx2;
        hand[2].y = bac_y0 - prev_hrdy2;
        hand[3].x = BAC_X0 + prev_hrdx3;
        hand[3].y = bac_y0 - prev_hrdy3;
        if (color_hands)
    ...
    if (mn_moved) {
        SCoord hand[4];  // order avoids bug in ESP
        hand[0].x = BAC_X0 + prev_mndx1;
        hand[0].y = bac_y0 - prev_mndy1;
        hand[1].x = BAC_X0;
        hand[1].y = bac_y0;
        hand[2].x = BAC_X0 + prev_mndx2;
        hand[2].y = bac_y0 - prev_mndy2;
        hand[3].x = BAC_X0 + prev_mndx3;
        hand[3].y = bac_y0 - prev_mndy3;
        if (color_hands)
            fillPolygon(hand, NARRAY(hand), RA8875_BLACK);
        else
            drawPolygon(hand, NARRAY(hand), RA8875_BLACK);
...
#define TIP_R 50  // redraw if this close to tip
        for (int i = 1; i <= 12; i++) {
            ...
            int16_t x = BAC_X0 + bac_scr * sinf(theta) - (i < 10 ? 5 : 12);
            ;  // wrt center of number
    ...
    if (want_numbers || mn_moved || hrmn_hit || mnsc_hit) {
        ...
        if (color_hands) {
            ...
            if (hrmn_hit) drawPolygon(hand, NARRAY(hand), RA8875_BLACK);  // mn hand border if near hr hand
        } else {
            fillPolygon(hand, NARRAY(hand), RA8875_BLACK);
            drawPolygon(hand, NARRAY(hand), BAC_HRCOL);
        }
    ...
    if (add_digital) drawAnalogDigital(all, hr, mn, sc);
...
static void showAlarmRinging() {
    ...
    if (sws_display == SWD_NONE) {
        ...
        if (findPaneChoiceNow(PLOT_CH_GIMBAL) == alarm_pane) closeGimbal();
        ...
        if (alarm_daily.state == ALMS_RINGING) alarm_daily.state = ALMS_ARMED;

        // turn off once-only if that's the one ringing
        if (alarm_once.state == ALMS_RINGING) alarm_once.state = ALMS_OFF;
...
#define TER_INDENT 16
    MenuItem mitems[MI_N] = {
        ...
        {MENU_1OFN, (bc_bits & (SW_DB12HBIT | SW_UTCBIT | SW_LSTBIT)) == SW_DB12HBIT, 3, TER_INDENT, "DE 12 hour", 0},
        ...
        {isSatDefined() ? MENU_TOGGLE : MENU_IGNORE, !!(bc_bits & SW_BCSATBIT), 9, SEC_INDENT, "Satellite", 0},
    ...
    if (menu.items[MI_EXT].set) sws_display = SWD_MAIN;
    ...
    if (sws_engine == SWE_COUNTDOWN && !menu.items[MI_ALL_CDW].set) sws_engine = SWE_RESET;
    if (sws_engine != SWE_COUNTDOWN && menu.items[MI_ALL_CDW].set) {
        sws_engine = SWE_COUNTDOWN;
        start_t = millis();
    }

    if (alarm_daily.state == ALMS_OFF && menu.items[MI_ALL_ALMD].set) alarm_daily.state = ALMS_ARMED;
    if (alarm_daily.state != ALMS_OFF && !menu.items[MI_ALL_ALMD].set) alarm_daily.state = ALMS_OFF;

    if (alarm_once.state == ALMS_OFF && menu.items[MI_ALL_ALMO].set) alarm_once.state = ALMS_ARMED;
    if (alarm_once.state != ALMS_OFF && !menu.items[MI_ALL_ALMO].set) alarm_once.state = ALMS_OFF;
...
static void checkSWPageTouch() {
    ...
    SCoord s;
    TouchType tt;
    if ((tt = readCalTouchWS(s)) == TT_NONE && (tt = checkKBWarp(s)) == TT_NONE) return;

    // update idle timer, ignore if this tap is restoring full brightness
    if (brightnessOn()) return;
    ...
    if (sws_display == SWD_MAIN) {
        ...
        if (inBox(s, countdown_lbl_b)) {
            ...
            if (tt == TT_TAP_BX) {
                tooltip(s,
                        "Click time zone to cycle the daily alarm to use DE or UTC,"
            ...
            if (tt == TT_TAP_BX) {
                tooltip(s,
                        "Click to cycle the one-time alarm Armed or Off;"
            ...
            if (tt == TT_TAP) setSWEngineState(new_sws, countdown_period);
...
void drawMainPageStopwatch(bool force) {
    if (sws_engine == SWE_COUNTDOWN) {
        ...
        // erase
        fillSBox(stopwatch_b, RA8875_BLACK);
        if (debugLevel(DEBUG_BC, 1)) drawSBox(stopwatch_b, RA8875_WHITE);
...
bool runStopwatch() {
    ...
    if (countdownSwitchIsTrue()) setSWEngineState(SWE_COUNTDOWN, countdown_period);

    // always check for completed countdown runout
    if (sws_engine == SWE_COUNTDOWN && getCountdownLeft() == 0 && millis() - start_t > countdown_period + SW_CD_AGEDT)
        setSWEngineState(SWE_RESET, 0);

    // always check alarms regardless of display state

    if (dailyAlarmWentOff()) showAlarmRinging();  // might channge alarm_daily.state to ALMS_RINGING
    if (alarm_daily.state == ALMS_RINGING) {
    ...
    if (onetimeAlarmWentOff()) showAlarmRinging();  // might channge alarm_daily.state to ALMS_RINGING
    if (alarm_once.state == ALMS_RINGING) {
    ...
    if (sws_display != SWD_NONE) {
        ...
        if (sws_engine == SWE_COUNTDOWN) drawCDTimeRemaining(false);
        ...
        if (sws_engine == SWE_COUNTDOWN) drawMainPageStopwatch(false);
...
bool setSWEngineState(SWEngineState new_sws, uint32_t ms) {
    switch (new_sws) {
        case SWE_RESET:
            setCDLEDState(SWCDS_OFF);
            if (sws_engine == SWE_RESET) return (true);  // ignore if no change
            sws_engine = SWE_RESET;
            break;
        case SWE_RUN:
            if (sws_engine == SWE_RUN) return (true);  // ignore if no change
            if (sws_engine == SWE_COUNTDOWN) break;    // just continue running countdown
            ...
            break;
        case SWE_STOP:
            if (sws_engine == SWE_STOP) return (true);        // ignore if no change
            if (sws_engine == SWE_COUNTDOWN) return (false);  // stop not implemented for countdown
            stop_dt = millis() - start_t;                     // capture delta
            sws_engine = SWE_STOP;
            break;
        case SWE_LAP:
            if (sws_engine == SWE_LAP) return (true);  // ignore if no change
...
SWEngineState getSWEngineState(uint32_t* sw_timer, uint32_t* cd_period) {
    ...
    if (cd_period) *cd_period = countdown_period;
 ...
 */
SWDisplayState getSWDisplayState() { return (sws_display); }
...
void getOneTimeAlarmState(AlarmState& as, time_t& t_utc, bool& utc, char str[], size_t str_l) {
    ...
    if (!alarm_once.utc) t += getTZ(de_tz);
    breakTime(t, tm);
    snprintf(str, str_l, "%s %04d-%02d-%02d %02d:%02d", alarm_once.utc ? "UTC" : "DE  ", tm.Year + 1970, tm.Month,
...
bool setOneTimeAlarmState(AlarmState as, bool utc, const char* t_str, const char* title) {
    if (as == ALMS_OFF) {
        ...
        if (!alarm_once.utc) user_t -= getTZ(de_tz);  // DE -> UTC
        if (user_t <= nowWO()) return (false);
        ...
        if (title) alarm_once.title = strdup(title);
    }
    saveSWNV();
...
SWBCBits getBigClockBits(void) { return ((SWBCBits)bc_bits); }
