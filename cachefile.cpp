...
static bool fileAgeOk(const char* path, int max_age) {
    if (max_age == CACHE_FOREVER) return (true);
    ...
    if (stat(path, &sbuf) < 0)
        ...
        if (age > max_age)
            ...
            const char* basename = strrchr(path, '/');
            if (basename) path = basename + 1;
            if (debugLevel(DEBUG_CACHE, 1)) Serial.printf("Cache: %s age ok: %d <= %d\n", path, age, max_age);
            return (true);
        }
    }
    return (false);
...
FILE* openCachedFile(const char* fn, const char* url, int max_age, int min_size) {
    ...
    if (strncmp(url, "http", 4) == 0) {
        ...
        char cmd[2048];
        // Try wget then curl
        snprintf(cmd, sizeof(cmd), "wget -q -T 15 -O %s \"%s\" || curl -s -m 15 -o %s \"%s\"", tmp_path, url, tmp_path,
                 url);
        ...
        if (cache_client.connect(backend_host, backend_port)) {
            ...
            if (access(tmp_path, F_OK) == 0) {
                if (unlink(tmp_path) < 0) Serial.printf("Cache: unlink(%s): %s\n", tmp_path, strerror(errno));
            }
        }
    }
