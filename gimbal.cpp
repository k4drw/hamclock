...
// arrows
typedef enum { AR_LEFT, AR_DOWN, AR_UP, AR_RIGHT } ArrowDir;
 ...
 */
static bool goodTime() { return (utcOffset() == 0 && clockTimeOk()); }

/* return whether we are currently connected to hamlib
 */
static bool connectionOk() { return (hamlib_client); }
...
static bool askHamlib(const char* cmd, char rsp[], size_t rsp_len) {
    ...
    if (debugLevel(DEBUG_GIMBAL, 2)) Serial.printf("GBL: ask %s", cmd);  // includes \n
    ...
    while (!found_RPRT && rsp_n < rsp_len && getTCPLine(hamlib_client, rsp + rsp_n, rsp_len - rsp_n, &ll)) {
        if (debugLevel(DEBUG_GIMBAL, 2)) Serial.printf("GBL: reply %s\n", rsp + rsp_n);
        if (sscanf(rsp + rsp_n, "RPRT %d", &RPRT) == 1) found_RPRT = true;
        rsp_n += ll;
    }

    // ok?
    if (found_RPRT && RPRT == 0) return (true);
    if (found_RPRT)
...
static bool connectHamlib(const SBox& box) {
    ...
    if (debugLevel(DEBUG_GIMBAL, 1)) Serial.printf("GBL: starting connection attempt\n");
    ...
    if (!askHamlib("+\\get_info\n", buf, sizeof(buf))) strcpy(buf, "Unknown");
    char* name;
    if (!findHamlibRspKey(buf, "Info", &name)) name = (char*)"Unknown";
...
static void drawTrackButton(bool force, const char* msg) {
    ...
    if (!force && prev_str[0] != 0 && strcmp(str, prev_str) == 0 && prev_track == auto_track) return;
 ...
 */
static void drawAxisInfo(const SBox& box, float target_value, float value_now, SBox& lbox, SBox& rbox, uint16_t y0,
...
static void updateGimbalGUI(const SBox& box) {
    ...
    // draw
    if (azNowChanged() || azTargetChanged()) drawAxisInfo(box, az_target, az_now, azccw_b, azcw_b, AZ_Y, color);
    ...
    if (el_state != ELS_NONE) {
        ...
        // draw
        if (elNowChanged() || elTargetChanged()) drawAxisInfo(box, el_target, el_now, eldown_b, elup_b, EL_Y, color);
    }
    ...
    for (float b = 0; b < end_b; b += wave_step) {
        ...
        for (int i = 0; i <= BEAM_W; i++) {  // inclusive to get symmetric about center
            ...
            if (b > M_PIF) {
                ...
                if ((i % 2) == 0) tft.drawPixelRaw(s1.x, s1.y, col);
...
#define SAT_EL_RSERR 0.2F
    ...
    if (el_state == ELS_NONE || el_max <= 90) return;
...
void stopGimbalNow() {
    if (connectionOk()) {
        char buf[100];
        if (!askHamlib("+\\stop\n", buf, sizeof(buf))) Serial.printf("GBL: %s\n", buf);
    }
 ...
 */
bool haveGimbal() { return (getRotctld(NULL, NULL)); }
...
void updateGimbal(const SBox& box) {
    ...
    if (!timesUp(&prev_ms, UPDATE_MS)) return;

    // init GUI if just opened
    if (!connectionOk()) {
        if (!connectHamlib(box)) return;  // already informed op
        initGimbalGUI(box);
    }
    ...
    if (auto_track) {
        ...
        if (el_state != ELS_NONE) {
            ...
            if (getSatNow(satnow)) {
                ...
                if (!goodTime()) {
                    ...
                    if (isNewPass() || upover_pending) initUpOver();
                    ...
                    if (!upover_pending) {
                        ...
                        if (az_target < az_min) az_target += 360;
                        if (az_target > az_max) az_target -= 360;
                    }
        ...
        if (!sat_ok) {
            ...
            if (el_state != ELS_NONE && el_now > 90) az_target = fmodf(az_target + 180 + 720, 360);
        }

    }  // else move to location commanded from GUI

    if (!user_stop && (azTargetChanged() || elTargetChanged())) setAzEl();
    updateGimbalGUI(box);
}
...
bool checkGimbalTouch(const SCoord& s, const SBox& box) {
    ...
    if (debugLevel(DEBUG_GIMBAL, 1)) Serial.printf("GBL: target after touch %g %g\n", az_target, el_target);
 ...
 */
bool getGimbalState(bool& conn, bool& vis_now, bool& has_el, bool& is_stop, bool& is_auto, float& az, float& el) {
