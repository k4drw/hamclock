...
void plotMapData(const char title[], const char x_label[], float x_data[], float y_data[], int n_data) {
    ...
    for (int i = 0; i < n_data; i++) {
        ...
        if (i > 0) tft.drawLineRaw(prev_x, prev_y, x, y, 2, DATAC);
        prev_x = x;
        prev_y = y;
    }
    ...
    UserInput ui = {map_b, UI_UFuncNone, UF_UNUSED, 30000, UF_CLOCKSOK, {0, 0}, TT_NONE, '\0', false, false};
    (void)waitForUser(ui);
...
out:
    ...
    if (ok) plotMapData(title, x_label, x_data, y_data, n_data);
