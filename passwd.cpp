 ...
 */
static void eraseChar(int p) { tft.fillRect(HM + p * FW, PY - FH + CD, FW + 1, FH, RA8875_BLACK); }
...
static void drawHideShow(const SBox& b, bool hide) { drawStringInBox(hide ? "Show" : "Hide", b, hide, HS_C); }
...
bool askPasswd(const char* category, bool restore) {
    ...
    if (!testPassword(category, NULL) || bypass_pw) return (true);
    ...
    drawActiveCursor(0);
    for (int i = 1; i < NP; i++) drawIdleCursor(i);
    ...
    for (int try_i = 1; !cancelled && !pw_ok && try_i <= NT; try_i++) {
        ...
        UserInput ui = {all, UI_UFuncNone, UF_UNUSED, TO, UF_NOCLOCKS, {0, 0}, TT_NONE, '\0', false, false};
        ...
        do {
            ...
            if (ui.kb_char != CHAR_NONE) {
                switch (ui.kb_char) {
                    ...
                    case CHAR_DEL:
                        if (cur_pos > 0) deleteChar(pw_buf, pw_len, cur_pos, hide);
                        break;
                    ...
                    default:
                        if (isprint(ui.kb_char) && pw_len < NP) insertChar(pw_buf, pw_len, cur_pos, hide, ui.kb_char);
                        break;
                }
            }
