...
class WatchList {
   private:
    ...
    void addFreqRange(float min_kHz, float max_kHz) {
        if (debugLevel(DEBUG_WL, 1)) Serial.printf("WLIST: setting range %g - %g kHz\n", min_kHz, max_kHz);
    ...
    void addPrefix(const char* p) {
        if (debugLevel(DEBUG_WL, 1)) Serial.printf("WLIST: setting prefix %s\n", p);
...
#define _MAX_SUBB 10
        ...
        for (int i = 0; i < n_e; i++) addFreqRange(min_kHz[i], max_kHz[i]);
        return (n_e);
    }
    ...
    bool checkBandRange(const char* token, Message& ynot) {
        ...
        // scan [h1, h2)
        int n_ranges = 0;
        for (int i = h1; i < h2; i++) n_ranges += addSubBand((HamBandSetting)i, units);
        if (n_ranges == 0) {
            ynot.set("no matches");
            return (false);
        }
    ...
    void finishedSpec(void) {
        if (debugLevel(DEBUG_WL, 2) && !finished_spec) Serial.printf("WLIST: spec complete\n");
        finished_spec = true;
    }
    ...
    void resetStorage() {
        if (specs) {
            for (int i = 0; i < n_specs; i++) {
                ...
                if (s.prefs) {
                    for (int j = 0; j < s.n_prefs; j++) free(s.prefs[j]);
                    free(s.prefs);
                    s.prefs = NULL;
                    s.n_prefs = 0;
                }
   ...
   public:
    ...
    void print(WatchListId wl_id) {
        ...
        for (int i = 0; i < n_specs; i++) {
            ...
            for (int j = 0; j < s.n_prefs; j++) printf(" %s%s", j > 0 ? "or " : "", s.prefs[j]);
            printf("\n");

            Serial.printf("    %d Freq ranges:", s.n_freqs);
            for (int j = 0; j < s.n_freqs; j++) printf(" %g-%g", s.freqs[j].min_kHz, s.freqs[j].max_kHz);
            printf("\n");

            char buf[200];
            size_t bl = snprintf(buf, sizeof(buf), "    ADIF:");
            if (s.noadif_dxcc) bl += snprintf(buf + bl, sizeof(buf) - bl, " %s", NOTADIFDXCC_KW);
            if (s.noadif_band) bl += snprintf(buf + bl, sizeof(buf) - bl, " %s", NOTADIFBAND_KW);
            if (s.noadif_pref) bl += snprintf(buf + bl, sizeof(buf) - bl, " %s", NOTADIFPREF_KW);
            if (s.noadif_grid) bl += snprintf(buf + bl, sizeof(buf) - bl, " %s", NOTADIFGRID_KW);
            Serial.printf("%s\n", buf);
        }
    }
    ...
    bool onList(const DXSpot& spot) {
        ...
        for (int i = 0; i < n_specs && !spot_matches; i++) {
            ...
            // good?
            if (match_pref && match_freq && match_noadif) spot_matches = true;

            if (debugLevel(DEBUG_WL, 2)) {
                Serial.printf("WLIST: %s (%s %s) %g: prefix?%c freq?%c ADIF?%c: %s\n", spot.tx_call, home_call, dx_call,
                              spot.kHz, match_pref ? 'Y' : 'N', match_freq ? 'Y' : 'N', match_noadif ? 'Y' : 'N',
    ...
    bool compile(const char* wl_specs, Message& ynot) {
        ...
        for (const char* wl_walk = wl_specs; true; wl_walk += tok_len) {
            ...
            if (strcasecmp(token, NOTADIFDXCC_KW) == 0) {
                if (!checkADIFFilename(getADIFilename(), ynot)) return (false);
            ...
            if (strchr(token, '-')) {
                if (!checkBandRange(token, ynot)) return (false);
                continue;
            }

            // something else?
            if (!checkPrefix(token, ynot)) return (false);
        }
...
static bool anyWLADIFKW(const char* s) {
    return (strcistr(s, NOTADIFDXCC_KW) || strcistr(s, NOTADIFGRID_KW) || strcistr(s, NOTADIFBAND_KW) ||
            strcistr(s, NOTADIFPREF_KW));
}
 ...
 */
bool wlIdOk(WatchListId wl_id) { return (wl_id >= 0 && wl_id < WLID_N); }
...
WatchListShow checkWatchListSpot(WatchListId wl_id, const DXSpot& dxsp) {
    if (!wlIdOk(wl_id)) fatalError("checkWatchList bogus %d", (int)wl_id);
...
bool compileWatchList(WatchListId wl_id, const char* wl_str, Message& ynot) {
    if (!wlIdOk(wl_id)) fatalError("compileWatchList bogus is %d", (int)wl_id);
...
static bool compileTestWatchList(struct _menu_text* tfp, Message& ynot) {
    ...
    if (lookupWatchListState(tfp->label) == WLA_OFF) return (true);
