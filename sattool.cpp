...
static void getObsCir(Observer* de_obsp, Observer* dx_obsp, time_t t0, SatAzEl& de_azel, SatAzEl& dx_azel) {
    SatNow snow;
    if (!getSatCir(de_obsp, t0, snow)) fatalError("SatTool failed to get sat DE info");
    de_azel.az = deg2rad(snow.az);
    de_azel.el = deg2rad(snow.el);
    if (!getSatCir(dx_obsp, t0, snow)) fatalError("SatTool failed to get sat DX info");
...
static void drawSTSetup(time_t t0) {
    // get sat name, if up
    SatNow snow;
    if (!getSatNow(snow)) fatalError("SatTool failed to get sat name");
...
static void drawSTElPlot(time_t t0, time_t& t_start, time_t& t_end) {
    ...
    for (time_t t = t0; t <= t0 + ST_DUR; t += ST_DT) {
        ...
        if (!prev_both_up && both_up_now) {
            ...
            if (x + width > ST_X0 + ST_PW) width = ST_X0 + ST_PW - x;
...
static void drawSTBothUpTable(time_t t0, time_t t_start, time_t t_end) {
    ...
    int detz = getTZ(de_tz);
    if (better_start == t0) {
        snprintf(buf, sizeof(buf), "DE    now    %02d:%02d", hour(better_end + detz), minute(better_end + detz));
    } else {
        snprintf(buf, sizeof(buf), "DE   %02d:%02d   %02d:%02d", hour(better_start + detz), minute(better_start + detz),
    ...
    int dxtz = getTZ(dx_tz);
    if (better_start == t0) {
        snprintf(buf, sizeof(buf), "DX    now    %02d:%02d", hour(better_end + dxtz), minute(better_end + dxtz));
    } else {
        snprintf(buf, sizeof(buf), "DX   %02d:%02d   %02d:%02d", hour(better_start + dxtz), minute(better_start + dxtz),
...
static void drawSTPopup(const time_t t, const SBox& popup_b) {
    ...
    int detz = getTZ(de_tz);
    snprintf(buf, sizeof(buf), "DE  %02d:%02d  %3.0f %4.0f", hour(t + detz), minute(t + detz), rad2deg(de_azel.az),
             rad2deg(de_azel.el));
    ...
    int dxtz = getTZ(dx_tz);
    snprintf(buf, sizeof(buf), "DX  %02d:%02d  %3.0f %4.0f", hour(t + dxtz), minute(t + dxtz), rad2deg(dx_azel.az),
             rad2deg(dx_azel.el));
...
void drawSatTool() {
    ...
    UserInput ui = {map_b, UI_UFuncNone, UF_UNUSED, ST_TO, UF_CLOCKSOK, {0, 0}, TT_NONE, '\0', false, false};

    while (waitForUser(ui)) {
        // done if return, esc or tap Resume button or tap outside box
        if (ui.kb_char == CHAR_CR || ui.kb_char == CHAR_NL || ui.kb_char == CHAR_ESC || inBox(ui.tap, resume_b) ||
        ...
        if (ui.tap.x > ST_X0 && ui.tap.x < ST_X0 + ST_PW && ui.tap.y > ST_E2Y(M_PI_2F) && ui.tap.y < ST_E2Y(-M_PI_2F) &&
            ...
            if (popup_b.x + popup_b.w > ST_X0 + ST_PW) popup_b.x = ST_X0 + ST_PW - popup_b.w;
            if (popup_b.y + popup_b.h > ST_E2Y(-M_PI_2F) - ST_MT) popup_b.y = ST_E2Y(-M_PI_2F) - ST_MT - popup_b.h;
