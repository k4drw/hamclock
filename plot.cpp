 ...
 */
bool plotXY(const SBox& box, float x[], float y[], int nxy, const char* xlabel, const char* ylabel, uint16_t color,
            float y_min, float y_max, float label_value) {
    char buf[32];
 ...
 */
bool plotXYstr(const SBox& box, float x[], float y[], int nxy, const char* xlabel, const char* ylabel, uint16_t color,
               float y_min, float y_max, char* label_str) {
    ...
    if (!overlay) {
        ...
        if (xray_plot) {
            ...
            for (int i = 0; i < nyt; i++) {
                ...
                switch ((int)yticks[i]) {
                    case -9:
                        tft.setCursor(tx - TICKLEN - 2 * FONTW - 1, ty - FONTH / 2);
                        tft.print(-9);
                        break;
                    case -8:
                        tft.print('A');
        ...
        if (ttx + tl >= box.x + box.w) ttx = box.x + box.w - tl;
...
#define PCHG_LG 3   // gap either side of label
        ...
        if (showATMhPa()) {
            tft.setCursor(pchg_x, pchg_y);
            tft.print("h");
            tft.setCursor(pchg_x, pchg_y + 8);
            tft.print("P");
            tft.setCursor(pchg_x, pchg_y + 14);
            tft.print("a");
        } else {
#ifdef _INHG
            tft.setCursor(pchg_x, pchg_y - 4);
            tft.print("i");
            tft.setCursor(pchg_x, pchg_y + 3);
            tft.print("n");
            tft.setCursor(pchg_x, pchg_y + 10);
            tft.print("H");
            tft.setCursor(pchg_x, pchg_y + 17);
            tft.print("g");
#else
            tft.setCursor(pchg_x, pchg_y + 4);
            tft.print("i");
            tft.setCursor(pchg_x, pchg_y + 12);
            tft.print("n");
#endif
        }
        ...
        if (wi.pressure_chg > 0)
            tft.fillTriangle(pchg_x + PCHG_W, pchg_y, pchg_x, pchg_y + PCHG_H, pchg_x + 2 * PCHG_W, pchg_y + PCHG_H,
                             color);
        else if (wi.pressure_chg < 0)
            tft.fillTriangle(pchg_x, pchg_y, pchg_x + 2 * PCHG_W, pchg_y, pchg_x + PCHG_W, pchg_y + PCHG_H, color);
        else
            tft.fillTriangle(pchg_x, pchg_y + 10 - PCHG_SH, pchg_x + 2 * PCHG_W, pchg_y + 10, pchg_x,
                             pchg_y + 10 + PCHG_SH, color);
...
#define RELTOA_COLOR RGB565(0x58, 0xa0, 0xc8)  // TOA/REL marker color
    ...
    if (!draw_all) tft.fillRect(box.x + 1, timeline_y - 1, box.w - 2, PFONT_H + 1, RA8875_BLACK);
...
void prepPlotBox(const SBox& box) {
    ...
    if (memcmp(&box, &plot_b[PANE_0], sizeof(SBox)) == 0) tft.drawLine(box.x, by, rx, by, BORDER_COLOR);  // bottom
}
...
bool installBMPBox(GenReader& gr, const SBox& box, ImageRefit fit, Message& ynot) {
    ...
    uint16_t* pix_565;
    if (!readBMPImage(gr, raw_b, pix_565, fit, ynot)) return (false);

    // fill raw_b with image
    uint16_t* pix = pix_565;
    for (int dy = 0; dy < raw_b.h; dy++)
        for (int dx = 0; dx < raw_b.w; dx++) tft.drawPixelRaw(raw_b.x + dx, raw_b.y + dy, *pix++);
