...
#define CLIENT_VALID(cli) \
    ((cli) != NULL && (cli) >= &client_socks[0] && (cli) <= &client_socks[MAX_CLIENTS - 1] && (cli)->client_sock > -1)
...
static int get_client_state(ws_cli_conn_t* client) {
    ...
    if (!CLIENT_VALID(client)) return (-1);
...
static void* close_timeout(void* p) {
    ws_cli_conn_t* conn = (ws_cli_conn_t*)p;
    struct timespec ts;
    int state;
    ...
    while (conn->state != WS_STATE_CLOSED &&
           pthread_cond_timedwait(&conn->cnd_state_close, &conn->mtx_state, &ts) != ETIMEDOUT);

    state = conn->state;
    pthread_mutex_unlock(&conn->mtx_state);

    /* If already closed. */
    if (state == WS_STATE_CLOSED) goto quit;
...
static int start_close_timeout(ws_cli_conn_t* client) {
    ...
    if (client->state != WS_STATE_OPEN) goto out;
...
static void set_client_address(ws_cli_conn_t* client) {
    ...
    if (getpeername(client->client_sock, (struct sockaddr*)&addr, &addr_size) < 0) return;
...
char* ws_getaddress(ws_cli_conn_t* client) {
    if (!CLIENT_VALID(client)) return (NULL);
 ...
 *
 * @return Returns the number of msg bytes written, -1 if error.
 *
...
int ws_sendframe(ws_cli_conn_t* client, const char* msg, uint64_t size, int type) {
    ...
    output = 0;
    if (client) output = SEND(client, response, idx_response);
    ...
    free(response);
    return ((int)(output - idx_first_rData));
}
...
static void send_ping_close(ws_cli_conn_t* cli, int threshold, int lock) {
    ...
    if (!CLIENT_VALID(cli) || get_client_state(cli) != WS_STATE_OPEN) return;
...
void ws_ping(ws_cli_conn_t* cli, int threshold) {
    ...
    if (cli) send_ping_close(cli, threshold, 1);
 ...
 * @see WS_STATE_CLOSED
 */
int ws_get_state(ws_cli_conn_t* client) { return (get_client_state(client)); }
...
int ws_close_client(ws_cli_conn_t* client) {
    ...
    if (!CLIENT_VALID(client) || client->client_sock == -1) return (-1);
...
static int do_handshake(struct ws_frame_data* wfd) {
    ...
    /* Advance our pointers after the header */
    wfd->amt_read = strlen((char*)wfd->frm);
    wfd->cur_pos = wfd->amt_read;
...
static int do_close(struct ws_frame_data* wfd, int close_code) {
    ...
    if (wfd->frame_size == 0 || wfd->frame_size > 2) goto send;
 ...
 * for completeness.
 */
static int read_frame(struct ws_frame_data* wfd, int opcode, unsigned char** buf, uint64_t* frame_length,
                      uint64_t* frame_size, uint64_t* msg_idx, uint8_t* masks, int is_fin) {
    ...
    if (*frame_length == 126) *frame_length = (((uint64_t)next_byte(wfd)) << 8) | next_byte(wfd);
    ...
    /* 64-bit messages. */
        ...
        *frame_length = (((uint64_t)next_byte(wfd)) << 56) | /* frame[2]. */
                        ...
                        (((uint64_t)next_byte(wfd)) << 8) | (((uint64_t)next_byte(wfd))); /* frame[9]. */
    }
    ...
    *frame_size += *frame_length;
    ...
    if (*frame_size > MAX_FRAME_LENGTH) {
        DEBUG(
            "Current frame from client %d, exceeds the maximum\n"
    ...
    if (*frame_length > 0) {
        if (!is_control_frame(opcode)) {
            tmp = (unsigned char*)realloc(msg, sizeof(unsigned char) * (*msg_idx + *frame_length + is_fin));
            if (!tmp) {
    ...
    if (is_fin && *frame_size > 0) {
        ...
        if (!*frame_length && !is_control_frame(opcode)) {
            tmp = (unsigned char*)realloc(msg, sizeof(unsigned char) * (*msg_idx + 1));
            if (!tmp) {
                DEBUG("Cannot allocate memory, requested: %" PRId64 "\n", (*msg_idx + 1));
...
static int next_frame(struct ws_frame_data* wfd) {
    ...
    do {
        ...
        if (opcode == WS_FR_OP_TXT || opcode == WS_FR_OP_BIN || opcode == WS_FR_OP_CONT || opcode == WS_FR_OP_PING ||
            ...
            if (get_client_state(wfd->client) == WS_STATE_CLOSING && opcode != WS_FR_OP_CLSE) {
                DEBUG(
                    "Unexpected frame received, expected CLOSE (%d), "
                    "received: (%d)",
            ...
            if (opcode != WS_FR_OP_CONT && !is_control_frame(opcode)) wfd->frame_type = opcode;
            ...
            if (is_control_frame(opcode) && (!is_fin || frame_length > 125)) {
                DEBUG(
                    "Control frame bigger than 125 octets or not a FIN "
                    "frame!\n");
                wfd->error = 1;
                break;
            }
            ...
            if (opcode == WS_FR_OP_TXT || opcode == WS_FR_OP_BIN || opcode == WS_FR_OP_CONT) {
                if (read_frame(wfd, opcode, &msg_data, &frame_length, &wfd->frame_size, &msg_idx_data, masks_data,
                               is_fin) < 0)
                    break;
             ...
             */
                ...
                if (read_frame(wfd, opcode, &msg_ctrl, &frame_length, &frame_size, &msg_idx_ctrl, masks_ctrl, is_fin) <
                    0)
                    break;
                ...
                if (frame_size != sizeof(wfd->client->last_pong_id)) continue;
            ...
            /* We should answer to a PING frame as soon as possible. */
                ...
                if (do_pong(wfd, frame_size) < 0) break;
...
/**
 * @brief read the http header from sock down through and including \r\n\r\n.
 *
 * @param sock web client tcp socket
 *
 * @param header pointer to malloced header pointer.
 *
 * @return Returns 0 if success else -1 if never see final blank line.
 *
 * @note sock is left positioned immediately after the blank line.
 *
 * @note caller must always free *header regrdless of return value.
 *
 * @attention This is part of the internal API and is documented just
 * for completeness.
 */
static int readHTTPHeader(int sock, char** header) {
    int n_hdr = 0;
    int n_malloc = 1000;
    char* hdr = (char*)malloc(n_malloc);
    int found = -1;

    /* this is not efficient but we must read one char at a time to insure sock is
     * left positioned just after the blank line.
     */
    while (found == -1 && read(sock, &hdr[n_hdr], 1) == 1) {
        n_hdr++;
        if (n_hdr == n_malloc - 1) /* also need room for final \0 */
            hdr = (char*)realloc(hdr, (n_malloc += 1000));
        if (n_hdr >= 4 && strncmp(&hdr[n_hdr - 4], "\r\n\r\n", 4) == 0) found = 0;
    }
 ...
 * events when occurs one or invoke onnonws() for normal http headers.
 *
...
static void* ws_establishconnection(void* vclient) {
    ...
    client = (ws_cli_conn_t*)vclient;

    /* read header */
    if (readHTTPHeader(client->client_sock, &client->header) < 0) goto closed;

    /* Prepare frame data. */
    memset(&wfd, 0, sizeof(wfd));
    strncpy((char*)wfd.frm, client->header, MESSAGE_LENGTH - 1); /* -1 to insure EOS */
    wfd.client = client;

    /* Do handshake else assume normal http request */
...
closed:
    ...
    if (get_client_state(client) != WS_STATE_CLOSED) close_client(client, 1);
    ...
    while (1) {
        ...
        for (i = 0; i < MAX_CLIENTS; i++) {
            if (client_socks[i].client_sock == -1) {
                ...
                if (pthread_mutex_init(&client_socks[i].mtx_state, NULL)) fatalError("Error on allocating close mutex");
                if (pthread_cond_init(&client_socks[i].cnd_state_close, NULL))
                    fatalError("Error on allocating condition var");
                if (pthread_mutex_init(&client_socks[i].mtx_snd, NULL)) fatalError("Error on allocating send mutex");
...
int ws_socket(struct ws_events* evs, uint16_t port, int thread_loop, uint32_t timeout_ms) {
    ...
    if (evs == NULL) fatalError("Invalid event list");
...
#ifdef _WIN32
    WSADATA wsaData;
    if (WSAStartup(MAKEWORD(2, 2), &wsaData) != 0) fatalError("WSAStartup failed");
    ...
    if (sock < 0) fatalError("Could not create socket: %s", strerror(errno));
