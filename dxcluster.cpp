...
static void rebuildDXWatchList(void) {
    ...
    for (int i = 0; i < n_dxspots; i++) {
        ...
        if (spot.spotted >= oldest && checkWatchListSpot(WLID_DX, spot) != WLS_NO) {
            ...
            if (!dxwl_spots) fatalError("No mem for %d watch list spots", dxc_ss.n_data + 1);
...
static void logDXSpot(const char* label, const DXSpot& spot) {
    ...
    dxcLog("%s: DX de %-10s %8.1f  %-44s%02d%02dZ\n", label, spot.rx_call, spot.kHz, spot.tx_call, spot_time->tm_hour,
           spot_time->tm_min);
}
...
static void addDXClusterSpot(DXSpot& new_spot, bool set_dx) {
    ...
    for (int i = 0; i < n_dxspots; i++) {
        ...
        if (spot.spotted < ancient) {
            ...
            if (new_spot.spotted > spot.spotted) {
                dxcLog("%s %g: updated %02d%02dZ > %02d%02dZ\n", new_spot.tx_call, new_spot.kHz, new_hr, new_mn, spt_hr,
                       spt_mn);
                ...
                dxcLog("%s %g: superseded %02d%02dZ < %02d%02dZ\n", spot.tx_call, spot.kHz, new_hr, new_mn, spt_hr,
                       spt_mn);
            }
        }
    }
    ...
    if (!dxc_spots) fatalError("No memory for %d DX spots", n_dxspots + 1);
...
static void incLostConn(void) {
    uint8_t n_lostconn;
    if (!NVReadUInt8(NV_DXMAX_N, &n_lostconn)) n_lostconn = 0;
    n_lostconn += 1;
...
static void detectMultiConnection(const char* line) {
    ...
    if (strstr(line, "econnected") != NULL || strstr(line, "Dupe call") != NULL) multi_cntn = true;
}
...
static bool sendNextUserCommand(bool restart) {
    ...
    // skip if RO
    if (cl_type == CT_READONLY) return (false);
    ...
    // restart if desired
    if (restart) next_cmd = 0;

    // send next until last
    while (next_cmd < N_DXCLCMDS && (!dx_on[next_cmd] || strlen(dx_cmds[next_cmd]) == 0)) next_cmd++;
    if (next_cmd < N_DXCLCMDS) dxcSendMsg("%s\n", dx_cmds[next_cmd++]);
...
static void showDXCHost(const SBox& box, uint16_t c) {
    ...
    if (cl_type == CT_UDP && !isHostMulticast(dxhost)) {
        ...
        if (hw > box.w - 5) snprintf(host, sizeof(host), "%s", dxhost);
    }
...
static void runDXClusterMenu(const SBox& box) {
    ...
    for (int i = 0; i < N_DXCAGES; i++) snprintf(dxages_str[i], sizeof(dxages_str[i]), "%d m", dxc_ages[i]);
...
static bool getNextDXCLine(char line[], size_t ll) {
    ...
    if (ok) {
        if (queryForQRA(line)) dxc_updateDE = true;
        dxc_activity_ms = millis();
    }
    return (ok);
}
...
static void incomingUDP() {
    ...
    while ((p_len = getUDPPacket(packet, sizeof(packet) - 1)) > 0) {  // allow for adding EOS
        ...
        // add EOS
        if (debugLevel(DEBUG_DXC, 1)) dxcLog("UDP: read packet containing %d bytes\n", p_len);
        packet[p_len] = '\0';
...
static void incomingDXC() {
    ...
    while (getNextDXCLine(line, sizeof(line))) {
        ...
        // crack and add
        DXSpot new_spot;
        if (crackClusterSpot(line, new_spot)) addDXClusterSpot(new_spot, false);  // already logged above
    }
    ...
    if (cl_type != CT_READONLY && timesUp(&dxc_activity_ms, HBEAT_MS)) sendHeartbeat();
    ...
    if (useWSJTX()) {
        ...
        if (dxc_client.connect(dxhost, dxport)) {
            ...
            while (cl_type == CT_UNKNOWN && rx_gt == false && getTCPLine(dxc_client, buf, bufl, &bl)) {
                ...
                if (buf[bl - 1] == '>') rx_gt = true;
            }

            // if no id string but do see > assume it's M0CKE's fire hose
            if (cl_type == CT_UNKNOWN && rx_gt) cl_type = CT_READONLY;
...
bool updateDXCluster(const SBox& box, bool fresh) {
    ...
    if (dxc_ss.atNewest()) {
        ...
        if (showingNewSpot()) dxc_ss.drawNewSpotsSymbol(true, false);  // show passively
        ROTHOLD_SET(PLOT_CH_DXCLUSTER);                                // disable rotation
    }
...
void checkDXCluster() {
    ...
    if (!isDXClusterConnected()) return;

    // not crazy fast
    static uint32_t prev_check;
    if (!timesUp(&prev_check, BGCHECK_DT)) return;
...
bool checkDXClusterTouch(const SCoord& s, const SBox& box) {
    if (s.y < box.y + PANETITLE_H) {
        ...
        // on hold?
        if (ROTHOLD_TST(PLOT_CH_DXCLUSTER)) return (true);
    ...
    int spot_row;
    if (dxc_ss.findDataIndex(vis_row, spot_row) && dxwl_spots[spot_row].tx_call[0] != '\0' && isDXClusterConnected())
 ...
 */
bool isDXClusterConnected() { return (useDXCluster() && (dxc_client || udp_server)); }
...
void drawDXClusterSpotsOnMap() {
    ...
    if (!dxpeds_using && !dxc_using) return;
...
bool getClosestDXCluster(LatLong& ll, DXSpot* sp, LatLong* llp) {
    // skip if we are not running
    if (!isDXClusterConnected()) return (false);
    ...
    // skip if neither of these are running
    if (!dxpeds_using && !dxc_using) return (false);
...
bool getDXCPaneSpot(const SCoord& ms, DXSpot* dxs, LatLong* ll) {
    ...
    if (!inBox(ms, plot_b[pp])) return (false);
...
void dxcLog(const char* fmt, ...) {
    ...
    for (char* bell = msg; (bell = strchr(bell, '\a')) != NULL; bell++) *bell = ' ';
 ...
 */
void sendDXClusterDELLGrid(void) { dxc_updateDE = true; }
...
const DXSpot* findDXCCall(const char* call) {
    ...
    if (dxpedsWatchingCluster() && isDXClusterConnected()) {
        for (int i = 0; i < n_dxspots; i++)
            if (strcasecmp(dxc_spots[i].tx_call, call) == 0) return (&dxc_spots[i]);
    }
