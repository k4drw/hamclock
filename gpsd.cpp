 ...
 *   while true; do echo '"class":"TPV","mode":2,"lat":34.567,"lon":-123.456,"time":"2020-01-02T03:04:05.000Z"'; done |
 * nc -k -l 192.168.7.11 2947
 */
...
static bool lookforTime(const char* buf, void* arg) {
    ...
    for (const char* bp = buf; !found_tpv && (classstr = strstr(bp, "\"class\":\"")) != NULL; bp = classstr + 9) {
        if (strncmp(classstr + 9, "TPV", 3) == 0) found_tpv = true;
    }
    if (!found_tpv) {
...
static bool lookforLatLong(const char* buf, void* arg) {
    ...
    if (!found_tpv) return (false);

    const char* modestr = strstr(classstr, "\"mode\":");
    if (!modestr || atoi(modestr + 7) < 2) return (false);

    const char* latstr = strstr(classstr, "\"lat\":");
    if (!latstr || strlen(latstr) < 6 + 10) return (false);

    const char* lonstr = strstr(classstr, "\"lon\":");
    if (!lonstr || strlen(latstr) < 6 + 10) return (false);
...
static bool getGPSDSomething(bool (*lookf)(const char* buf, void* arg), void* arg) {
    ...
    if (!useGPSDTime() && !useGPSDLoc()) return (false);
...
#define MAXGLL 1000  // max line length
    ...
    if (gpsd_client.connect(host, GPSD_PORT)) {
        ...
        for (size_t ll = 0; !timesUp(&t0, GPSD_TO) && ll < MAXGLL && !look_ok && getTCPChar(gpsd_client, &line[ll]);
             /* none */) {
            // hopeful?
            got_something = true;
...
time_t getGPSDUTC(void) {
    ...
    if (getGPSDSomething(lookforTime, &gpsd_time)) return (gpsd_time);
