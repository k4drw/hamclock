...
static unsigned char* myZDeflate(unsigned char* data, int data_len, int* out_len, int quality) {
    ...
    if (!out_mem) bye("myZDeflate no memory for %d\n", n_out);
    ...
    if (ret != Z_OK) bye("myZDeflate deflateInit returns %d\n", ret);
    ...
    if (ret != Z_STREAM_END) bye("myZDeflate error %d %d/%d\n", ret, *out_len, data_len);
...
static void updateExistingClient(ws_cli_conn_t* client) {
    ...
    if (!img_client) bye("No memory for LIVE update\n");
    memcpy(img_client, pixels, LIVE_NBYTES);

    // replace clients's pixels with current screen contents
    if (!tft.getRawPix(pixels, LIVE_NPIX)) bye("getRawPix for update failed\n");
...
#define CTR_RAWY (tft.SCALESZ * (lkscrn_b.y + lkscrn_b.h + 3))
        ...
        if (client->port == liveweb_ro_port) {
            ...
            if (n_roweb < 10) digit_b.x += CTR_RAWW;
            ...
            if (n_rwweb < 10) digit_b.x += CTR_RAWW;
...
#error too many live regions
    ...
    for (int ry = 0; ry < BLOK_NROWS; ry++) {
        ...
        if (memcmp(&img_now[band_start], &img_client[band_start], LIVE_BYPPIX * BLOK_H * BUILD_W) == 0) continue;
    ...
    if (!chg_regns) bye("No memory for sprites %d\n", n_bloks);
    ...
    stbi_write_png_to_func(wifiSTBWrite_helper, client, BLOK_W * n_bloks, BLOK_H, COMP_RGB, chg_regns,
                           BLOK_WBYTES * n_bloks);

    if (debugLevel(DEBUG_WEB, 2)) {
        struct timeval tv1;
        gettimeofday(&tv1, NULL);
        Serial.printf("LIVE: client %s: write hdr %d bytes and update took %ld usec\n", ws_getaddress(client), hdr_l,
                      TVDELUS(tv0, tv1));
    }
...
static void sendClientPNG(ws_cli_conn_t* client) {
    ...
    // fresh capture
    if (!tft.getRawPix(pixels, LIVE_NPIX)) bye("getRawPix for png failed\n");
    ...
    if (debugLevel(DEBUG_WEB, 1)) Serial.printf("LIVE: client %s: sent full PNG\n", ws_getaddress(client));
}

/* send message that user wants full screen.
 * N.B. coordinate with liveweb-html
 */
static void sendFullScreen(ws_cli_conn_t* client) { ws_sendframe_txt(client, "full-screen"); }
...
static void getLiveUpdate(ws_cli_conn_t* client, char args[], size_t args_len) {
    ...
    if (liveweb_fs_ready && getWebFullScreen()) sendFullScreen(client);
...
static void setLiveChar(ws_cli_conn_t* client, char args[], size_t args_len) {
    ...
    if (client->port == liveweb_ro_port) {
        if (debugLevel(DEBUG_WEB, 1)) Serial.printf("LIVE: ignoring setLiveChar on r/o port %d\n", liveweb_ro_port);
        return;
    }
    ...
    if (!parseWebCommand(wa, args, args_len)) {
        ...
        if (c) {
            ...
            if (debugLevel(DEBUG_WEB, 1)) Serial.printf("LIVE: set_char %d %c\n", c, c);
        }
    }
}
...
static void setLiveTouch(ws_cli_conn_t* client, char args[], size_t args_len) {
    // ignore if this is from the r/o port
    if (client->port == liveweb_ro_port) {
        if (debugLevel(DEBUG_WEB, 1)) Serial.printf("LIVE: ignoring setLiveTouch on r/o port %d\n", liveweb_ro_port);
        return;
    }
...
static void setLiveMouse(ws_cli_conn_t* client, char args[], size_t args_len) {
    // ignore if this is from the r/o port
    if (client->port == liveweb_ro_port) {
        if (debugLevel(DEBUG_WEB, 1)) Serial.printf("LIVE: ignoring setLiveMouse on r/o port %d\n", liveweb_ro_port);
        return;
    }
    ...
    if (!parseWebCommand(wa, args, args_len)) {
        ...
        int y = atoi(wa.value[1]);
        tft.setMouse(x, y);
        if (debugLevel(DEBUG_WEB, 1)) Serial.printf("LIVE: set_mouse %d %d\n", x, y);
    }
}
...
static void sendLiveHTML(FILE* sockfp) {
    ...
    if (debugLevel(DEBUG_WEB, 1)) Serial.printf("LIVE: sent live.html\n");
}
...
static void sendLiveFavicon(FILE* sockfp) {
    ...
    if (debugLevel(DEBUG_WEB, 1)) Serial.printf("LIVE: sent favicon\n");
}
...
static void ws_onopen(ws_cli_conn_t* client) {
    ...
    if (!new_sip) {
        if (si_n < liveweb_max) {
            ...
            if (!si_list) bye("No memory for new live session info %d\n", si_n);
    ...
    if (new_sip) {
        ...
        if (!new_sip->pixels) bye("No memory for new live session pixels\n");
...
static void ws_onmessage(ws_cli_conn_t* client, const unsigned char* msg, uint64_t size, int type) {
    ...
    static struct {
        ...
        {"get_live.bin?", getLiveUpdate}, {"set_mouse?", setLiveMouse},  {"set_touch?", setLiveTouch},
        {"set_char?", setLiveChar},       {"get_live.png?", getLivePNG},
    };
    ...
    for (int i = 0; !found && i < NARRAY(commands); i++) {
        ...
        if (strncmp(cmd, cmd_name, cmd_name_len) == 0) {
            if (debugLevel(DEBUG_WEB, 2)) Serial.printf("LIVE: running %s\n", cmd);
    ...
    // check
    if (!found) Serial.printf("LIVE: ignoring unknown ws command: %s\n", cmd);
}
...
static void ws_not(FILE* sockfp, const char* header) {
    ...
    if (debugLevel(DEBUG_WEB, 1)) Serial.printf("LIVE: ws_not GET %s\n", fn);
...
void initLiveWeb(bool verbose) {
    if (verbose) {
        ...
        if (liveweb_max >= MAX_CLIENTS) bye("liveweb_max must be %d < %d\n", liveweb_max, MAX_CLIENTS);
        ...
        // R/W service
        if (liveweb_rw_port < 0) {
            if (debugLevel(DEBUG_WEB, 1)) Serial.printf("LIVE: R/W live web is disabled\n");
        } else {
        ...
        // R/O service
        if (liveweb_ro_port < 0) {
            if (debugLevel(DEBUG_WEB, 1)) Serial.printf("LIVE: R/O live web is disabled\n");
        } else {
            struct ws_events evs;
            evs.onopen = ws_onopen;
            evs.onclose = ws_onclose;
        ...
        // log timeout, if any
        if (liveweb_to > 0) Serial.printf("LIVE: timeout set to %d minutes\n", liveweb_to);
    }
}
 ...
 */
bool isLiveWebTouch(void) { return (lastest_ws_touch_client != NULL); }
