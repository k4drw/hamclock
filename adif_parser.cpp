...
static bool parseDT2UNIX(const char* date, const char* tim, time_t& unix) {
    ...
    if (sscanf(date, "%4d%2d%2d", &yr, &mo, &dd) != 3 || sscanf(tim, "%2d%2d%2d", &hh, &mm, &ss) < 2) {
        if (debugLevel(DEBUG_ADIF, 2)) Serial.printf("ADIF: parseDT2UNIX(%s, %s) failed\n", date, tim);
        return (false);
    }
    ...
    if (debugLevel(DEBUG_ADIF, 3)) Serial.printf("ADIF: spotted %s %s -> %ld\n", date, tim, (long)unix);
...
static bool parseADIFBand(const char* band, float& kHz) {
    ...
    static ADIFBand bands[] = {
        {"2190m", 0.1357}, {"630m", 0.472}, {"560m", 0.501},   {"160m", 1.8},   {"80m", 3.5},   {"60m", 5.36},
        {"40m", 7.0},      {"30m", 10.1},   {"20m", 14.0},     {"17m", 18.068}, {"15m", 21.0},  {"12m", 24.890},
        {"10m", 28.0},     {"8m", 40},      {"6m", 50},        {"5m", 54},      {"4m", 70},     {"2m", 144},
        {"1.25m", 222},    {"70cm", 420},   {"33cm", 902},     {"23cm", 1240},  {"13cm", 2300}, {"9cm", 3300},
        {"6cm", 5650},     {"3cm", 10000},  {"1.25cm", 24000}, {"6mm", 47000},  {"4mm", 75500}, {"2.5mm", 119980},
        {"2mm", 134000},   {"1mm", 241000}, {"submm", 300000},
    };

    for (int i = 0; i < NARRAY(bands); i++) {
        if (strcasecmp(band, bands[i].name) == 0) {
            kHz = 1e3 * bands[i].MHz;
            if (findHamBand(kHz) != HAMBAND_NONE) return (true);
        }
    }
    return (false);
}
...
static bool parseADIFLocation(const char* loc, float& degs) {
    ...
    int deg;
    float min;
    if (sscanf(loc, "%c%d %f", &dir, &deg, &min) != 3) return (false);

    degs = deg + min / 60;
    if (tolower(dir) == 'w' || tolower(dir) == 's') degs = -degs;
...
static bool addADIFFIeld(ADIFParser& adif, DXSpot& spot) {
    ...
    if (!strcasecmp(adif.name, "OPERATOR")) {
        ...
        if (!parseADIFLocation(adif.value, spot.rx_ll.lat_d) || spot.rx_ll.lat_d < -90 || spot.rx_ll.lat_d > 90) {
            if (debugLevel(DEBUG_ADIF, 3)) Serial.printf("ADIF: line %d bogus MY_LAT %s\n", adif.line_n, adif.value);
            return (false);
        }
        ADD_AFB(adif, AFB_MY_LAT);

    } else if (!strcasecmp(adif.name, "MY_LON")) {
        if (!parseADIFLocation(adif.value, spot.rx_ll.lng_d) || spot.rx_ll.lng_d < -180 || spot.rx_ll.lng_d > 180) {
            if (debugLevel(DEBUG_ADIF, 3)) Serial.printf("ADIF: line %d bogus MY_LON %s\n", adif.line_n, adif.value);
            return (false);
        }
        ...
        if (findHamBand(spot.kHz) == HAMBAND_NONE) {
            if (debugLevel(DEBUG_ADIF, 3)) Serial.printf("ADIF: line %d bogus FREQ %s\n", adif.line_n, adif.value);
            return (false);
        }
        ...
        if (!parseADIFLocation(adif.value, spot.tx_ll.lat_d) || spot.tx_ll.lat_d < -90 || spot.tx_ll.lat_d > 90) {
            if (debugLevel(DEBUG_ADIF, 3)) Serial.printf("ADIF: line %d bogus LAT %s\n", adif.line_n, adif.value);
            return (false);
        }
        ADD_AFB(adif, AFB_LAT);

    } else if (!strcasecmp(adif.name, "LON")) {
        if (!parseADIFLocation(adif.value, spot.tx_ll.lng_d) || spot.tx_ll.lng_d < -180 || spot.tx_ll.lng_d > 180) {
            if (debugLevel(DEBUG_ADIF, 3)) Serial.printf("ADIF: line %d bogus LON %s\n", adif.line_n, adif.value);
            return (false);
        }
...
static bool spotLooksGood(ADIFParser& adif, DXSpot& spot) {
    if (!(CHECK_AFB(adif, AFB_CALL) || CHECK_AFB(adif, AFB_CONTACTED_OP))) {
        if (debugLevel(DEBUG_ADIF, 2)) Serial.printf("ADIF: line %d No CALL or CONTACTED_OP\n", adif.line_n);
        return (false);
    }

    if (!(CHECK_AFB(adif, AFB_FREQ) || CHECK_AFB(adif, AFB_BAND))) {
        if (debugLevel(DEBUG_ADIF, 2)) Serial.printf("ADIF: line %d No FREQ or BAND\n", adif.line_n);
        return (false);
    }

    if (!(CHECK_AFB(adif, AFB_QSO_DATE))) {
        if (debugLevel(DEBUG_ADIF, 2)) Serial.printf("ADIF: line %d No QSO_DATE\n", adif.line_n);
        return (false);
    }

    if (!(CHECK_AFB(adif, AFB_TIME_ON))) {
        if (debugLevel(DEBUG_ADIF, 2)) Serial.printf("ADIF: line %d No TIME_ON\n", adif.line_n);
        return (false);
    }
    ...
    if (!CHECK_AFB(adif, AFB_MODE)) {
        if (debugLevel(DEBUG_ADIF, 2)) Serial.printf("ADIF: line %d No MODE\n", adif.line_n);
        return (false);
    }

    if (!(CHECK_AFB(adif, AFB_OPERATOR) || CHECK_AFB(adif, AFB_STATION_CALLSIGN))) {
        // assume us?
        if (debugLevel(DEBUG_ADIF, 3)) Serial.printf("ADIF: assuming RX is %s\n", getCallsign());
    ...
    if (!CHECK_AFB(adif, AFB_MY_LAT) || !CHECK_AFB(adif, AFB_MY_LON)) {
        if (CHECK_AFB(adif, AFB_MY_GRIDSQUARE)) {
            ...
            if (debugLevel(DEBUG_ADIF, 3))
                Serial.printf("ADIF: line %d: add ll for %s from grid %s\n", adif.line_n, spot.rx_call, spot.rx_grid);
        } else {
    ...
    if (!CHECK_AFB(adif, AFB_MY_DXCC)) {
        if (!call2DXCC(spot.rx_call, spot.rx_dxcc)) {
            if (debugLevel(DEBUG_ADIF, 2)) Serial.printf("ADIF: line %d no DXCC for %s\n", adif.line_n, spot.rx_call);
            return (false);
        }
    }
...
static bool parseADIF(char c, ADIFParser& adif, DXSpot& spot) {
    ...
    switch (adif.ps) {
        ...
        case ADIFPS_STARTSPOT:
            // init spot
            if (debugLevel(DEBUG_ADIF, 3)) Serial.printf("ADIF: starting new spot scan\n");
            spot = {};
        ...
        case ADIFPS_INNAME:
            if (c == ':') {
                ...
                if (!strcasecmp(adif.name, "EOH")) {
                    if (debugLevel(DEBUG_ADIF, 3)) Serial.printf("ADIF: found EOH\n");
                    adif.ps = ADIFPS_STARTSPOT;
                } else if (!strcasecmp(adif.name, "EOR")) {
                    // yah!
                    if (debugLevel(DEBUG_ADIF, 3)) Serial.printf("ADIF: found EOR\n");
                ...
                if (debugLevel(DEBUG_ADIF, 3))
                    Serial.printf("ADIF: line %d: ignoring long name %.*s %d > %d\n", adif.line_n, adif.name_seen,
                                  adif.name, adif.name_seen, (int)(sizeof(adif.name) - 1));
        ...
        case ADIFPS_INLENGTH:
            if (c == ':') {
                ...
                if (debugLevel(DEBUG_ADIF, 3)) Serial.printf("ADIF: line %d: in type for %s\n", adif.line_n, adif.name);
        ...
        case ADIFPS_INVALUE:
            ...
            if (adif.value_seen == adif.value_len) {
                ...
                if (adif.value_seen < sizeof(adif.value)) {
                    ...
                    Serial.printf("ADIF: ignoring long value <%s:%d>%.*s %d > %d\n", adif.name, adif.value_len,
                                  adif.value_seen, adif.value, adif.value_len, (int)(sizeof(adif.value) - 1));
                }
                // start next field
                adif.ps = ADIFPS_STARTFIELD;
            }
        ...
        case ADIFPS_SKIPTOEOR:
            ...
            if (c == '>') {
                if (adif.name_seen < sizeof(adif.name)) {
                    adif.name[adif.name_seen] = '\0';
                    if (strcasecmp(adif.name, "EOR") == 0) adif.ps = ADIFPS_STARTSPOT;
                }
                adif.name_seen = 0;
            } else if (c == '<') {
                adif.name_seen = 0;
    ...
    if (finished && debugLevel(DEBUG_ADIF, 3)) Serial.printf("ADIF: finished parsing\n");
    return (finished);
}
            ...
            if (spotLooksGood(adif, spot)) {
                ...
                if (wl_ok) {
                    ...
                    if (n_good + 1 > n_malloc) {
                        ...
                        if (!spots) fatalError("No memory for %d ADIF Spots", n_malloc);
                    }
                    spots[n_good++] = spot;
                }
                ...
                if ((wl_ok && debugLevel(DEBUG_ADIF, 1)) || (!wl_ok && debugLevel(DEBUG_ADIF, 2))) {
                    Serial.printf("ADIF: %s %-9.9s %-6.6s %4d  %-9.9s %-6.6s %4d  %5.1f %6.1f %4.4s %8.1f\n",
                                  wl_ok ? "OK" : "NO", spot.rx_call, spot.rx_grid, spot.rx_dxcc, spot.tx_call,
                                  spot.tx_grid, spot.tx_dxcc, spot.tx_ll.lat_d, spot.tx_ll.lng_d, spot.mode, spot.kHz);
                }
            } else
                n_bad++;  // count actual broken spots, not ones that just aren't selected by WL

            // look alive
            if (((n_good + n_bad) % 100) == 0) updateClocks(false);
        }
    }
