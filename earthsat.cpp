...
static const char esat_url[] =
    "https://celestrak.org/NORAD/elements/gp.php?GROUP=amateur&FORMAT=tle";  // server file URL
 ...
 */
void satResetIO() { disableBlinker(SATALARM_PIN); }
...
static int currentSat(void) {
    if (sat_state[0].sat && dxpaneSat == 0) return 0;

    if (sat_state[1].sat && dxpaneSat == 1) return 1;
...
static void updateFootPrint(SatState& s, float satlat, float satlng) {
    ...
    for (uint8_t alt_i = 0; alt_i < N_FOOT; alt_i++) {
        ...
        if (!s.foot[alt_i] && n_malloc > 0) fatalError("no memort for sat foot: %d", n_malloc);
        ...
        for (uint16_t foot_i = 0; foot_i < m; foot_i++) {
            ...
            if (n_foot == 0 || memcmp(&s.foot[alt_i][n_foot], &s.foot[alt_i][n_foot - 1], sizeof(SCoord))) n_foot++;
        }
...
#define FINE_DT (-2L)                    // seconds/step backward for refined search
    ...
    if (name) {
        ...
        Serial.printf("SAT: %*s @ %04d-%02d-%02d %02d:%02d:%02d next rise in %6.3f hrs, set in %6.3f (%u ms)\n",
                      NV_SATNAME_LEN, name, yr, mo, dy, hr, mn, sc, rs.rise_ok ? 24 * (rs.rise_time - t_now) : 0.0F,
                      rs.set_ok ? 24 * (rs.set_time - t_now) : 0.0F, millis() - t0);
    }
}
...
static void drawSatSkyDome(SatState& s) {
    ...
    // erase sky dome
    tft.fillRect(dx_info_b.x + 1, dx_info_b.y + 2 * FONT_H + 1, dx_info_b.w - 2, dx_info_b.h - 2 * FONT_H - 1,
                 RA8875_BLACK);

    // skip if no sat or never up
    if (!s.sat || !obs || !s.rs.ever_up) return;
    ...
    if (s.rs.rise_ok && s.rs.set_ok) {
        ...
        if (n_steps > MAX_PASS_STEPS) n_steps = MAX_PASS_STEPS;
...
#define HGRIDCOL RGB565(50, 90, 50)
    ...
    // draw elevations
    for (uint8_t el = 30; el < 90; el += 30) tft.drawCircle(xc, yc, r0 * (90 - el) / 90, HGRIDCOL);
    ...
    for (uint8_t i = 0; i < n_steps; i++) {
        ...
        if (el < 0 && n_steps == 1) break;  // only showing pos now but it's down
    ...
    if (max_el > 0 && full_pass) {
        ...
        if (draw_left_of_pass) x = tft.getCursorX() - bw + 4;  // account for deg
...
static void drawSatName(SatState& s) {
    if (!s.sat || !obs || !SAT_NAME_IS_SET(s) || !dx_info_for_sat || SHOWING_PANE_0()) return;
...
static void setSatMapNameLoc(SatState& s) {
    ...
    if (!overMap(s.name_b)) s.name_b.x = 0;
}
...
static void fatalSatError(const char* fmt, ...) {
    ...
    UserInput ui = {ok_b, UI_UFuncNone, UF_UNUSED, UI_NOTIMEOUT, UF_NOCLOCKS, {0, 0}, TT_NONE, '\0', false, false};
    (void)waitForUser(ui);

    // restart without sats
    for (int i = 0; i < MAX_ACTIVE_SATS; i++) unsetSat(sat_state[i]);
    initScreen();
}
 ...
 */
static bool readNextSat(FILE*& fp, RNS_t& state, char name[NV_SATNAME_LEN], char t1[TLE_LINEL], char t2[TLE_LINEL]) {
...
next:
    ...
    if (state == RNS_INIT) {
        if (!fp) {
            ...
            if (!fp) {
                if (debugLevel(DEBUG_ESATS, 1)) Serial.printf("SAT: %s: %s\n", esat_ufn, strerror(errno));
                state = RNS_SERVER;
            }
        }
    }
    ...
    if (state == RNS_DONE) return (false);
...
static bool satLookup(SatState& s) {
    if (!SAT_NAME_IS_SET(s)) return (false);

    if (debugLevel(DEBUG_ESATS, 1)) Serial.printf("SAT: Looking up '%s'\n", s.name);
...
static bool satNameIsActive(const char* name) {
    for (int i = 0; i < MAX_ACTIVE_SATS; i++) {
        ...
        if (s.sat && SAT_NAME_IS_SET(s) && strcasecmp(s.name, name) == 0) return (true);
    }
    return (false);
}
...
static int askSat(char selections[MAX_ACTIVE_SATS][NV_SATNAME_LEN]) {
    ...
    UserInput ui = {screen_b, UI_UFuncNone, UF_UNUSED, MENU_TO, UF_NOCLOCKS, {0, 0}, TT_NONE, '\0', false, false};
    ...
    for (n_sat_table = 0; n_sat_table < MAX_NSAT; n_sat_table++) {
        ...
        char t1[TLE_LINEL];
        char t2[TLE_LINEL];
        if (!readNextSat(rns_fp, rns_state, tbl_name, t1, t2)) break;
        ...
        if (satEpochOk(sat, tbl_name, now)) {
            ...
            if (rs.rise_ok) {
                ...
                if (rs.rise_time < rs.set_time) {
                    ...
                    if (hrs_to_rise * 60 < SOON_MINS) tft.setTextColor(SOON_COLOR);
                    int mins_to_rise = (hrs_to_rise - floor(hrs_to_rise)) * 60;
                    if (hrs_to_rise < 1 && mins_to_rise < 1) mins_to_rise = 1;  // 00:00 looks wrong
                    if (hrs_to_rise < 10) tft.print('0');
                    tft.print((uint16_t)hrs_to_rise);
                    tft.print(':');
                    if (mins_to_rise < 10) tft.print('0');
...
static PassState findPassState(SatRiseSet& rs, float* days, float* az) {
    ...
    if (!rs.ever_up || !rs.ever_down) {
        ...
        if (t_now < rs.rise_time) {
            ...
            // just set
            ps = PS_HASSET;
            if (days) *days = t_now - rs.set_time;
        }
...
static bool checkSatUpToDate(SatState& s, bool* updated) {
    ...
    if (!obs || !SAT_NAME_IS_SET(s)) return (false);
    ...
    if (!satLookup(s)) return (false);  // already posted error
    ...
    for (int i = 0; i < MAX_ACTIVE_SATS; i++) {
        ...
        if (s.sat) {
            ...
            if (!ok)
                ...
                n_ok++;
                if (!updated) findNextPass(s.sat, s.name, nowWO(), s.rs);
            }
        }
    }
 ...
 */
bool getSatNow(SatNow& satnow) { return (getSatCir(obs, nowWO(), satnow)); }
...
bool getSatCir(Observer* snow_obs, time_t t0, SatNow& sat_at_t0) {
    ...
    int cs = currentSat();
    if (!obs || cs == NO_CUR_SAT) return (false);
    SatState& s = sat_state[cs];
    ...
    // times
    if (s.rs.rise_ok) sat_at_t0.rdt = (s.rs.rise_time - t_now) * 24;
    if (s.rs.set_ok) sat_at_t0.sdt = (s.rs.set_time - t_now) * 24;
...
static void drawSatPass(SatState& s) {
    if (!obs || !dx_info_for_sat || SHOWING_PANE_0()) return;
...
void updateSatPass() {
    ...
    if (!timesUp(&last_run, 1000)) return;

    // done if can't even get the basics
    bool fresh_update;
    if (!checkSatUpToDate(s, &fresh_update)) return;
...
void updateSatPath() {
    ...
    for (int i = 0; i < MAX_ACTIVE_SATS; i++) {
        ...
        // ski if not defined
        if (!s.sat || !obs || !SAT_NAME_IS_SET(s)) continue;
        ...
        if (!s.path) fatalError("No memory for satellite path");
        ...
        for (uint16_t p = 0; p < max_path; p++) {
            ...
            // skip duplicate points
            if (s.n_path == 0 || memcmp(&s.path[s.n_path], &s.path[s.n_path - 1], sizeof(SCoord))) s.n_path++;
...
void drawSatPathAndFoot() {
    for (int i = 0; i < MAX_ACTIVE_SATS; i++) {
        ...
        if (s.show_path && pw) {
            ...
            for (last_vis_i = s.n_path - 1; s.path[last_vis_i].x == OFFSCRN; --last_vis_i) continue;
            for (int i = 1; i < s.n_path; i++) {
                ...
                if (segmentSpanOkRaw(sp0, sp1, 2 * pw)) {
                    ...
                    if (i == last_vis_i || ((dashed && !prev_vis) || (!dashed && (i % ARROW_EVERY) == 0))) {
        ...
        if (fw) {
            ...
            for (int alt_i = 0; alt_i < N_FOOT; alt_i++) {
                for (uint16_t foot_i = 0; foot_i < s.n_foot[alt_i]; foot_i++) {
                    ...
                    if (segmentSpanOkRaw(sf0, sf1, 1)) tft.drawLineRaw(sf0.x, sf0.y, sf1.x, sf1.y, fw, fc);
                }
            }
        }
    }
...
void drawSatName(void) {
    for (int i = 0; i < MAX_ACTIVE_SATS; i++) {
        ...
        if (!s.sat || !obs || s.name_b.x == 0 || (dx_info_for_sat && !SHOWING_PANE_0() && nActiveSats() < 2)) return;
...
bool querySatSelection() {
    ...
    for (int i = 0; i < n_sel; i++) {
        ...
        if (!satLookup(s)) return (false);  // already showed err
        NVWriteString(s.nv_name, s.name);
...
bool initSat() {
    ...
    s0.cs = SAT1_CSPR;
    if (!NVReadString(NV_SAT1NAME, s0.name) || !SAT_NAME_IS_SET(s0) || !checkSatUpToDate(s0, NULL)) unsetSat(s0);
    ...
    s1.cs = SAT2_CSPR;
    if (!NVReadString(NV_SAT2NAME, s1.name) || !SAT_NAME_IS_SET(s1) || !checkSatUpToDate(s1, NULL)) unsetSat(s1);
    ...
    if (s1.sat && !s0.sat) {
        ...
        if (!checkSatUpToDate(s0, NULL)) fatalError("sat %s disappeared", s0.name);
    }
...
#define _SNE_OKY 12         // Ok box y
        ...
        for (int i = 0; i < n_times; i++) {
            ...
            if (rt_wd != st_wd) {
                ...
                if ((y += _SNE_ROWH) > tft.height()) {
                    if ((x += tft.width() / 2) > tft.width()) break;  // no more room
    ...
    UserInput ui = {ok_b, UI_UFuncNone, UF_UNUSED, _SNE_TIMEOUT, UF_NOCLOCKS, {0, 0}, TT_NONE, '\0', false, false};

    do {
        waitForUser(ui);
    } while (!(ui.kb_char == CHAR_CR || ui.kb_char == CHAR_NL || ui.kb_char == CHAR_ESC || inBox(ui.tap, ok_b)));
...
void drawDXSatMenu(const SCoord& s) {
    ...
    if (n_sats > 0) strncpySubChar(name1, sat_state[0].name, ' ', '_', NV_SATNAME_LEN);
    if (n_sats > 1) strncpySubChar(name2, sat_state[1].name, ' ', '_', NV_SATNAME_LEN);
 ...
 */
bool isSatDefined() { return (nActiveSats() > 0); }
