...
char* strtolower(char* str) {
    char* str0 = str;
    for (char c = *str; c != '\0'; c = *++str) *str = tolower(c);
    return (str0);
}
...
char* strtoupper(char* str) {
    char* str0 = str;
    for (char c = *str; c != '\0'; c = *++str) *str = toupper(c);
    return (str0);
}
...
char* strTrimAll(char* str) {
    ...
    for (; *from; from++)
        if (!isspace(*from) || (from[1] && !isspace(from[1]))) *to++ = *from;
    *to = '\0';
    return (str);
}
 ...
 */
bool strHasAlpha(const char* s) { return (strHasCType(s, isalpha)); }

/* return whether the given string contains at least one digit.
 */
bool strHasDigit(const char* s) { return (strHasCType(s, isdigit)); }

/* return whether the given string contains at least one punct
 */
bool strHasPunct(const char* s) { return (strHasCType(s, ispunct)); }

/* return whether the given string contains at least one space
 */
bool strHasSpace(const char* s) { return (strHasCType(s, isspace)); }

/* strncpy that avoids g++ fussing when from is longer than to
 */
void quietStrncpy(char* to, const char* from, int len) { snprintf(to, len, "%.*s", len - 1, from); }

/* qsort-style comparison of two strings in ascending order (a b c d : a will be first in result)
 */
int qsAString(const void* v1, const void* v2) { return (strcmp(*(char**)v1, *(char**)v2)); }

/* qsort-style comparison of two strings in descending order (a b c d : a will be last in result)
 */
int qsDString(const void* v1, const void* v2) { return (strcmp(*(char**)v2, *(char**)v1)); }
...
uint16_t maxStringW(char* str, uint16_t maxw) {
    ...
    while (strl > 0 && (bw = getTextWidth(str)) >= maxw) str[--strl] = '\0';
