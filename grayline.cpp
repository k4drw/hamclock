...
static void drawGLInit(const time_t yr0, SBox& resume_b) {
    ...
    tft.print("Set Times");
    for (uint16_t x = rise_x; x < rise_x + 42; x += 2) tft.fillCircle(x, map_b.y + 42, RISE_R, GL_TC);
    for (uint16_t x = set_x; x < set_x + 34; x += 2) tft.drawPixel(x, map_b.y + 42, GL_TC);
...
static void drawGLData(const time_t yr0, uint16_t x0, uint16_t x1) {
    ...
    if (today_x >= x0 && today_x <= x1) tft.drawLine(today_x, GL_Y0, today_x, GL_Y1, DE_COLOR);
...
void plotGrayline() {
    ...
    UserInput ui = {map_b, UI_UFuncNone, UF_UNUSED, 60000, UF_CLOCKSOK, {0, 0}, TT_NONE, '\0', false, false};
    ...
    while (waitForUser(ui)) {
        ...
        if (ui.tap.x > GL_X0 && ui.tap.x < GL_X1 && ui.tap.y > GL_Y0 && ui.tap.y < GL_Y1) {
            ...
            if (popup.b.x + popup.b.w > GL_X1) popup.b.x = GL_X1 - popup.b.w;
            if (popup.b.y + popup.b.h > GL_Y1) popup.b.y = GL_Y1 - popup.b.h;
