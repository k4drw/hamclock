...
enum { NPKB_NONE, NPKB_LAT, NPKB_LNG, NPKB_GRID, NPKB_DEL, NPKB_CANCEL, NPKB_OK };
...
// field names
typedef enum { NPF_LAT, NPF_LNG, NPF_GRID, NPF_N } NPFieldName;
...
static void addNPChar(const SBox& b, NPField f[NPF_N], NPFieldName focus_fn, int new_char) {
    ...
    if (l < maxlen) {
        ...
        if (focus_fn == NPF_GRID && l >= 4) new_char = tolower(new_char);
...
static int processNPTap(char kbc, NPFieldName focus_fn, const SBox& b, const SCoord& s) {
    ...
    if (kbc != NPKB_NONE) {
        switch (kbc) {
            ...
            case CHAR_DEL:
                return (NPKB_DEL);
            default:
                if (isalnum(kbc)) return (toupper(kbc));
                return (NPKB_NONE);
        }
    }
    ...
    if (k != NPKB_NONE) return (k);

    // last row?
    if (row == 7) return (s.x < b.x + b.w / 3 ? NPKB_DEL : (s.x < b.x + 2 * b.w / 3 ? NPKB_OK : NPKB_CANCEL));
...
bool askNewPos(const SBox& b, LatLong& op_ll, char op_grid[MAID_CHARLEN]) {
    ...
    UserInput ui = {b, UI_UFuncNone, UF_UNUSED, NP_TIMEOUT, UF_NOCLOCKS, {0, 0}, TT_NONE, '\0', false, false};
