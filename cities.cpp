...
static void readCities() {
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        float lat, lng;
        if (sscanf(line, "%f, %f, \"%100[^\"]\"", &lat, &lng, name) != 3) continue;
        ...
        if (n_cities + 1 > n_malloced) {
            ...
            if (!names || !lls) fatalError("alloc cities: %d", n_malloced);
        }
        ...
        if (name_l > max_city_len) max_city_len = name_l;
    ...
    if (!city_malloc && n_cities > 0) fatalError("alloc cities tree: %d", n_cities);
