...
TouchType checkKBWarp(SCoord& s) {
    ...
    if (!want_kbcursor) return (TT_NONE);
        ...
        switch (c) {
            ...
            case CHAR_SPACE:
                // engage
                {
                    ...
                    if (engage_ok && tft.getMouse(&s.x, &s.y)) tt = TT_TAP;
                    if (engage_rate_ok)
                        n_fast_engages = 0;
                    else if (!engage_ok)
...
TouchType readCalTouch(SCoord& s) {
    ...
    if (tt != TT_NONE) Serial.printf("Touch: \t%4d %4d\ttype %d\n", s.x, s.y, (int)tt);
...
void drainTouch() {
    uint16_t tx, ty;
    while (tft.touched()) tft.touchRead(&tx, &ty, NULL);

    bool control, shift;
    while (tft.getChar(&control, &shift) != CHAR_NONE) continue;
}
