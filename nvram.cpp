...
static void initEEPROM() {
    ...
    if (ee_used > ee_size) fatalError("EEPROM too large: %u > %u", ee_used, ee_size);
...
static void nvramWriteBytes(NV_Name e, const uint8_t data[], uint8_t xbytes) {
    ...
    uint8_t e_len = 0;
    uint16_t e_addr = 0;
    if (!nvramStartAddr(e, &e_addr, &e_len)) fatalError("NVBUG! Write: bad id %d", e);
    if (xbytes && e_len != xbytes) fatalError("NVBUG! Write: %d %d != %d bytes", e, e_len, xbytes);
    ...
    for (int i = 0; i < e_len; i++) EEPROM.write(e_addr++, *data++);
    if (!EEPROM.commit()) fatalError("EEPROM.commit failed");
}
...
static bool nvramReadBytes(NV_Name e, uint8_t* buf, uint8_t xbytes) {
    ...
    uint8_t e_len = 0;
    uint16_t e_addr = 0;
    if (!nvramStartAddr(e, &e_addr, &e_len)) fatalError("NVBUG! Read: bad id %d", e);
    if (xbytes && e_len != xbytes) fatalError("NVBUG! Read: %d %d != %d bytes", e, e_len, xbytes);
    ...
    for (int i = 0; i < e_len; i++) *buf++ = EEPROM.read(e_addr++);
    return (true);
}
...
void reportEESize(uint16_t& ee_used, uint16_t& ee_size) {
    ...
    for (unsigned i = 0; i < n; i++) ee_used += 1 + nv_sizes[i];  // +1 for cookie
 ...
 */
void NVWriteFloat(NV_Name e, float f) { nvramWriteBytes(e, (uint8_t*)&f, 4); }

/* write the given uint32_t value to the given NV_name
 */
void NVWriteUInt32(NV_Name e, uint32_t u) { nvramWriteBytes(e, (uint8_t*)&u, 4); }

/* write the given int32_t value to the given NV_name
 */
void NVWriteInt32(NV_Name e, int32_t i) { nvramWriteBytes(e, (uint8_t*)&i, 4); }

/* write the given uint16_t value to the given NV_name
 */
void NVWriteUInt16(NV_Name e, uint16_t u) { nvramWriteBytes(e, (uint8_t*)&u, 2); }

/* write the given int16_t value to the given NV_name
 */
void NVWriteInt16(NV_Name e, int16_t i) { nvramWriteBytes(e, (uint8_t*)&i, 2); }

/* write the given uint8_t value to the given NV_name
 */
void NVWriteUInt8(NV_Name e, uint8_t u) { nvramWriteBytes(e, (uint8_t*)&u, 1); }

/* write the given int8_t value to the given NV_name
 */
void NVWriteInt8(NV_Name e, int8_t i) { nvramWriteBytes(e, (uint8_t*)&i, 1); }

/* write the given string value to the given NV_name
 */
void NVWriteString(NV_Name e, const char* str) { nvramWriteBytes(e, (uint8_t*)str, 0); }
 ...
 */
bool NVReadFloat(NV_Name e, float* fp) { return (nvramReadBytes(e, (uint8_t*)fp, 4)); }

/* read the given NV_Name uint32_t value, return whether found in NVRAM.
 */
bool NVReadUInt32(NV_Name e, uint32_t* up) { return (nvramReadBytes(e, (uint8_t*)up, 4)); }

/* read the given NV_Name int32_t value, return whether found in NVRAM.
 */
bool NVReadInt32(NV_Name e, int32_t* ip) { return (nvramReadBytes(e, (uint8_t*)ip, 4)); }

/* read the given NV_Name uint16_t value, return whether found in NVRAM.
 */
bool NVReadUInt16(NV_Name e, uint16_t* up) { return (nvramReadBytes(e, (uint8_t*)up, 2)); }

/* read the given NV_Name int16_t value, return whether found in NVRAM.
 */
bool NVReadInt16(NV_Name e, int16_t* ip) { return (nvramReadBytes(e, (uint8_t*)ip, 2)); }

/* read the given NV_Name uint8_t value, return whether found in NVRAM.
 */
bool NVReadUInt8(NV_Name e, uint8_t* up) { return (nvramReadBytes(e, (uint8_t*)up, 1)); }

/* read the given NV_Name int8_t value, return whether found in NVRAM.
 */
bool NVReadInt8(NV_Name e, int8_t* ip) { return (nvramReadBytes(e, (uint8_t*)ip, 1)); }

/* read the given NV_Name string value, return whether found in NVRAM.
 */
bool NVReadString(NV_Name e, char* buf) { return (nvramReadBytes(e, (uint8_t*)buf, 0)); }

/* read the given NV_Name TZInfo, return whether found in NVRAM.
 */
bool NVReadTZ(NV_Name e, TZInfo& tzi) {
    int32_t secs;
    if (!NVReadInt32(e, &secs)) return (false);
    if (secs == NVTZ_AUTO) {
        tzi.auto_tz = true;
    } else {
        tzi.auto_tz = false;
...
bool NVReadColorTable(int tbl_AB, uint8_t r[N_CSPR], uint8_t g[N_CSPR], uint8_t b[N_CSPR]) {
    ...
    // check cookie
    if (EEPROM.read(e_addr++) != NV_COOKIE) return (false);
...
void NVWriteColorTable(int tbl_AB, const uint8_t r[N_CSPR], const uint8_t g[N_CSPR], const uint8_t b[N_CSPR]) {
    ...
    if (!EEPROM.commit()) fatalError("EEPROM.commit colors failed");
}
...
void NVReadX11Geom(int& x, int& y, int& w, int& h) {
    ...
    if (NVReadInt16(NV_X11GEOM_X, &geom_x) && NVReadInt16(NV_X11GEOM_Y, &geom_y) &&
        NVReadInt16(NV_X11GEOM_W, &geom_w) && NVReadInt16(NV_X11GEOM_H, &geom_h) && geom_w >= BUILD_W &&
        geom_h >= BUILD_H) {
        x = geom_x;
        y = geom_y;
        w = geom_w;
