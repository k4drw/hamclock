...
static void drawCfgTable(CfgTable& ctbl) {
    ...
    for (int tbl_row = 0; tbl_row < MAX_VIS; tbl_row++) {
        ...
        if (TVIS(ctbl, tbl_row)) {
            ...
            // no TBs
            for (int c = 0; c < N_CIDX; c++) fillSBox(ctbl.tbl_tb[tbl_row][c].box, TB_OFF_CLR);
...
static void setAllCfgTableOff(CfgTable& ctbl) {
    for (int i = 0; i < ctbl.n_cfg; i++)
        for (int j = 0; j < N_CIDX; j++) ctbl.cfg[i].on[j] = false;
    drawCfgTable(ctbl);
}
...
static int getCfgFiles(const char* dir, char*** cfg_names) {
    ...
    if (dirp == NULL) fatalError("%s: %s", dir, strerror(errno));
    ...
    while ((dp = readdir(dirp)) != NULL) {
        ...
        if (nm_list == NULL) fatalError("No memory for %d config files", n_cfgs + 1);
        char cfg_name[MAX_NAMLEN];
        if (file2cfg(dp->d_name, cfg_name, sizeof(cfg_name))) nm_list[n_cfgs++] = strdup(cfg_name);
    }
...
static void engageCfgFile(const char* cfg_name) {
    ...
    if (!from_fp) fatalError("%s: %s", buf, strerror(errno));

    // overwrite existing
    const char* eeprom = EEPROM.getFilename();
    FILE* to_fp = fopen(eeprom, "w");
    if (!to_fp) fatalError("%s: %s", eeprom, strerror(errno));
    ...
    while ((n_r = fread(buf, 1, sizeof(buf), from_fp)) > 0) {
        if (fwrite(buf, 1, n_r, to_fp) != n_r) fatalError("%s: %s\n", eeprom, strerror(errno));
    }
...
static void saveCfgFile(const char* cfg_name) {
    ...
    if (!to_fp) fatalError("%s: %s", buf, strerror(errno));
    ...
    if (!from_fp) fatalError("%s: %s", eeprom, strerror(errno));
...
static void deleteCfgFile(const char* cfg_name) {
    char buf[2000];
    cfg2file(cfg_name, buf, sizeof(buf));
    if (unlink(buf) < 0) fatalError("unlink(%s): %s", buf, strerror(errno));
...
static void renameCfgFile(const char* from, const char* to) {
    ...
    if (rename(f_buf, t_buf) < 0) fatalError("rename(%s,%s): %s", f_buf, t_buf, strerror(errno));
...
static void freeCfgNames(char** cfg_names, int n_cfg) {
    for (int i = 0; i < n_cfg; i++) free((void*)cfg_names[i]);
    free((void*)cfg_names);
}
...
static bool runConfigMenu(char** names, int n_cfgs, bool& restore_def) {
    ...
    if (n_cfgs > 0 && ctbl.cfg == NULL) fatalError("no memory for %d configuration files", ctbl.n_cfg);
    for (int i = 0; i < ctbl.n_cfg; i++) {
    ...
    UserInput ui = {screen_b, UI_UFuncNone, UF_UNUSED, UI_NOTIMEOUT, UF_NOCLOCKS, {0, 0}, TT_NONE, '\0', false, false};
...
#define EDITING_NONE (-2)
    ...
    while (waitForUser(ui)) {
        ...
        if (ctbl.sb.checkTouch(ui.kb_char, ui.tap)) {
            ...
            drawTBControl(ctbl.reset_tb);
            if (ctbl.reset_tb.on) setAllCfgTableOff(ctbl);
            ...
            for (int tbl_row = 0; !found && tbl_row < MAX_VIS && TVIS(ctbl, tbl_row); tbl_row++) {
                ...
                for (int cidx = 0; !found && cidx < N_CIDX; cidx++) {
                    ...
                    if (inBox(ui.tap, tb.box)) {
                        ...
                        if (cidx == REN_CIDX) editing = tb.on ? tbl_row : EDITING_NONE;
    ...
    bool restart = false;
    if (user_ok) {
        if (ctbl.save_tb.on) saveCfgFile(ctbl.save_name.name);  // copy eeprom to save_name
        for (int r = 0; r < n_cfgs; r++) {
            ...
            if (ci.on[DEL_CIDX]) deleteCfgFile(ci.orig_name);  // delete cfg name
            if (ci.on[REN_CIDX] && strcmp(ci.orig_name, ci.edit_name))
                renameCfgFile(ci.orig_name, ci.edit_name);  // rename cfg name
        }
        if (restore_def) restart = true;  // restart to restore def
    }
