...
static void drawIB_WX(const LatLong& ll, uint16_t tx, int dy, int max_ty, uint16_t& ty) {
    ...
    if (ty <= max_ty && getFastWx(ll, wi)) {
        ...
        if (ty <= max_ty) {
            ...
            if (clen > 0) {
                tft.setCursor(tx, ty += dy);
                if (clen > IB_MAXCHARS) clen = IB_MAXCHARS;
...
#define N_MAPMARK NARRAY(marker_colors)
    ...
    if (overMap(mark_s) && (ignore_minfo || mark_s.x > minfo_b.x + minfo_b.w + MAPMARK_R ||
        ...
        if (tft.SCALESZ == 1) {
            for (int i = 0; i < N_MAPMARK; i++) tft.drawCircle(mark_s.x, mark_s.y, MAPMARK_R - i, marker_colors[i]);
        } else {
...
static void drawIB_DX(const SBox& minfo_b, const DXSpot& dx_s, const LatLong& dxc_ll) {
    ...
    // show mode if known
    if (!drawIB_Mode(dx_s.mode, tx + IB_INDENT, IB_LINEDY, ty)) ty += IB_LINEDY;
...
void drawInfoBox() {
    ...
    bool over_spot = over_map && !over_psk && !over_dxped &&
                     (getClosestDXCluster(ll, &dx_s, &dxc_ll) || getClosestOnTheAirSpot(ll, &dx_s, &dxc_ll) ||
                      getClosestADIFSpot(ll, &dx_s, &dxc_ll));
    bool over_pane = over_app && !over_map && !over_dxped &&
                     (getDXCPaneSpot(ms, &dx_s, &dxc_ll) || getMaxDistPSK(ms, &dx_s, &dxc_ll) ||
                      getOnTheAirPaneSpot(ms, &dx_s, &dxc_ll) || getADIFPaneSpot(ms, &dx_s, &dxc_ll));
    bool over_sdo =
        over_app && !over_map && (pp = findPaneChoiceNow(PLOT_CH_SDO)) != PANE_NONE && inBox(ms, plot_b[pp]);
    bool over_moon =
        over_app && !over_map && (pp = findPaneChoiceNow(PLOT_CH_MOON)) != PANE_NONE && inBox(ms, plot_b[pp]);
    ...
    if (draw_info || (map_proj != MAPP_MERCATOR && drew_menu)) fillSBox(minfo_b, RA8875_BLACK);
    drew_menu = draw_info;
