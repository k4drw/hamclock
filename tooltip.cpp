...
void tooltip(const SCoord& s, const char* tip) {
    ...
    for (int i = 0; i < n_lines; i++) {
        ...
        for (ol.count = 0; tip[start + ol.count] != '\0'; ol.count++)
            if (tip[start + ol.count] == ' ' && ol.count > nom_ll) break;
        if (ol.count > max_ll) max_ll = ol.count;
    ...
    // wait
    UserInput ui = {tip_b, UI_UFuncNone, UF_UNUSED, TT_TIMEOUT, UF_NOCLOCKS, {0, 0}, TT_NONE, '\0', false, false};
