...
static inline void packLE4(uint8_t* buf, int& offset, int v) {
    if (debugLevel(DEBUG_BMP, 2)) Serial.printf("BMP: LE4 offset %3d value %d\n", offset, v);
...
static inline void packLE2(uint8_t* buf, int& offset, int v) {
    if (debugLevel(DEBUG_BMP, 2)) Serial.printf("BMP: LE2 offset %3d value %d\n", offset, v);
...
static inline bool read2ByteInt(GenReader& gr, int& byte_os, int& v) {
    char i2[2];
    if (!gr.getChar(&i2[0]) || !gr.getChar(&i2[1])) return (false);
    byte_os += 2;
    v = unpackLE2(i2);
    return (true);
}
...
static inline bool read4ByteInt(GenReader& gr, int& byte_os, int& v) {
    char i4[4];
    if (!gr.getChar(&i4[0]) || !gr.getChar(&i4[1]) || !gr.getChar(&i4[2]) || !gr.getChar(&i4[3])) return (false);
    byte_os += 4;
    v = unpackLE4(i4);
    return (true);
}
...
static inline bool determineBigEndian(void) {
    ...
    e2.e2 = 1;
    bool be = e2.a[1] == 1;
    if (debugLevel(DEBUG_BMP, 2)) Serial.printf("BMP: we are %s-endian\n", be ? "big" : "lil");
    return (be);
}
...
static inline bool next24RGB565(GenReader& gr, uint16_t* pix_p) {
    ...
    if (!gr.getChar(&b) || !gr.getChar(&g) || !gr.getChar(&r)) return (false);
...
static inline bool next16RGB565(GenReader& gr, uint16_t* pix_p) {
    ...
    int p;
    if (!read2ByteInt(gr, byte_os, p)) return (false);
    *pix_p = (uint16_t)p;
    return (true);
}
 ...
 */
static bool read565TB(GenReader& gr, uint16_t* img_565, int img_w, int img_h, int img_bpp, int img_pad, Message& ynot) {
    // time
    struct timeval tv0;
...
#define BITMAPCOREHEADER 12
    ...
    if (file_size != pix_start + img_bpr * abs(img_h)) {
        ...
        Serial.printf("BMP: broken dib_size= %d file_size= %d pix_start= %d w= %d h= %d bpp= %d pad= %d\n", dib_size,
                      file_size, pix_start, img_w, img_h, img_bpp, img_pad);
        return (false);
    }
...
bool readBMPImage(GenReader& gr, const SBox& box, uint16_t*& box_565, ImageRefit fit, Message& ynot) {
    ...
    if (!readBMPHeader(gr, img_w, img_h, img_bpp, img_pad, ynot)) return (false);
    ...
    if (!box_565) fatalError("no mem for %d x %d BMP box", box.w, box.h);
    ...
    if (!img_565) fatalError("no mem for %d x %d BMP image", img_w, img_h);
...
bool createBMP565Header(uint8_t*& hdr, int& hdr_len, int& file_bytes, int img_w, int img_h) {
    ...
    if (!hdr) fatalError("BMP: createBMP565Header failed to allocate %d", hdr_len);
    ...
    // sanity check
    if (offset != hdr_len) fatalError("createBMP565Header bogus header %d != %d", offset, hdr_len);
...
bool writeBMP565File(const char* filename, uint16_t*& pix_565, int img_w, int img_h, Message& ynot) {
    if (debugLevel(DEBUG_BMP, 1)) Serial.printf("BMP: writing %s %d x %d\n", filename, img_w, img_h);
    ...
    if (!ok) ynot.printf("%s write failed: %s", filename, strerror(errno));
