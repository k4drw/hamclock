...
class GenReader {
   public:
    ...
    bool getChar(char* bp) {
        switch (my_type) {
            ...
            case GR_FILE: {
                int i = fgetc(my_fp);
                if (i == EOF || feof(my_fp) || ferror(my_fp)) return (false);
                *bp = (char)i;
                return (true);
            } break;
            case GR_CLIENT:
   ...
   private:
    typedef enum { GR_ARRAY, GR_FILE, GR_CLIENT } GRType;

    GRType my_type;
    FILE* my_fp;
    WiFiClient* my_client;  // pointer to avoid having to init the reference
                            // everywhere with a dummy
    long my_clen;
    const char* my_array;
...
class StackMalloc {
   public:
    ...
    size_t getSize(void) { return (siz); }

    void* getMem(void) { return (mem); }
...
class Message {
   public:
    ...
    const char* get(void) { return (msg); }
...
typedef enum { AUXTIMES AUXT_N } AuxTimeFormat;
#undef X
...
typedef enum {
    ...
    NV_DE_TIMEFMT,     // DE: 0=info; 1=analog; 2=cal; 3=analog+day; 4=dig 12hr; 5=dig
                       // 24hr
    ...
    NV_GRIDSTYLE,   // map grid style 0=off; 1=tropics; 2=lat-lng; 3=maindenhead,
                    // 4=radial
    ...
    // 70
    NV_BCFLAGS,         // Big Clock bitmask:
                        // 1=date;2=wx;4=dig;8=12hr;16=nosec;32=UTC;64=an+dig;128=hrs;256=SpWx;512=hands;1024=sat
    ...
    NV_PSK_MODEBITS,   // live spots mode: bit 0: on=psk off=wspr bit 1: on=bycall
                       // off=bygrid
...
// N.B. names should not include blanks, but _ are changed to blanks for
// prettier printing
#define PLOTNAMES                      \
...
typedef enum { PLOTNAMES PLOT_CH_N } PlotChoice;
#undef X
 ...
 *
 * only thing I don't like about this is that the need to set the unit vector
 * prevents using const LatLong when the intent is to promise not to change
 * location.
 */
class LatLong {
   public:
    ...
    /* direct access -- too late for setters/getters -- that's why we need
     * lat/lng_set
     */
    float lat, lat_d;  // rads and degrees N
    float lng, lng_d;  // rads and degrees E

    /* great circle distance between us and ll in rads; mult by ERAD_M to get
     * miles
     */
...
typedef struct {
    ...
    char onair[NV_ONAIR_LEN];    // real on-air message for CT_ONAIR, used by
                                 // setup.cpp
...
typedef enum { MAPPROJS MAPP_N } MapProjection;
#undef X
...
typedef struct {
    ...
    // ontheair:       spotted is TX, repurpose rx_call for id and rx_grid for
    // program name psk live spots: if PSKMB_OFDE then DE is TX, else DE is RX
...
typedef struct {
    time_t u[N_BME_READINGS];  // circular queue of UNIX sensor read times, 0 if no
                               // data
...
typedef enum { SUPPORTED_BANDS HAMBAND_N } HamBandSetting;
#undef X
...
typedef enum { LOMD_ALL, LOMD_JUSTDOT } LabelOnMapDot;
...
typedef struct {
    ...
    int16_t pan_x,
        pan_y;  // offset from original position, unzoomed pixels, + right/up
} PanZoom;
...
extern bool getGimbalState(bool& connected, bool& vis_now, bool& has_el, bool& is_stop, bool& is_auto, float& az,
                           float& el);
...
extern void nearestKD3Node(const KD3Node* root, const KD3Node* nd, int level, const KD3Node** best, float* best_dist,
                           int* n_visited);
...
typedef enum { COREMAPS CM_N } CoreMaps;
#undef X

#define CM_NONE CM_N  // handy alias meaning none active

// macro to test whether the given core map style is just a file (not an active
// query)
#define CM_ISFILE(cm)                                                                                       \
    ((cm) == CM_COUNTRIES || (cm) == CM_TERRAIN || (cm) == CM_DRAP || (cm) == CM_AURORA || (cm) == CM_WX || \
     (cm) == CM_MUF_RT || (cm) == CM_CLOUDS || (cm) == CM_USER)
...
typedef struct _menu_text {
    ...
    char* label;     // mutable "label" memory replaces const MenuItem label, must
                     // include EOS
    ...
    bool (*text_fp)(struct _menu_text*,
                    Message& ynot);        // call to check text, unless NULL
...
typedef enum {
    ...
    MENU_TEXT,    // must supply MenuText* -- N.B. at most one and must be last in
                  // items[]
...
extern void drawNCDXFStats(uint16_t color, const char titles[NCDXF_B_NFIELDS][NCDXF_B_MAXLEN],
                           const char values[NCDXF_B_NFIELDS][NCDXF_B_MAXLEN], const uint16_t colors[NCDXF_B_NFIELDS]);
...
extern void NVWriteColorTable(int tbl_AB, const uint8_t r[N_CSPR], const uint8_t g[N_CSPR], const uint8_t b[N_CSPR]);
...
extern bool plotXY(const SBox& box, float x[], float y[], int nxy, const char* xlabel, const char* ylabel,
                   uint16_t color, float y_min, float y_max, float big_value);
extern bool plotXYstr(const SBox& box, float x[], float y[], int nxy, const char* xlabel, const char* ylabel,
...
#define PANE_0_CH_MASK                                                                                \
    ((1 << PLOT_CH_DXCLUSTER) | (1 << PLOT_CH_CONTESTS) | (1 << PLOT_CH_ADIF) | (1 << PLOT_CH_ONTA) | \
     (1 << PLOT_CH_DXPEDS))
...
#define NBITS_SET(v)                                                                                                   \
    ((((((((v) - (((v) >> 1) & 0x55555555)) & 0x33333333) + ((((v) - (((v) >> 1) & 0x55555555)) >> 2) & 0x33333333)) + \
...
typedef enum { QRZTABLE QRZ_N } QRZURLId;
#undef X
...
class ScrollState {
   public:
    ...
    int top_vis;  // index into the data array being dislayed at the front of the
                  // list
...
typedef enum { BOLD_FONT, LIGHT_FONT } FontWeight;

typedef enum { FAST_FONT, SMALL_FONT, LARGE_FONT } FontSize;
...
typedef enum { LABELSTYLES LBL_N } LabelType;
#undef X

typedef enum { DF_MDY, DF_DMY, DF_YMD, DF_N } DateFormat;
...
typedef enum { _WATCH_DEFN WLA_N } WatchListState;
#undef X
...
/* consolidated space weather enum and stats. #define X to extract desired
 * components. N.B. max name chars NCDXF_B_MAXLEN-1 N.B. set rank of four [0,3]
 * then set the rest to 9.
 */
#define SPCWX_DATA                                                                                      \
...
typedef enum { SPCWX_DATA SPCWX_N } SPCWX_t;
#undef X
...
extern bool getClosestSpot(DXSpot* list, int n_list, SpotFilter sfp, LabelOnMapEnd which_ends, LatLong& ll, DXSpot* sp,
                           LatLong* llp);
...
typedef int (*PQSF)(const void*,
                    const void*);  // pointer to qsort-style compare function
...
// alarm state
typedef enum { ALMS_OFF, ALMS_ARMED, ALMS_RINGING } AlarmState;
...
typedef struct {
    ...
    bool found[MAX_WEBARGS];         // whether this name was found in the original GET
                                     // command
    int nargs;
} WebArgs;
 ...
 * N.B. put TEMP first because 1) most likely used and 2) short enough for DE/DX
 * prefix (see drawNCDXFBoxWx()) N.B. max name chars NCDXF_B_MAXLEN-1
 */
#define WXSTATS             \
    X(WXS_TEMP, "Temp")     \
    X(WXS_HUM, "Humidity")  \
...
typedef enum { WXSTATS WXS_N } WeatherStats;
#undef X
...
typedef enum { ZONE_CQ, ZONE_ITU } ZoneID;
