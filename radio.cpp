...
static void postMsgToMain(const char* fmt, ...) {
    ...
    if (debugLevel(DEBUG_RIG, 1)) Serial.printf("RADIO: posting %s\n", buf);

    // inform main thread of new malloced message, discard are message not yet picked up.
    if (pthread_mutex_lock(&msg_lock) == 0) {
        if (thread_msg) free(thread_msg);
        thread_msg = strdup(buf);
        if (!thread_msg) fatalError("No memory for radio message: %s", buf);
...
static bool sendHamlibCmd(WiFiClient& client, const char cmd[]) {
    // send
    if (debugLevel(DEBUG_RIG, 2)) Serial.printf("RADIO: HAMLIB: send: %s\n", cmd);
    client.println(cmd);
    ...
    do {
        ...
        if (ok) {
            if (debugLevel(DEBUG_RIG, 2)) Serial.printf("RADIO: HAMLIB reply: %s\n", buf);
        } else {
...
static bool setHamlibFreq(WiFiClient& client, int Hz) {
    ...
    static const char* setup_cmds[] = {
        // without --vfo
        "+\\set_split_vfo 0 VFOA", "+\\set_vfo VFOA", "+\\set_func RIT 0", "+\\set_rit 0",
        "+\\set_func XIT 0",       "+\\set_xit 0",
    };
    static const char* setup_cmds_vfo[] = {
        // with --vfo
        "+\\set_split_vfo currVFO 0 VFOA", "+\\set_vfo VFOA",      "+\\set_func currVFO RIT 0", "+\\set_rit currVFO 0",
        "+\\set_func currVFO XIT 0",       "+\\set_xit currVFO 0",
    };

    bool ok = true;
    if (hamlib_vfo)
        for (int i = 0; ok && i < NARRAY(setup_cmds_vfo); i++) ok = sendHamlibCmd(client, setup_cmds_vfo[i]);
    else
        for (int i = 0; ok && i < NARRAY(setup_cmds); i++) ok = sendHamlibCmd(client, setup_cmds[i]);
    ...
    if (ok) {
        ...
        if (!ok) Serial.printf("RADIO: HAMLIB set %d Hz but acked %d\n", Hz, reply);
    }

    if (ok && debugLevel(DEBUG_RIG, 1)) Serial.printf("RADIO: HAMLIB: set %d Hz\n", Hz);
...
static bool tryHamlibConnect(WiFiClient& client) {
    ...
    int port;
    if (!getRigctld(host, &port)) fatalError("bug! tryHamlibConnect no getRigctld()");

    // connect, bale if can't
    if (!client.connect(host, port)) return (false);

    if (debugLevel(DEBUG_RIG, 1)) Serial.printf("RADIO: HAMLIB: %s:%d connect ok\n", host, port);
...
static bool sendFlrigCmd(WiFiClient& client, const char cmd[], const char value[], const char type[]) {
    ...
    if (debugLevel(DEBUG_RIG, 2)) Serial.printf("RADIO: FLRIG: reply:\n");
    char reply_buf[200];
    bool ok = false;
    do {
        ok = getTCPLine(client, reply_buf, sizeof(reply_buf), NULL);
        if (ok) {
            if (debugLevel(DEBUG_RIG, 2)) printf("%s\n", reply_buf);
        } else {
 ...
 */
static bool intFlrigCmd(WiFiClient& client, const char cmd[], const char value[], const char type[], int& reply) {
    // send core command
    coreFlrigCmd(client, cmd, value, type);
    ...
    do {
        ...
        if (ok) {
            if (debugLevel(DEBUG_RIG, 2)) printf("%s\n", reply_buf);
            if (sscanf(reply_buf, " <value><i4>%d", &reply) == 1 || sscanf(reply_buf, " <value>%d", &reply) == 1) {
                if (reply < 0) Serial.printf("RADIO: FLRIG: cmd %s reply %d\n", cmd, reply);
            }
        } else {
            Serial.printf("RADIO: FLRIG cmd %s: no reply\n", cmd);
        }
...
static bool setFlrigFreq(WiFiClient& client, int Hz) {
    ...
    return (sendFlrigCmd(client, "rig.set_split", "0", "int") &&
            ...
            intFlrigCmd(client, "rig.get_vfoA", "0", "int", reply) && reply == Hz);
}
...
static bool tryFlrigConnect(WiFiClient& client) {
    ...
    int port;
    if (!getFlrig(host, &port)) fatalError("tryFlrigConnect no control");

    // connect, bale if can't
    if (!client.connect(host, port)) return (false);

    if (debugLevel(DEBUG_RIG, 1)) Serial.printf("RADIO: FLRIG: %s:%d connect ok\n", host, port);
...
static uint32_t KX3sendOneString(float correction, const char str[]) {
    ...
    if (!hipri_ok) Serial.printf("RADIO: KX3: sched_setscheduler(%d): %s\n", hi_param.sched_priority, strerror(errno));
    ...
    if (hipri_ok) sched_setscheduler(0, orig_sched, &orig_param);
 ...
 */
static void KX3radioResetIO() { mcp.pinMode(MCP_FAKE_KX3, INPUT); }
        ...
        if (getRigctld(NULL, NULL)) {
            ...
            if (!hl_client.connected()) {
                if (tryHamlibConnect(hl_client))
                    ...
                    if (timesUp(&hlwarn_ms, WARN_MS)) postMsgToMain("HAMLIB no connection");
            ...
            if (timesUp(&hlpoll_ms, RADIOPOLL_MS) && hl_client.connected()) {
                ...
                if (hamlib_vfo) {
                    ...
                    if (intHamlibCmd(hl_client, "+\\get_ptt", "PTT:", reply) && reply >= 0) thread_onair = reply;
                }
        ...
        if (getFlrig(NULL, NULL)) {
            ...
            if (!fl_client.connected()) {
                if (tryFlrigConnect(fl_client))
                    ...
                    if (timesUp(&flwarn_ms, WARN_MS)) postMsgToMain("FLRIG no connection");
                    usleep(RETRY_MS * 1000);
                }
            }
...
static bool startRadioThread(void) {
    if (!getFlrig(NULL, NULL) && !getRigctld(NULL, NULL)) return (false);
    ...
    if (!thread_running) {
        ...
        if (e) fatalError("radioThread failed: %s", strerror(e));
        thread_running = true;
    }
...
void setRadioSpot(float kHz) {
    ...
    if (setRadio()) setKX3Spot(kHz);
#endif  // _SUPPORT_KX3

    // insure thread running or bale if not needed
    if (!startRadioThread()) return;

    // inform thread if desired
    if (setRadio()) set_fl_Hz = set_hl_Hz = (int)(kHz * 1000);
}
