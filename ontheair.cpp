...
static void parseONTAOrgs(void) {
    ...
    if (next_ontaorg >= onta_norgs) next_ontaorg = 0;
...
static bool isSpotOrgOk(const DXSpot& s) {
    ...
    // all if no orgs
    if (onta_norgs == 0) return (true);

    // just next_ontaorg unless merging
    if (!onta_merge) return (strcasecmp(onta_orgs[next_ontaorg], s.rx_grid) == 0);
...
static void loadONTASettings(void) {
    ...
    if (getQRZId() != QRZ_NONE) {
        if (!NVReadUInt8(NV_ONTABIO, &bio)) bio = 0;
    }
    onta_showbio = (bio != 0);
}
...
static void formatONTASpot(const DXSpot& spot, const SBox& box, char* line, size_t l_len, int& freq_len) {
    ...
    if (BOX_IS_PANE_0(box)) {
        ...
        if (spot.kHz > 99999) {
            size_t ns = snprintf(line, l_len, "%4.0fM%*.*s %*.*s", spot.kHz * 1e-3, call_len, call_len, spot.tx_call,
                                 id_len, id_len, id);
            formatAge(age, line + ns, l_len - ns, 1);
        } else {
            size_t ns = snprintf(line, l_len, "%*.0f%*.*s %*.*s", freq_len, spot.kHz, call_len, call_len, spot.tx_call,
                                 id_len, id_len, id);
            formatAge(age, line + ns, l_len - ns, 1);
        }
...
static void drawONTAVisSpots(const SBox& box) {
    ...
    if (onta_ss.getVisDataIndices(min_i, max_i) > 0) {
        for (int i = 0; i < onta_ss.n_data; i++) {
            ...
            if (i < min_i) {
                if (!any_older) any_older = checkWatchListSpot(WLID_ONTA, spot) == WLS_HILITE;
            } else if (i > max_i) {
                if (!any_newer) any_newer = checkWatchListSpot(WLID_ONTA, spot) == WLS_HILITE;
            } else {
                // build info line
                char line[50];
                int freq_len;
    ...
    if (onta_ss.okToScrollDown() && ((scrollTopToBottom() && any_older) || (!scrollTopToBottom() && any_newer)))
        dw_color = RA8875_RED;
    if (onta_ss.okToScrollUp() && ((scrollTopToBottom() && any_newer) || (!scrollTopToBottom() && any_older)))
        up_color = RA8875_RED;
...
static void rebuildONTAWatchList(void) {
    ...
    for (int i = 0; i < n_ontaspots; i++) {
        ...
        if (spot.spotted < oldest)
            ...
            if (!ontawl_spots) fatalError("No mem for %d watch list spots", onta_ss.n_data + 1);
            ontawl_spots[onta_ss.n_data++] = spot;
        }
    }

    Serial.printf("ONTA: %d total - %d too-old - %d not-org - %d not-WL = %d showing\n", n_ontaspots, n_old, n_no_org,
                  n_no_wl, onta_ss.n_data);
...
static void runONTASortMenu(const SBox& box) {
    ...
    for (int i = 0; i < N_ONTAAGES; i++) snprintf(onta_ages_str[i], sizeof(onta_ages_str[i]), "%d m", onta_ages[i]);
...
static bool retrieveONTA(void) {
    ...
    if (fp) {
        ...
        while (fgets(line, sizeof(line), fp)) {
            ...
            if (!onta_spots) fatalError("No room for %d ONTA spots", n_ontaspots + 1);
            ...
            if (sscanf(line, "%19[^,],%lu,%lu,%19[^,],%19[^,],%f,%f,%19[^,],%19s", dxcall, &hz, &unx, mode, dxgrid,
                       &lat_d, &lng_d, id, prog) != 9) {
                // maybe a blank mode?
                if (sscanf(line, "%19[^,],%lu,%lu,,%19[^,],%f,%f,%19[^,],%19s", dxcall, &hz, &unx, dxgrid, &lat_d,
                           &lng_d, id, prog) != 8) {
                    // .. nope, something else
...
bool checkOnTheAirTouch(const SCoord& s, const SBox& box) {
    ...
    if (s.y < box.y + PANETITLE_H) {
        ...
        // on hold?
        if (ROTHOLD_TST(PLOT_CH_ONTA)) return (true);
    ...
    if (onta_ss.findDataIndex(vis_row, spot_row)) engageONTARow(ontawl_spots[spot_row]);
...
bool getOnTheAirSpots(DXSpot** spp, uint8_t* nspotsp) {
    ...
    if (!onta_spots || findPaneForChoice(PLOT_CH_ONTA) == PANE_NONE) return (false);
 ...
 */
bool isONTARotating(void) { return (onta_norgs > 1 && !onta_merge && onta_ss.atNewest()); }
