...
static void drawMenuText(const MenuItem& mi, const SBox& pb, bool draw_label, bool draw_cursor) {
    // require
    if (mi.type != MENU_TEXT) fatalError("drawMenuText not %d: %d", (int)MENU_TEXT, (int)mi.type);
    ...
    if (draw_label) {
        ...
        if (debugLevel(DEBUG_MENUS, 1)) tft.drawRect(MENU_LX(pb), pb.y, MENU_LW(mi), pb.h, RA8875_GREEN);
    ...
    // check left end
    if (tfp->c_pos < tfp->w_pos) tfp->w_pos = tfp->c_pos;

    // check right end: insure cursor is both within the string and within the text window
    int t_n = MENU_TW(pb, mi) / MENU_FW;
    if (tfp->c_pos > t_len) tfp->c_pos = t_len;
    if (tfp->c_pos > tfp->w_pos + t_n - 1) tfp->w_pos = tfp->c_pos - t_n + 1;
...
static void menuDrawItem(const MenuItem& mi, const SBox& pb, bool draw_label, bool kb_focus) {
    ...
    if (debugLevel(DEBUG_MENUS, 1)) drawSBox(pb, RA8875_RED);

    // draw now if menu is over map_b
    if (!draw_label && boxesOverlap(pb, map_b)) tft.drawPR();
}
...
static int menuCountItemsSet(MenuInfo& menu, int ii) {
    ...
    for (int i = 0; i < menu.n_items; i++) {
        if (menu.items[i].type == MENU_IGNORE) continue;
        if (menu.items[i].type != menu_ii.type) continue;
        if (menu.items[i].group != menu_ii.group) continue;
        if (menu.items[i].set) n_set++;
    }
...
static int textFieldEdit(MenuInfo& menu, SBox* pick_boxes, int textf_i, const char kb_char) {
    ...
    if (mi.type != MENU_TEXT) fatalError("textFieldEdit called with type %d", (int)mi.type);
    ...
    switch (kb_char) {
        ...
        case CHAR_LEFT:
            if (debugLevel(DEBUG_MENUS, 2)) printf(" LEFT ");
            if (tfp->c_pos > 0) {
                tfp->c_pos -= 1;
        ...
        case CHAR_RIGHT:
            if (debugLevel(DEBUG_MENUS, 2)) printf(" RIGT ");
            tfp->c_pos += 1;
            menuDrawItem(mi, pb, false, true);
            break;
        ...
        case CHAR_BS:
        case CHAR_DEL:
            if (debugLevel(DEBUG_MENUS, 2)) printf(" DEL  ");
        ...
        default:
            ...
            if (isprint(kb_char)) {
                if (t_len < tfp->t_mem - 1) {
                    if (debugLevel(DEBUG_MENUS, 2)) printf(" IN %c ", kb_char);
                    memmove(&tfp->text[tfp->c_pos + 1], &tfp->text[tfp->c_pos], t_len - tfp->c_pos + 1);  // w/EOS
                    tfp->text[tfp->c_pos] = tfp->to_upper ? toupper(kb_char) : kb_char;
                    tfp->c_pos += 1;
                    menuDrawItem(mi, pb, false, true);
                } else {
                    if (debugLevel(DEBUG_MENUS, 2)) printf(" FULL ");
    ...
    if (t_len > tfp->t_mem - 1) fatalError("textFieldEdit t_len %d > t_mem-1 %d", (int)t_len, (int)tfp->t_mem - 1);

    if (debugLevel(DEBUG_MENUS, 2))
        printf("  %2d %2d %2d='%-*s'\n", tfp->c_pos, tfp->w_pos, (int)strlen(tfp->text), (int)tfp->t_mem, tfp->text);
...
bool runMenu(MenuInfo& menu) {
    ...
    for (int i = 0; i < menu.n_items; i++) {
        MenuItem& mi = menu.items[i];
        if (mi.type == MENU_IGNORE) continue;
        if (mi.type == MENU_TEXT) {
            // enforce being at the bottom -- width is always full width
            if (i < menu.n_items - 1) {
                MenuFieldType next_mft = menu.items[i + 1].type;
                if (next_mft != MENU_TEXT && next_mft != MENU_IGNORE) fatalError("all MENU_TEXT must be last");
            }
            n_mt++;
        } else {
    ...
    if (menu.menu_b.w < widest * menu.n_cols + MENU_RM) menu.menu_b.w = widest * menu.n_cols + MENU_RM;

    // insure menu width accommodates ok and/or cancel buttons
    if (menu.cancel == M_NOCANCEL) {
        if (menu.menu_b.w < MENU_BB + menu.ok_b.w + MENU_BB) menu.menu_b.w = MENU_BB + menu.ok_b.w + MENU_BB;
    } else {
    ...
    if (menu.menu_b.x + menu.menu_b.w >= tft.width()) menu.menu_b.x = tft.width() - menu.menu_b.w - 1;
    if (menu.menu_b.y + menu.menu_b.h >= tft.height()) menu.menu_b.y = tft.height() - menu.menu_b.h - 1;
    ...
    if (boxesOverlap(menu.menu_b, map_b)) tft.drawPR();
    ...
    UserInput ui = {menu.menu_b, UI_UFuncNone, UF_UNUSED, MENU_TIMEOUT, menu.update_clocks,
    ...
    for (int i = 0; i < menu.n_items; i++) {
        MenuItem& mi = menu.items[i];
        if (MENU_ACTIVE(mi.type)) Serial.printf("  %-15s g%d s%d\n", mi.label ? mi.label : "", mi.group, mi.set);
    }
...
void menuRedrawOk(SBox& ok_b, MenuOkState oks) {
    ...
    if (boxesOverlap(ok_b, map_b)) tft.drawPR();
}
...
bool waitForUser(UserInput& ui) {
    ...
    for (;;) {
        if ((ui.tt = readCalTouchWS(ui.tap)) != TT_NONE) return (true);

        ui.kb_char = tft.getChar(&ui.kb_ctrl, &ui.kb_shift);
        if (ui.kb_char != CHAR_NONE) return (true);

        if (ui.to_ms != UI_NOTIMEOUT && timesUp(&t0, ui.to_ms)) return (false);
        ...
        if (ui.update_clocks == UF_CLOCKSOK) updateClocks(false);

        wdDelay(10);

        // refresh protected region in case X11 window is moved
        if (boxesOverlap(ui.inbox, map_b)) tft.drawPR();
    }
}
