 ...
 */
bool getClosestSpot(DXSpot* list, int n_list, SpotFilter sfp, LabelOnMapEnd which_end, LatLong& from_ll,
...
static void drawSpotTXRXOnMap(DXSpot& spot, LabelOnMapEnd txrx, LabelOnMapDot dot) {
    ...
    if (lblt == LBL_DOT || just_dot) return;
...
void drawSpotLabelOnMap(DXSpot& spot, LabelOnMapEnd txrx, LabelOnMapDot dot) {
    if (txrx == LOME_TXEND || txrx == LOME_BOTH) drawSpotTXRXOnMap(spot, LOME_TXEND, dot);
    if (txrx == LOME_RXEND || txrx == LOME_BOTH) drawSpotTXRXOnMap(spot, LOME_RXEND, dot);
}
    ...
    for (int i = 0; i <= n_step; i++) {  // fence posts
        ...
        if (prev_s.x > 0) {
            if (segmentSpanOkRaw(prev_s, s, raw_pw)) {
                if (!dashed || n_step < 7 || (i & 1)) tft.drawLineRaw(prev_s.x, prev_s.y, s.x, s.y, raw_pw, color);
            } else
                s.x = 0;
        }
        prev_s = s;
...
void drawSpotOnList(const SBox& box, const DXSpot& spot, int row, uint16_t bg_col) {
    if (debugLevel(DEBUG_SCROLL, 1)) Serial.printf("row %2d: drawing\n", row);
...
void drawVisibleSpots(WatchListId wl_id, const DXSpot* spots, const ScrollState& ss, const SBox& box,
    ...
    if (ss.getVisDataIndices(min_i, max_i) > 0) {
        for (int i = 0; i < ss.n_data; i++) {
            ...
            if (i < min_i) {
                if (!any_older) any_older = checkWatchListSpot(wl_id, spot) == WLS_HILITE;
            } else if (i > max_i) {
                if (!any_newer) any_newer = checkWatchListSpot(wl_id, spot) == WLS_HILITE;
            } else {
    ...
    for (int row = ss.getDisplayRow(--min_i); row >= 0 && row < ss.max_vis; row = ss.getDisplayRow(--min_i)) {
        if (debugLevel(DEBUG_SCROLL, 1)) Serial.printf("row %2d: erasing\n", row);
    ...
    if (ss.okToScrollDown() && ((scrollTopToBottom() && any_older) || (!scrollTopToBottom() && any_newer)))
        dw_color = RA8875_RED;
    if (ss.okToScrollUp() && ((scrollTopToBottom() && any_newer) || (!scrollTopToBottom() && any_older)))
        up_color = RA8875_RED;
