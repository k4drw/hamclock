...
typedef enum { NTP_STATES NTPSC_N } NTPStateCode;
#undef X

// NTP state names
#define X(a, b) b,  // expands NTP_STATES to each name plus comma
static const char* ntp_sn[NTPSC_N] = {NTP_STATES};
#undef X
...
typedef enum { UNITS_CHOICES UNITS_N } UnitsCode;
#undef X

#define X(a, b) b,  // expands UNITS_CHOICES to each name plus comma
static const char* units_names[UNITS_N] = {UNITS_CHOICES};
#undef X
...
// max tle ages
static const char* maxTLEAges[] = {"3 days", "7 days", "14 days", "21 days"};
#define N_MAXTLE NARRAY(maxTLEAges)
#define MAXTLE_DEF 1  // index of default

// min label distances
static const char* minLblD[] = {"None", "1000", "2000", "4000"};
...
static StringPrompt string_pr[N_SPR] = {
    ...
    {5,
     {CSEL_VX, CSEL_VYR, 0, PR_H},
     {CSEL_VX, CSEL_VYR, 80, PR_H},
     NULL,
     NULL,
     0,
     0,
    ...
    {5,
     {CSEL_VX, CSEL_VYG, 0, PR_H},
     {CSEL_VX, CSEL_VYG, 80, PR_H},
     NULL,
     NULL,
     0,
     0,
    ...
    {5,
     {CSEL_VX, CSEL_VYB, 0, PR_H},
     {CSEL_VX, CSEL_VYB, 80, PR_H},
     NULL,
     NULL,
     0,
     0,
...
static const char* wla_name[WLA_N] = {_WATCH_DEFN};
#undef X
...
static BoolPrompt bool_pr[N_BPR] = {
    ...
    {4,
     ...
     false,
     NULL,
     units_names[UNITS_MET],
     units_names[UNITS_BRIT],
     UNITSA_BPR,
     0},
    // 3x entangled: FX -> TF -> TT ...
    ...
    {4, {400, R2Y(5), 190, PR_H}, {590, R2Y(5), 170, PR_H}, false, NULL, minLblD[2], minLblD[3], LBLDISTA_BPR, 0},
    // 4x entangled: FF -> TF -> FT -> TT -> ...
    ...
    {4,
     ...
     false,
     "Look up bio?",
     qrz_urltable[QRZ_NONE].label,
     qrz_urltable[QRZ_QRZ].label,
     QRZBIOB_BPR,
    ...
    {4,
     ...
     false,
     NULL,
     qrz_urltable[QRZ_HAMCALL].label,
     qrz_urltable[QRZ_CQQRZ].label,
     QRZBIOA_BPR,
     0},
    // 4x entangled: FF -> TF -> FT -> TT -> ...
    ...
    {4,
     ...
     false,
     "Auto upgrade?",
     autoup_tbl[AUP_OFF].label,
     autoup_tbl[AUP_P1].label,
     AUTOUPB_BPR,
#if defined(NO_UPGRADE)
    ...
    {4,
     ...
     false,
     NULL,
     autoup_tbl[AUP_P2].label,
     autoup_tbl[AUP_P3].label,
     AUTOUPA_BPR,
     0},
    // 4x entangled: FF -> TF -> FT -> TT -> ...
...
static ColSelPrompt csel_pr[N_CSPR] = {
    {{CSEL_COL1X + CSEL_PDX, R2Y(1), CSEL_PW, PR_H},
     ...
     NV_SHORTPATHCOLOR,
     "Short path",
     {CSEL_COL1X + CSEL_ADX, R2Y(1) + CSEL_TBDY, CSEL_TBSZ, CSEL_TBSZ},
     false,
     0,
     0,
     0},
    ...
    {{CSEL_COL1X + CSEL_PDX, R2Y(2), CSEL_PW, PR_H},
     ...
     NV_LONGPATHCOLOR,
     "Long path",
     {CSEL_COL1X + CSEL_ADX, R2Y(2) + CSEL_TBDY, CSEL_TBSZ, CSEL_TBSZ},
     false,
     0,
     0,
     0},
    ...
    {{CSEL_COL1X + CSEL_PDX, R2Y(3), CSEL_PW, PR_H},
     ...
     NV_SAT1COLOR,
     "Satellite 1",
     {CSEL_COL1X + CSEL_ADX, R2Y(3) + CSEL_TBDY, CSEL_TBSZ, CSEL_TBSZ},
     false,
     0,
     0,
     0},
    ...
    {{CSEL_COL1X + CSEL_PDX, R2Y(4), CSEL_PW, PR_H},
     ...
     NV_SAT2COLOR,
     "Satellite 2",
     {CSEL_COL1X + CSEL_ADX, R2Y(4) + CSEL_TBDY, CSEL_TBSZ, CSEL_TBSZ},
     false,
     0,
     0,
     0},
    ...
    {{CSEL_COL1X + CSEL_PDX, R2Y(7), CSEL_PW, PR_H},
     ...
     NV_160M_COLOR,
     "160 m",
     {CSEL_COL1X + CSEL_ADX, R2Y(7) + CSEL_TBDY, CSEL_TBSZ, CSEL_TBSZ},
     false,
     0,
     0,
     0},
    ...
    {{CSEL_COL1X + CSEL_PDX, R2Y(8), CSEL_PW, PR_H},
     ...
     NV_80M_COLOR,
     "80 m",
     {CSEL_COL1X + CSEL_ADX, R2Y(8) + CSEL_TBDY, CSEL_TBSZ, CSEL_TBSZ},
     false,
     0,
     0,
     0},
    ...
    {{CSEL_COL1X + CSEL_PDX, R2Y(9), CSEL_PW, PR_H},
     ...
     NV_60M_COLOR,
     "60 m",
     {CSEL_COL1X + CSEL_ADX, R2Y(9) + CSEL_TBDY, CSEL_TBSZ, CSEL_TBSZ},
     false,
     0,
     0,
     0},
    ...
    {{CSEL_COL1X + CSEL_PDX, R2Y(10), CSEL_PW, PR_H},
     ...
     NV_40M_COLOR,
     "40 m",
     {CSEL_COL1X + CSEL_ADX, R2Y(10) + CSEL_TBDY, CSEL_TBSZ, CSEL_TBSZ},
     false,
     0,
     0,
     0},
    ...
    {{CSEL_COL1X + CSEL_PDX, R2Y(11), CSEL_PW, PR_H},
     ...
     NV_30M_COLOR,
     "30 m",
     {CSEL_COL1X + CSEL_ADX, R2Y(11) + CSEL_TBDY, CSEL_TBSZ, CSEL_TBSZ},
     false,
     0,
     0,
     0},
    ...
    {{CSEL_COL1X + CSEL_PDX, R2Y(12), CSEL_PW, PR_H},
     ...
     NV_20M_COLOR,
     "20 m",
     {CSEL_COL1X + CSEL_ADX, R2Y(12) + CSEL_TBDY, CSEL_TBSZ, CSEL_TBSZ},
     false,
     0,
     0,
     0},
    ...
    {{CSEL_COL2X + CSEL_PDX, R2Y(7), CSEL_PW, PR_H},
     ...
     NV_17M_COLOR,
     "17 m",
     {CSEL_COL2X + CSEL_ADX, R2Y(7) + CSEL_TBDY, CSEL_TBSZ, CSEL_TBSZ},
     false,
     0,
     0,
     0},
    ...
    {{CSEL_COL2X + CSEL_PDX, R2Y(8), CSEL_PW, PR_H},
     ...
     NV_15M_COLOR,
     "15 m",
     {CSEL_COL2X + CSEL_ADX, R2Y(8) + CSEL_TBDY, CSEL_TBSZ, CSEL_TBSZ},
     false,
     0,
     0,
     0},
    ...
    {{CSEL_COL2X + CSEL_PDX, R2Y(9), CSEL_PW, PR_H},
     ...
     NV_12M_COLOR,
     "12 m",
     {CSEL_COL2X + CSEL_ADX, R2Y(9) + CSEL_TBDY, CSEL_TBSZ, CSEL_TBSZ},
     false,
     0,
     0,
     0},
    ...
    {{CSEL_COL2X + CSEL_PDX, R2Y(10), CSEL_PW, PR_H},
     ...
     NV_10M_COLOR,
     "10 m",
     {CSEL_COL2X + CSEL_ADX, R2Y(10) + CSEL_TBDY, CSEL_TBSZ, CSEL_TBSZ},
     false,
     0,
     0,
     0},
    ...
    {{CSEL_COL2X + CSEL_PDX, R2Y(11), CSEL_PW, PR_H},
     ...
     NV_6M_COLOR,
     "6 m",
     {CSEL_COL2X + CSEL_ADX, R2Y(11) + CSEL_TBDY, CSEL_TBSZ, CSEL_TBSZ},
     false,
     0,
     0,
     0},
    ...
    {{CSEL_COL2X + CSEL_PDX, R2Y(12), CSEL_PW, PR_H},
     ...
     NV_2M_COLOR,
     "2 m",
     {CSEL_COL2X + CSEL_ADX, R2Y(12) + CSEL_TBDY, CSEL_TBSZ, CSEL_TBSZ},
     false,
     0,
     0,
     0},
...
static const uint8_t qroff[NQR] = {KB_INDENT, KB_INDENT, KB_INDENT + KB_CHAR_W, KB_INDENT + 3 * KB_CHAR_W / 2};
...
static const char* getEntangledValue(BPIds a_bpr, BPIds b_bpr) {
    ...
    if (a_bpr != B.ent_mate || b_bpr != A.ent_mate) fatalError("getEntangledValue: %s vs %s", A.p_str, B.p_str);
...
static int getEntangledIndex(BPIds a_bpr, BPIds b_bpr) {
    ...
    if (a_bpr != B.ent_mate || b_bpr != A.ent_mate) fatalError("getEntangledIndex: %s vs %s", A.p_str, B.p_str);
...
static void setEntangledValue(BPIds a_bpr, BPIds b_bpr, const char* value) {
    ...
    if (a_bpr != B.ent_mate || b_bpr != A.ent_mate) fatalError("unpaired setEntangledValue %s %s", A.p_str, B.p_str);
...
static void logAllPrompts(void) {
    ...
    for (int i = 0; i < N_BPR; i++) {
        ...
        if (bp->ent_mate == i + 1)  // only print the forward-reference pair
            Serial.printf("Setup:    ent %3d: %s = %s\n", i, bp->p_str, getEntangledValue((BPIds)i, (BPIds)(i + 1)));
...
static void initShadowedParams() {
    ...
    snprintf(string_pr[DXPORT_SPR].v_str = (char*)malloc(8), string_pr[DXPORT_SPR].v_len = 8, "%u", dx_port);
    snprintf(string_pr[RIGPORT_SPR].v_str = (char*)malloc(8), string_pr[RIGPORT_SPR].v_len = 8, "%u", rig_port);

    snprintf(string_pr[ROTPORT_SPR].v_str = (char*)malloc(8), string_pr[ROTPORT_SPR].v_len = 8, "%u", rot_port);
    snprintf(string_pr[FLRIGPORT_SPR].v_str = (char*)malloc(8), string_pr[FLRIGPORT_SPR].v_len = 8, "%u", flrig_port);
    snprintf(string_pr[BME76DT_SPR].v_str = (char*)malloc(8), string_pr[BME76DT_SPR].v_len = 8, "%.2f",
             temp_corr[BME_76]);
    snprintf(string_pr[BME76DP_SPR].v_str = (char*)malloc(8), string_pr[BME76DP_SPR].v_len = 8, "%.3f",
             pres_corr[BME_76]);
    snprintf(string_pr[BME77DT_SPR].v_str = (char*)malloc(8), string_pr[BME77DT_SPR].v_len = 8, "%.2f",
             temp_corr[BME_77]);

    snprintf(string_pr[BME77DP_SPR].v_str = (char*)malloc(8), string_pr[BME77DP_SPR].v_len = 8, "%.3f",
             pres_corr[BME_77]);
    snprintf(string_pr[BRMIN_SPR].v_str = (char*)malloc(8), string_pr[BRMIN_SPR].v_len = 8, "%u", bright_min);
    snprintf(string_pr[BRMAX_SPR].v_str = (char*)malloc(8), string_pr[BRMAX_SPR].v_len = 8, "%u", bright_max);
    snprintf(string_pr[CENTERLNG_SPR].v_str = (char*)malloc(5), string_pr[CENTERLNG_SPR].v_len = 5, "%.0f%c",
             fabsf((float)center_lng), center_lng < 0 ? 'W' : 'E');
    // conversion to float just to avoid g++ snprintf size warning

    // value is set when displayed
    string_pr[CSELRED_SPR].v_str = (char*)calloc(4, 1);
    string_pr[CSELRED_SPR].v_len = 4;
    string_pr[CSELGRN_SPR].v_str = (char*)calloc(4, 1);
    string_pr[CSELGRN_SPR].v_len = 4;
    string_pr[CSELBLU_SPR].v_str = (char*)calloc(4, 1);
    string_pr[CSELBLU_SPR].v_len = 4;
}
 ...
 */
void formatLat(float lat_d, char s[], int s_len) { snprintf(s, s_len, "%.3f%c", fabsf(lat_d), lat_d < 0 ? 'S' : 'N'); }

/* format longitude into s[].
 */
void formatLng(float lng_d, char s[], int s_len) { snprintf(s, s_len, "%.3f%c", fabsf(lng_d), lng_d < 0 ? 'W' : 'E'); }
...
static bool boolIsRelevant(BoolPrompt* bp) {
    if (bp->page != cur_page) return (false);

#if !defined(_SHOW_ALL)

#if !defined(_USE_X11)
    if (bp == &bool_pr[X11_FULLSCRN_BPR]) return (false);
#endif
    ...
    if (bp == &bool_pr[CLISUDP_BPR]) {
        if (!bool_pr[CLUSTER_BPR].state) return (false);
    }

    if (bp == &bool_pr[DXCLCMDPGA_BPR] || bp == &bool_pr[DXCLCMDPGB_BPR]) {
        if (!bool_pr[CLUSTER_BPR].state || bool_pr[CLISUDP_BPR].state) return (false);
    }
    ...
    if (bp >= &bool_pr[DXCLCMD0_BPR] && bp < &bool_pr[DXCLCMD0_BPR + N_DXCLCMDS]) {
        ...
        if (cmd_page != pr_page || !bool_pr[CLUSTER_BPR].state || bool_pr[CLISUDP_BPR].state) return (false);
    }
    ...
    if (bp == &bool_pr[KX3B_BPR]) {
        ...
        return (false);
#else
        if (!bool_pr[KX3A_BPR].state || !bool_pr[GPIOOK_BPR].state) return (false);
#endif
    }
    ...
    if (bp == &bool_pr[DATEFMT_DMYYMD_BPR]) {
        ...
        if (!bool_pr[DATEFMT_MDY_BPR].state) return (false);
    }
    ...
    if (bp == &bool_pr[GPSDFOLLOW_BPR]) {
        if (!bool_pr[GPSDON_BPR].state) return (false);
    }

    if (bp == &bool_pr[NMEAFOLLOW_BPR]) {
        if (!bool_pr[NMEAON_BPR].state) return (false);
    }
    ...
    if (bp == &bool_pr[SETRADIO_BPR]) {
        if (!bool_pr[FLRIGUSE_BPR].state && !bool_pr[RIGUSE_BPR].state) return (false);
    }
...
static bool stringIsRelevant(StringPrompt* sp) {
    if (sp->page != cur_page) return (false);
...
#endif
#if defined(_WIFI_ASK)
        if (!bool_pr[WIFI_BPR].state) return (false);
#endif
    }
    ...
    if (sp == &string_pr[RIGHOST_SPR] || sp == &string_pr[RIGPORT_SPR]) {
        if (!bool_pr[RIGUSE_BPR].state) return (false);
    }

    if (sp == &string_pr[ROTHOST_SPR] || sp == &string_pr[ROTPORT_SPR]) {
        if (!bool_pr[ROTUSE_BPR].state) return (false);
    }

    if (sp == &string_pr[FLRIGHOST_SPR] || sp == &string_pr[FLRIGPORT_SPR]) {
        if (!bool_pr[FLRIGUSE_BPR].state) return (false);
    }

    if (sp == &string_pr[NTPHOST_SPR]) {
        if (strcmp(getEntangledValue(NTPA_BPR, NTPB_BPR), "host")) return (false);
    }

    if (sp == &string_pr[LAT_SPR] || sp == &string_pr[LNG_SPR] || sp == &string_pr[GRID_SPR]) {
        if (bool_pr[GEOIP_BPR].state || bool_pr[GPSDON_BPR].state || bool_pr[NMEAON_BPR].state) return (false);
    }
    ...
    if (sp == &string_pr[BME76DT_SPR] || sp == &string_pr[BME77DT_SPR] || sp == &string_pr[BME76DP_SPR] ||
        sp == &string_pr[BME77DP_SPR]) {
        return (bool_pr[GPIOOK_BPR].state || bool_pr[I2CON_BPR].state);
    }

    if (sp == &string_pr[BRMIN_SPR] || sp == &string_pr[BRMAX_SPR]) return (brDimmableOk());

    if (sp == &string_pr[ADIFFN_SPR]) {
        if (!bool_pr[ADIFSET_BPR].state) return (false);
    }
...
#define N_TAB_FIELDS NARRAY(tab_fields)
    ...
    for (tab_pos = 0; tab_pos < N_TAB_FIELDS; tab_pos++)
        if (memcmp(&cur_focus[cur_page], &tab_fields[tab_pos], sizeof(Focus)) == 0) break;
...
static void setInitialFocus() {
    // skip if already set
    if (cur_focus[cur_page].sp || cur_focus[cur_page].bp) return;
 ...
 */
static void eraseSPPrompt(const StringPrompt* sp) { fillSBox(sp->p_box, BG_C); }

/* erase the value of the given StringPrompt
 */
static void eraseSPValue(const StringPrompt* sp) { fillSBox(sp->v_box, BG_C); }
...
static void drawSPValue(StringPrompt* sp) {
    ...
    if (sp->v_ci > v_len) sp->v_ci = v_len;
    if (sp->v_ci < sp->v_wi) sp->v_wi = sp->v_ci;
...
static void drawBPState(BoolPrompt* bp) {
    ...
    if (show_t || show_f) {
        ...
        if (show_t) tft.print(bp->t_str);
        if (show_f) tft.print(bp->f_str);
#ifdef _MARK_BOUNDS
 ...
 */
static void eraseBPState(BoolPrompt* bp) { fillSBox(bp->s_box, BG_C); }
 ...
 */
static void eraseBPPrompt(BoolPrompt* bp) { fillSBox(bp->p_box, BG_C); }
...
static void checkLLGEdit(const StringPrompt* sp) {
    if (sp == &string_pr[LAT_SPR] || sp == &string_pr[LNG_SPR]) {
        ...
        if (latSpecIsValid(string_pr[LAT_SPR].v_str, ll.lat_d) && lngSpecIsValid(string_pr[LNG_SPR].v_str, ll.lng_d)) {
            ll.normalize();
...
static bool tappedStringPrompt(SCoord& s, StringPrompt** spp) {
    for (int i = 0; i < N_SPR; i++) {
        StringPrompt* sp = &string_pr[i];
        if (!stringIsRelevant(sp)) continue;
...
static NTPStateCode getNTPStateCode(void) {
    ...
    if (i >= 0 && i < NTPSC_N) return ((NTPStateCode)i);
...
static void drawCurrentPageFields() {
    ...
    for (int i = 0; i < N_SPR; i++) {
        StringPrompt* sp = &string_pr[i];
        if (stringIsRelevant(sp)) drawSPPromptValue(sp);
    }
    ...
    if (cur_page == bool_pr[NTPA_BPR].page) drawNTPPrompts();

    // draw spider header if appropriate
    if (cur_page == SPIDER_PAGE && bool_pr[CLUSTER_BPR].state && !bool_pr[CLISUDP_BPR].state) drawExtraCluster();

#if defined(_WIFI_ALWAYS)
    // show prompt but otherwise is not relevant
    if (bool_pr[WIFI_BPR].page == cur_page) drawBPPrompt(&bool_pr[WIFI_BPR]);
#endif
 ...
 */
static int getRawSpotRadius(ColSelPrompt& csp) { return (2 * (csp.t_state ? RAWTHINPATHSZ : RAWWIDEPATHSZ)); }
...
static void drawCSelInitGUI() {
    ...
    selectFontStyle(LIGHT_FONT, FAST_FONT);
    tft.setTextColor(PR_C);
    tft.setCursor(CSEL_COL1X + 4, R2Y(0) + PR_H / 2);
    tft.print("On");
    tft.setCursor(CSEL_COL1X + CSEL_EDX, R2Y(0) + PR_H / 2);
    tft.print("Edit");
    tft.setCursor(CSEL_COL1X + CSEL_ADX, R2Y(0) + PR_H / 2);
    tft.print("Dash");
    tft.setCursor(CSEL_COL1X + CSEL_TDX, R2Y(0) + PR_H / 2);
    tft.print("Thin");
    tft.setCursor(CSEL_COL2X + 4, R2Y(6) + PR_H / 2);
    tft.print("On");
    tft.setCursor(CSEL_COL2X + CSEL_EDX, R2Y(6) + PR_H / 2);
    tft.print("Edit");
    tft.setCursor(CSEL_COL2X + CSEL_ADX, R2Y(6) + PR_H / 2);
    tft.print("Dash");
    tft.setCursor(CSEL_COL2X + CSEL_TDX, R2Y(6) + PR_H / 2);
    tft.print("Thin");

    // N.B. must restore default font after using smaller font
    selectFontStyle(LIGHT_FONT, SMALL_FONT);
    tft.setTextColor(TX_C);
    tft.setCursor(350, 27);
    tft.print("Color Editor");
    ...
    for (int i = 0; i < N_CSPR; i++) {
        ...
        if (p.e_state) drawCSelPromptColor(p, true);
    }
 ...
 */
static void saveColorTable(int tbl_AB, NV_Name a_nv, NV_Name t_nv, NV_Name o_nv, const char* prompt, const SBox& box) {
 ...
 * prompt and box are to show some feedback.
 */
static void loadColorTable(int tbl_ab, NV_Name a_nv, NV_Name t_nv, NV_Name o_nv, const char* prompt, const SBox& box) {
    uint8_t r[N_CSPR], g[N_CSPR], b[N_CSPR];
...
#define N_PSK (BAND2_CSPR - BAND160_CSPR + 1)  // inclusive
    static const uint8_t psk_r[N_PSK] = {156, 213, 0, 90, 131, 238, 246, 197, 164, 238, 238, 238};
    static const uint8_t psk_g[N_PSK] = {250, 89, 0, 89, 214, 198, 242, 161, 48, 113, 60, 56};
    static const uint8_t psk_b[N_PSK] = {74, 222, 131, 246, 115, 65, 123, 106, 41, 180, 32, 148};
...
static void drawOnOffControls() {
    // title
    const char* title = brDimmableOk() ? "DE Daily Display On/Dim Times" : "DE Daily Display On/Off Times";
    ...
    for (int i = 0; i < DAYSPERWEEK; i++) tft.drawLine(OO_DHX(i), OO_Y0 - OO_RH, OO_DHX(i), OO_Y0 + 6 * OO_RH, KB_C);
...
static bool portOK(char* port_str, int min_port, uint16_t* portp) {
    ...
    if (first_bad == port_str || *first_bad != '\0' || portn < min_port || portn > 65535) return (false);
    *portp = portn;
    return (true);
}
...
static bool hostOK(char* host_str, int max_len) {
    ...
    if (hl > max_len - 1 || hl == 0) return (false);

    // localhost?
    if (!strcmp(host_str, "localhost")) return (true);
    ...
    if (n_dots < 1 || host_str[0] == '.' || host_str[hl - 1] == '.') return (false);
    if (n_other == 0 && (n_dots != 3 || n_digits != hl - 3)) return (false);
...
static bool clusterLoginOk() {
    ...
    char call[100];
    int ssid = 0;
    return (sscanf(dx_login, "%99[^-]-%d", call, &ssid) == 2 && ssid > 0 && ssid <= DXC_MAX_SSID &&
...
static bool callsignOk(const char* s) {
    ...
    if (slash == s + s_len - 1) return (false);  // slash is at the end

    return (s_len < NV_CALLSIGN_LEN && s_len >= 3 && !strHasSpace(s) && strHasDigit(s) && strHasAlpha(s));
}
...
static bool validateStringPrompts(bool show_errors) {
    ...
    if (!callsignOk(cs_info.call)) badsids[n_badsids++] = CALL_SPR;

    // check lat/long unless using something else
    if (!bool_pr[GEOIP_BPR].state && !bool_pr[GPSDON_BPR].state && !bool_pr[NMEAON_BPR].state) {
        if (!latSpecIsValid(string_pr[LAT_SPR].v_str, de_ll.lat_d)) badsids[n_badsids++] = LAT_SPR;

        if (!lngSpecIsValid(string_pr[LNG_SPR].v_str, de_ll.lng_d)) badsids[n_badsids++] = LNG_SPR;

        LatLong ll;
        if (!maidenhead2ll(ll, string_pr[GRID_SPR].v_str)) badsids[n_badsids++] = GRID_SPR;
    }
    ...
    if (bool_pr[RIGUSE_BPR].state) {
        if (!hostOK(string_pr[RIGHOST_SPR].v_str, NV_RIGHOST_LEN)) badsids[n_badsids++] = RIGHOST_SPR;
        if (!portOK(string_pr[RIGPORT_SPR].v_str, 1000, &rig_port)) badsids[n_badsids++] = RIGPORT_SPR;
    }

    // check rot_host and port if used
    if (bool_pr[ROTUSE_BPR].state) {
        if (!hostOK(string_pr[ROTHOST_SPR].v_str, NV_ROTHOST_LEN)) badsids[n_badsids++] = ROTHOST_SPR;
        if (!portOK(string_pr[ROTPORT_SPR].v_str, 1000, &rot_port)) badsids[n_badsids++] = ROTPORT_SPR;
    }

    // check flrig host and port if used
    if (bool_pr[FLRIGUSE_BPR].state) {
        if (!hostOK(string_pr[FLRIGHOST_SPR].v_str, NV_FLRIGHOST_LEN)) badsids[n_badsids++] = FLRIGHOST_SPR;
        if (!portOK(string_pr[FLRIGPORT_SPR].v_str, 1000, &flrig_port)) badsids[n_badsids++] = FLRIGPORT_SPR;
    }
    ...
    if (bool_pr[GPIOOK_BPR].state || bool_pr[I2CON_BPR].state) {
        ...
        if (fabsf(temp_corr[BME_76]) > MAX_BME_DTEMP) badsids[n_badsids++] = BME76DT_SPR;
        char* tc2_str = string_pr[BME77DT_SPR].v_str;
        temp_corr[BME_77] = atof(tc2_str);
        if (fabsf(temp_corr[BME_77]) > MAX_BME_DTEMP) badsids[n_badsids++] = BME77DT_SPR;

        char* pc_str = string_pr[BME76DP_SPR].v_str;
        pres_corr[BME_76] = atof(pc_str);
        if (fabsf(pres_corr[BME_76]) > MAX_BME_DPRES) badsids[n_badsids++] = BME76DP_SPR;
        char* pc2_str = string_pr[BME77DP_SPR].v_str;
        pres_corr[BME_77] = atof(pc2_str);
        if (fabsf(pres_corr[BME_77]) > MAX_BME_DPRES) badsids[n_badsids++] = BME77DP_SPR;
    }

    // require ssid and pw if wifi
    if (bool_pr[WIFI_BPR].state) {
        if (strlen(string_pr[WIFISSID_SPR].v_str) == 0) badsids[n_badsids++] = WIFISSID_SPR;
        if (strlen(string_pr[WIFIPASS_SPR].v_str) == 0) badsids[n_badsids++] = WIFIPASS_SPR;
    }

    // require plausible gpsd host name if used
    if (bool_pr[GPSDON_BPR].state) {
        if (!hostOK(string_pr[GPSDHOST_SPR].v_str, NV_GPSDHOST_LEN)) badsids[n_badsids++] = GPSDHOST_SPR;
    }
    ...
    if (strcmp(getEntangledValue(NTPA_BPR, NTPB_BPR), "host") == 0) {
        if (!hostOK(string_pr[NTPHOST_SPR].v_str, NV_NTPHOST_LEN)) badsids[n_badsids++] = NTPHOST_SPR;
    }
    ...
    if (brDimmableOk()) {
        ...
        if (!brmn_ok || (!order_ok && brmx_ok)) badsids[n_badsids++] = BRMIN_SPR;
        if (!brmx_ok || (!order_ok && brmn_ok)) badsids[n_badsids++] = BRMAX_SPR;
    ...
    if (bool_pr[I2CON_BPR].state) {
        strTrimAll(i2c_fn);
        if (!I2CFnOk()) badsids[n_badsids++] = I2CFN_SPR;
    }

    // if not showing, just return whether all ok
    if (!show_errors) return (n_badsids == 0);
    ...
    if (n_badsids > 0) {
        ...
        for (int pg_offset = 0; !show_bad && pg_offset < N_PAGES; pg_offset++) {
            ...
            for (int badsid_i = 0; !show_bad && badsid_i < n_badsids; badsid_i++) {
                ...
                if (sp->page == tmp_page) {
                    ...
                    if (tmp_page != cur_page || chg_pg1) changePage(tmp_page);
        ...
        if (!show_bad) fatalError("%d bad fields but none found", n_badsids);
...
static bool getWPACreds() {
    ...
    while (fgets(buf, sizeof(buf), wpa_fp)) {
        if (sscanf(buf, " ssid=\"%100[^\"]\"", wpa_ssid) == 1) found_ssid = true;
        if (sscanf(buf, " psk=\"%100[^\"]\"", wpa_psk) == 1) found_psk = true;
    }
    ...
    if (!found_ssid || !found_psk) return (false);
    ...
    if (!(is_debian && found_version && version < 12)) return (false);
...
static void initSetup() {
    ...
    if (!NVReadUInt16(NV_NMEABAUD, &nv_nmea_baud)) nv_nmea_baud = 9600;
...
// supply a reasonable system-dependent default
    ...
    for (int i = 0; i < N_DXCLCMDS; i++) bool_pr[DXCLCMD0_BPR + i].state = (dx_cmdmask & (1 << i)) != 0;
    ...
    if (!NVReadString(NV_DAILYONOFF, (char*)onoff)) {
        ...
        uint16_t on, off;
        if (!NVReadUInt16(NV_DPYON_OLD, &on)) on = 0;
        if (!NVReadUInt16(NV_DPYOFF_OLD, &off)) off = 0;
        for (int i = 0; i < DAYSPERWEEK; i++) {
            onoff[i] = on;
    ...
    uint8_t gray_dpy;
    if (!NVReadUInt8(NV_GRAYDPY, &gray_dpy)) gray_dpy = GRAY_OFF;
...
static void initDisplay() {
    ...
    for (int i = 0; i < N_SPR; i++) string_pr[i].v_ci = strlen(string_pr[i].v_str);
...
static void runSetup() {
    ...
    UserInput ui = {screen,        // bounding box
    ...
    do {
        ...
        if (ui.kb_char == CHAR_NONE) {
            if (!s2char(ui.tap, ui.kb_char)) ui.kb_char = CHAR_NONE;
        }

        // check NL
        if (ui.kb_char == CHAR_NL) continue;
        ...
        if (cur_page == COLOR_PAGE) {
            ...
            if (ui.kb_char == CHAR_NONE && handleCSelTouch(ui.tt, ui.tap)) continue;
        }

        if (cur_page == ONOFF_PAGE) {
            if (checkOnOffTouch(ui.tt, ui.tap)) continue;
        }
        ...
        if (ui.kb_char == CHAR_TAB || ui.kb_char == CHAR_UP || ui.kb_char == CHAR_DOWN) {
            ...
            if (vl > 0 && sp->v_ci > 0) {
                ...
                if (cur_page == LATLNG_PAGE) checkLLGEdit(sp);
                if (cur_page == COLOR_PAGE) handleCSelKB();
            ...
            if (sp == &string_pr[CALL_SPR]) ui.kb_char = toupper(ui.kb_char);
            ...
            if (ui.kb_char == CHAR_SPACE) bp = cur_focus[cur_page].bp;

            // ignore tapping on bools not being shown
            if (!bp || !boolIsRelevant(bp)) continue;
...
static void saveParams2NV() {
    ...
    for (int i = 0; i < N_DXCLCMDS; i++)
        if (bool_pr[DXCLCMD0_BPR + i].state) dx_cmdmask |= (1 << i);
    NVWriteUInt16(NV_DXCMDMASK, dx_cmdmask);

    // insure legal dx_login
    if (!clusterLoginOk()) setClusterLogin();

    NVWriteUInt16(NV_DXPORT, dx_port);
    NVWriteString(NV_DXLOGIN, dx_login);

    NVWriteUInt8(NV_LOGUSAGE, bool_pr[LOGUSAGE_BPR].state);
    NVWriteUInt8(NV_LBLSTYLE, bool_pr[SPOTLBLA_BPR].state ? (bool_pr[SPOTLBLB_BPR].state ? LBL_CALL : LBL_DOT)
    ...
    uint16_t x11flags = 0;
    if (bool_pr[X11_FULLSCRN_BPR].state) x11flags |= X11BIT_FULLSCREEN;
...
void clockSetup() {
    ...
    if ((!str_ok || askRun()) && askPasswd("setup", false)) {
        ...
        if (!str_ok) validateStringPrompts(true);
        ...
        if (ll_edited) sendDXClusterDELLGrid();
 ...
 */
const char* getCallsign() { return (cs_info.call); }
...
LabelType getSpotLabelType(void) {
    ...
    for (int i = 0; i < LBL_N; i++)
        if (strcmp(lbl_styles[i], lbl) == 0) return ((LabelType)i);
 ...
 */
bool useGeoIP() { return (bool_pr[GEOIP_BPR].state); }

/* return whether to use GPSD for time
 */
bool useGPSDTime() { return (bool_pr[GPSDON_BPR].state); }

/* return whether to use GPSD for location
 */
bool useGPSDLoc() { return (bool_pr[GPSDON_BPR].state && bool_pr[GPSDFOLLOW_BPR].state); }

/* return whether to use NMEA for time
 */
bool useNMEATime() { return (bool_pr[NMEAON_BPR].state); }

/* return whether to use NMEA for location
 */
bool useNMEALoc() { return (bool_pr[NMEAON_BPR].state && bool_pr[NMEAFOLLOW_BPR].state); }

/* return NMEA connection speed
 */
const char* getNMEABaud(void) { return (getEntangledValue(NMEABAUDA_BPR, NMEABAUDB_BPR)); }

/* return whether to use the NTP host set herein
 */
bool useLocalNTPHost() { return (strcmp(getEntangledValue(NTPA_BPR, NTPB_BPR), ntp_sn[NTPSC_HOST]) == 0); }

/* return whether to use OS for time
 */
bool useOSTime() { return (strcmp(getEntangledValue(NTPA_BPR, NTPB_BPR), ntp_sn[NTPSC_OS]) == 0); }
 ...
 */
bool logUsageOk() { return (bool_pr[LOGUSAGE_BPR].state); }

/* return whether ok to use GPIO
 */
bool GPIOOk() { return (bool_pr[GPIOOK_BPR].state); }
...
bool setBMETempCorr(BMEIndex i, float delta) {
    if ((i != BME_76 && i != BME_77) || !getBMEData(i, false)) return (false);
 ...
 */
float getBMETempCorr(int i) { return (temp_corr[i % MAX_N_BME]); }
 ...
 * at this point it's just a number, caller should interpret according to useMetricUnits()/useBritishUnits().
 */
float getBMEPresCorr(int i) { return (pres_corr[i % MAX_N_BME]); }

/* return KX3 baud rate, 0 if off or no GPIO
 */
uint32_t getKX3Baud() {
    return (bool_pr[KX3A_BPR].state && bool_pr[GPIOOK_BPR].state ? (bool_pr[KX3B_BPR].state ? 38400 : 4800) : 0);
}

/* return desired maximum brightness, percentage
 */
uint8_t getBrMax() { return (brDimmableOk() ? bright_max : 100); }

/* return desired minimum brightness, percentage
 */
uint8_t getBrMin() { return (brDimmableOk() ? bright_min : 0); }

/* whether to engage full screen using X11.
 */
bool getX11FullScreen(void) { return (bool_pr[X11_FULLSCRN_BPR].state); }

/* whether to engage full screen using the web interface.
 */
bool getWebFullScreen(void) { return (bool_pr[WEB_FULLSCRN_BPR].state); }

/* whether demo mode is requested
 */
bool getDemoMode(void) { return (bool_pr[DEMO_BPR].state); }

/* set whether demo mode is active
 */
void setDemoMode(bool on) { bool_pr[DEMO_BPR].state = on; }

/* return desired mercator map center longitude.
 * caller may assume -180 <= x < 180
 */
int16_t getCenterLng() { return (alt_center_lng_set ? alt_center_lng : center_lng); }
...
bool getRigctld(char host[NV_RIGHOST_LEN], int* portp) {
    if (bool_pr[RIGUSE_BPR].state) {
        if (host != NULL) strcpy(host, rig_host);
        if (portp != NULL) *portp = rig_port;
        return (true);
    }
    return (false);
}
...
bool getRotctld(char host[NV_ROTHOST_LEN], int* portp) {
    if (bool_pr[ROTUSE_BPR].state) {
        if (host != NULL) strcpy(host, rot_host);
        if (portp != NULL) *portp = rot_port;
        return (true);
    }
    return (false);
}
...
bool getFlrig(char host[NV_FLRIGHOST_LEN], int* portp) {
    if (bool_pr[FLRIGUSE_BPR].state) {
        if (host != NULL) strcpy(host, flrig_host);
        if (portp != NULL) *portp = flrig_port;
        return (true);
    }
    return (false);
}
 ...
 */
bool setRadio(void) { return (bool_pr[SETRADIO_BPR].state); }

/* get name to use for cluster login
 */
const char* getDXClusterLogin() { return (dx_login[0] != '\0' ? dx_login : cs_info.call); }
 ...
 */
bool getPathDashed(ColorSelection id) { return (csel_pr[id].a_state); }

/* return whether dx host is actually WSJT-X or actually any UDP source.
 */
bool useWSJTX(void) { return (bool_pr[CLISUDP_BPR].state); }

/* return name of ADIF, else NULL if not set
 */
const char* getADIFilename(void) { return (bool_pr[ADIFSET_BPR].state ? adif_fn : NULL); }
...
WatchListState getWatchListState(WatchListId wl_id, char name[WLA_MAXLEN]) {
    // insure valid
    if (!wlIdOk(wl_id)) fatalError("getWatchListState %d", (int)wl_id);
    ...
    if (wl_s == WLA_NONE) fatalError("getWatchListState for %d unknown state %s", wl_id, wl_v);

    // pass back if interested
    if (name) snprintf(name, WLA_MAXLEN, "%s", wla_name[wl_s]);
...
void getWatchList(WatchListId wl_id, char** wl_copypp, size_t* wl_lenp) {
    if (!wlIdOk(wl_id)) fatalError("getWatchList %d", (int)wl_id);
...
void setWatchList(WatchListId wl_id, const char* new_state, char* new_wlstr) {
    ...
    if (!wlIdOk(wl_id)) fatalError("setWatchList %d %.10s %.10s", (int)wl_id, new_state, new_wlstr);
    WLInfo& wli = wl_info[wl_id];
...
void rotateWatchListState(struct _menu_text* tfp) {
    ...
    if (wl_s == WLA_NONE) fatalError("rotateWatchListState unknown name: %.10s", tfp->label);
...
const char* getWatchListName(WatchListId wl_id) {
    if (!wlIdOk(wl_id)) fatalError("getWatchListName %d", (int)wl_id);
    return (wl_info[wl_id].name);
}

/* return whether scolling panes should show the newest entry on top, else newest on bottom
 */
bool scrollTopToBottom(void) { return (bool_pr[SCROLLDIR_BPR].state); }

/* return whether to automatically show new DX or DE weather when either changes
 */
bool showNewDXDEWx(void) { return (bool_pr[NEWDXDEWX_BPR].state); }

/* return the desired pane rotation period, seconds
 */
int getPaneRotationPeriod(void) { return (atoi(getEntangledValue(PANE_ROTPA_BPR, PANE_ROTPB_BPR))); }

/* return whether to show the puplic IP address
 */
bool showPIP() { return (bool_pr[SHOWPIP_BPR].state); }

/* return whether to run the automatic space weather map detection.
 */
bool autoMap() { return (bool_pr[AUTOMAP_BPR].state); }

/* return map rotation period, seconds
 */
int getMapRotationPeriod() { return (atoi(getEntangledValue(MAP_ROTPA_BPR, MAP_ROTPB_BPR))); }
...
QRZURLId getQRZId(void) {
    ...
    for (int i = 0; i < QRZ_N; i++)
        if (strcmp(label, qrz_urltable[i].label) == 0) return ((QRZURLId)i);
 ...
 */
bool showTempC(void) { return (useMetricUnits() || useBritishUnits()); }

/* show atmopsheric pressure in hPa, else inHg
 */
bool showATMhPa(void) { return (useMetricUnits() || useBritishUnits()); }

/* distance in km and speeds in km/hr, else mi and mph
 */
bool showDistKm(void) { return (useMetricUnits()); }

/* return whether a UDP spot also sets DX
 */
bool UDPSetsDX(void) { return (bool_pr[UDPSETSDX_BPR].state); }
...
bool useUDPSpot(const DXSpot& s) {
    ...
    if (bool_pr[UDPSPOTS_BPR].state) return (true);
 ...
 */
int maxTLEAgeDays(void) { return (atoi(getEntangledValue(MAXTLEA_BPR, MAXTLEB_BPR))); }
