...
bool maidenhead2ll(LatLong& ll, const char maid[MAID_CHARLEN]) {
    ...
    for (int i = 0; i < MAID_CHARLEN; i++) uc_maid[i] = toupper(maid[i]);

    // check first four chars, always required
    if (uc_maid[0] < 'A' || uc_maid[0] > 'R' || uc_maid[1] < 'A' || uc_maid[1] > 'R' || !isdigit(uc_maid[2]) ||
        !isdigit(uc_maid[3]))
        return (false);
...
void getNVMaidenhead(NV_Name nv, char maid[MAID_CHARLEN]) {
    if (!NVReadString(nv, maid)) fatalError("getNVMaidenhead invalid %d", (int)nv);
}
