...
static void ourSystem(const char* fmt, ...) {
    ...
    if (child_pid == 0) {
        ...
        // engage full perm
        if (setuid(geteuid()) < 0) ::printf("setuid(%d): %s\n", geteuid(), strerror(errno));
    ...
    char rsp[1000];
    rsp[0] = 0;
    while (fgets(rsp, sizeof(rsp), rsp_fp)) ::printf("%s", rsp);
...
bool Adafruit_RA8875::begin(int not_used) {
    ...
    img = XCreateImage(display, visual, visdepth, ZPixmap, 0, (char*)fb_stage, FB_XRES, FB_YRES, BITSPFBPIX, 0);
    ...
    win = XCreateWindow(display, root, win_x, win_y, fb_si.xres, fb_si.yres, 0, visdepth, InputOutput, visual,
                        value_mask, &wa);
    ...
    XSelectInput(display, win,
                 KeyPressMask | KeyReleaseMask | PointerMotionMask | LeaveWindowMask | ButtonReleaseMask |
    ...
    if (fb_fb == MAP_FAILED) {
        ::printf("mmap(%d,%ux%ux%u=%u): %s\n", fb_fd, BYTESPFBPIX, fb_si.xres, fb_si.yres, (unsigned)si_bytes,
                 strerror(errno));
        close(fb_fd);
        exit(1);
    }
...
bool Adafruit_RA8875::displayReady() { return (ready); }
...
void Adafruit_RA8875::fillScreen(uint16_t color16) { fillRect(0, 0, width(), height(), color16); }

void Adafruit_RA8875::setTextColor(uint16_t color16) { text_color = RGB16TOFBPIX(color16); }
...
void Adafruit_RA8875::getTextBounds(const char* string, int16_t x, int16_t y, int16_t* x1, int16_t* y1, uint16_t* w,
                                    uint16_t* h) {
    (void)x;
    (void)y;
    ...
    char c;
    while ((c = *string++) != '\0') {
        if (c < current_font->first || c > current_font->last) continue;  // don't print so don't count
        GFXglyph* gp = &current_font->glyph[c - current_font->first];
        totw += gp->xAdvance;
        if (gp->yOffset < miny) miny = gp->yOffset;
        if (gp->yOffset + gp->height > maxy) maxy = gp->yOffset + gp->height;
    }
...
void Adafruit_RA8875::print(char c) { plotChar(c); }

void Adafruit_RA8875::print(char* s) {
    char c;
    while ((c = *s++) != '\0') plotChar(c);
}
...
void Adafruit_RA8875::print(int i, int b) {
    ...
    for (int i = 0; i < sl; i++) plotChar(buf[i]);
}
...
const GFXfont* Adafruit_RA8875::getFont(void) { return (current_font ? current_font : &Courier_Prime_Sans6pt7b); }
...
void Adafruit_RA8875::drawPixel(int16_t x, int16_t y, uint16_t color16) {
    ...
    if (SCALESZ == 2) {
        ...
        for (uint8_t dx = 0; dx < SCALESZ; dx++)
            for (uint8_t dy = 0; dy < SCALESZ; dy++) plotfb(x + dx, y + dy, fbpix);
    }
    fb_dirty = true;
...
void Adafruit_RA8875::drawLine(int16_t x0, int16_t y0, int16_t x1, int16_t y1, int16_t thickness, uint16_t color16) {
    fbpix_t fbpix = RGB16TOFBPIX(color16);
    x0 *= SCALESZ;
    y0 *= SCALESZ;
    x1 *= SCALESZ;
 ...
 */
void Adafruit_RA8875::drawLineRaw(int16_t x0, int16_t y0, int16_t x1, int16_t y1, int16_t thickness, uint16_t color16) {
    fbpix_t fbpix = RGB16TOFBPIX(color16);
    pthread_mutex_lock(&fb_lock);
    plotLineRaw(x0, y0, x1, y1, thickness, fbpix);
    // if (thickness >= 3) {
...
void Adafruit_RA8875::drawCircleRaw(int16_t x0, int16_t y0, uint16_t r0, int thickness, uint16_t color16) {
    ...
    for (int i = 0; i < thickness; i++)
        if (i < r0) plotDrawCircle(x0, y0, r0 - i, fbpix);
}
...
void Adafruit_RA8875::fillTriangleRaw(int16_t x0, int16_t y0, int16_t x1, int16_t y1, int16_t x2, int16_t y2,
    ...
    if (y0 > y2) swap2(x0, y0, x2, y2);
    if (y0 > y1) swap2(x0, y0, x1, y1);
    if (y1 > y2) swap2(x1, y1, x2, y2);
...
void Adafruit_RA8875::plotFillRect(int16_t x0, int16_t y0, int16_t w, int16_t h, fbpix_t fbpix) {
    ...
    for (uint16_t y = y0; y < y0 + h; y++)
        for (uint16_t x = x0; x < x0 + w; x++) plotfb(x, y, fbpix);
    fb_dirty = true;
    pthread_mutex_unlock(&fb_lock);
}
...
void Adafruit_RA8875::plotDrawCircle(int16_t x0, int16_t y0, uint16_t r0, fbpix_t fbpix) {
    ...
    for (int32_t dy = -2 * r0; dy <= 2 * r0; dy += 2) {
        for (int32_t dx = -2 * r0; dx <= 2 * r0; dx += 2) {
            ...
            if (xy2 >= iradius2 && xy2 <= oradius2) plotfb(x0 + dx / 2, y0 + dy / 2, fbpix);
        }
    }
    fb_dirty = true;
    pthread_mutex_unlock(&fb_lock);
...
void Adafruit_RA8875::plotFillCircle(int16_t x0, int16_t y0, uint16_t r0, fbpix_t fbpix) {
    ...
    for (int32_t dy = -2 * r0; dy <= 2 * r0; dy += 2) {
        for (int32_t dx = -2 * r0; dx <= 2 * r0; dx += 2) {
            uint32_t xy2 = dx * dx + dy * dy;
            if (xy2 <= radius2) plotfb(x0 + dx / 2, y0 + dy / 2, fbpix);
        }
    }
    fb_dirty = true;
    pthread_mutex_unlock(&fb_lock);
 ...
 * Overlap draws additional pixel when changing minor direction. For standard bresenham overlap, choose
 * LINE_OVERLAP_NONE (0).
 *
 *  Sample line:
 *
 ...
 */
void Adafruit_RA8875::drawLineOverlap(int16_t aXStart, int16_t aYStart, int16_t aXEnd, int16_t aYEnd, uint8_t aOverlap,
                                      fbpix_t aColor) {
 ...
 * aThicknessMode can be one of LINE_THICKNESS_MIDDLE, LINE_THICKNESS_DRAW_CLOCKWISE,
 * LINE_THICKNESS_DRAW_COUNTERCLOCKWISE
 */
void Adafruit_RA8875::drawThickLine(int16_t aXStart, int16_t aYStart, int16_t aXEnd, int16_t aYEnd, int16_t aThickness,
                                    uint8_t aThicknessMode, fbpix_t aColor) {
    ...
    if (tDeltaX >= tDeltaY) {
        ...
        /*
         * Vector for draw direction of the starting points of lines is rectangular and counterclockwise to main line
         * direction Therefore no pixel will be missed if LINE_OVERLAP_MAJOR is used on change in minor rectangular
         * direction
         */
        ...
        // draw start line. We can alternatively use drawLineOverlap(aXStart, aYStart, aXEnd, aYEnd, LINE_OVERLAP_NONE,
        // aColor) here.
 ...
 */
void Adafruit_RA8875::plotLineRaw(int16_t x0, int16_t y0, int16_t x1, int16_t y1, int16_t thick, fbpix_t color) {
 ...
 */
void Adafruit_RA8875::plotEarth(uint16_t x0, uint16_t y0, float lat0, float lng0, float dlatr, float dlngr, float dlatd,
                                float dlngd, float fract_day) {
    // beware of no map files
    if (!DEARTH_BIG || !NEARTH_BIG) return;
...
void Adafruit_RA8875::plotChar(char ch) {
    if (ch < current_font->first || ch > current_font->last) return;  // don't print if don't count length
    ...
    for (uint16_t r = 0; r < gp->height; r++) {
        for (uint16_t c = 0; c < gp->width; c++) {
            ...
            if (bit) plotfb(x + c, y + r, text_color);
            bitn++;
        }
    }
    fb_dirty = true;
...
char Adafruit_RA8875::getChar(bool* control_set, bool* shift_set) {
    ...
    if (kb_qhead != kb_qtail) {
        ...
        c = ks.c;
        if (control_set) *control_set = ks.control;
        if (shift_set) *shift_set = ks.shift;
        if (++kb_qhead == KB_N) kb_qhead = 0;
    }
...
void Adafruit_RA8875::X11OptionsEngageNow(bool fs) {
    ...
    // wait here
    while (options_engage) usleep(1000);
}
...
bool Adafruit_RA8875::requestSelection(KeySym ks, unsigned kb_state) {
    if (ks == XK_v) {
        ...
        if (kb_state == m1) {
            ...
            Atom bufid = XInternAtom(display, "CLIPBOARD", False), fmtid = XInternAtom(display, "STRING", False),
        ...
        if (kb_state == m2 || kb_state == m3) {
            ...
            Atom bufid = XInternAtom(display, "PRIMARY", False), fmtid = XInternAtom(display, "STRING", False),
                 propid = XInternAtom(display, "XSEL_DATA", False);
...
void Adafruit_RA8875::captureSelection() {
    ...
    int resbits;
    Atom fmtid = XInternAtom(display, "STRING", False), propid = XInternAtom(display, "XSEL_DATA", False);

    if (Success == XGetWindowProperty(display, win, propid, 0, 100, False, AnyPropertyType, &fmtid, &resbits, &ressize,
                                      &restail, &result) &&
        resbits == 8) {  // means result is array of char

        // inject selection into kb q
        for (unsigned i = 0; i < ressize; i++) putChar(result[i], false, false);
...
void Adafruit_RA8875::encodeKeyEvent(XKeyEvent* event) {
    ...
    switch (ks) {
        ...
        case XK_v:  // might be paste
            if (requestSelection(XK_v, event->state)) return;
            break;
    }

    // if nothing yet try a string
    if (!c && XLookupString(event, buf, sizeof(buf), NULL, NULL) > 0) c = buf[0];
...
void Adafruit_RA8875::saveWinGeom(void) {
    if (options_fullscreen || ignore_x11geom) return;
...
int Adafruit_RA8875::decodeMouseButton(XEvent event) {
    ...
    return ((event.xbutton.button == Button1 && mods) || (event.xbutton.button == Button2) ? Button2 : Button1);
}
...
// #define _DUMP_CURSOR_MASK
    ...
    for (;;) {
        ...
        if (options_engage) {
            ...
            XSendEvent(display, DefaultRootWindow(display), False, SubstructureNotifyMask | SubstructureRedirectMask,
                       &event);
        ...
        while (XPending(display) > 0) {
            ...
            switch (event.type) {
                case Expose:
                    ...
                    XCopyArea(display, pixmap, win, black_gc, event.xexpose.x - FB_X0, event.xexpose.y - FB_Y0,
                ...
                case SelectionNotify:
                    ...
                    if (event.xselection.property) captureSelection();
                    break;
...
bool Adafruit_RA8875::warpCursor(char dir, unsigned n, int* xp, int* yp) {
    ...
    switch (dir) {
        case CHAR_LEFT:
            new_x = win_x - n * SCALESZ;
            break;
        case CHAR_DOWN:
            new_y = win_y + n * SCALESZ;
            break;
        case CHAR_UP:
            new_y = win_y - n * SCALESZ;
            break;
...
// _WEB_ONLY
void Adafruit_RA8875::X11OptionsEngageNow(bool fs) { (void)fs; }
...
void Adafruit_RA8875::findKeyboard() {
    ...
    if (kb_fd < 0) {
        ...
        if (ioctl(kb_fd, KDSETMODE, KD_GRAPHICS) < 0) ::printf("KDSETMODE KD_GRAPHICS: %s\n", strerror(errno));
...
#define test_bit(bit, array) ((array[LONG(bit)] >> OFF(bit)) & 1)
        ...
        if (touch_fd < 0 && test_bit(EV_ABS, bit[0]) && ioctl(evfd, EVIOCGBIT(EV_ABS, KEY_MAX), bit[EV_ABS]) >= 0 &&
            test_bit(ABS_X, bit[EV_ABS]) && test_bit(ABS_Y, bit[EV_ABS]) &&
            ioctl(evfd, EVIOCGBIT(EV_KEY, KEY_MAX), bit[EV_KEY]) >= 0 && test_bit(BTN_TOUCH, bit[EV_KEY])) {
            ::printf("POINTER: found touch screen at %s\n", fullevpath);
            touch_fd = evfd;
            evfd_used = true;
        }

        if (mouse_fd < 0 && test_bit(EV_REL, bit[0]) && ioctl(evfd, EVIOCGBIT(EV_REL, KEY_MAX), bit[EV_REL]) >= 0 &&
            test_bit(REL_X, bit[EV_REL]) && test_bit(REL_Y, bit[EV_REL]) &&
            ioctl(evfd, EVIOCGBIT(EV_KEY, KEY_MAX), bit[EV_KEY]) >= 0 && test_bit(BTN_LEFT, bit[EV_KEY])) {
...
void Adafruit_RA8875::mouseThread(void) {
    ...
    for (;;) {
        ...
        if (touch_fd >= 0) {
            FD_SET(touch_fd, &rfd);
            if (touch_fd > max_fd) max_fd = touch_fd;
        }
        if (max_fd == 0) {
        ...
        if (read(ready_fd, &iev, sizeof(iev)) == sizeof(iev)) {
            ...
            if (fb_dirty) {
                // insure in range
                if (mouse_x < FB_X0) mouse_x = FB_X0;
                if (mouse_x >= FB_X0 + SCALESZ * width()) mouse_x = FB_X0 + SCALESZ * width() - 1;
                if (mouse_y < FB_Y0) mouse_y = FB_Y0;
                if (mouse_y >= FB_Y0 + SCALESZ * height()) mouse_y = FB_Y0 + SCALESZ * height() - 1;
...
bool Adafruit_RA8875::warpCursor(char dir, unsigned n, int* xp, int* yp) {
    ...
    switch (dir) {
        case CHAR_LEFT:
            new_x = mouse_x - n * SCALESZ;
            break;
        case CHAR_DOWN:
            new_y = mouse_y + n * SCALESZ;
            break;
        case CHAR_UP:
            new_y = mouse_y - n * SCALESZ;
            break;
...
void Adafruit_RA8875::setCursorIfVis(uint16_t row, uint16_t col, fbpix_t color) {
    ...
    row += mouse_y - FB_Y0;
    col += mouse_x - FB_X0;
    if (row < FB_YRES && col < FB_XRES) fb_cursor[row * FB_XRES + col] = color;
}
    ...
    for (;;) {
        ...
        if (is_new || mouse_idle < MOUSE_FADE) {
            ...
            if (mouse_idle < MOUSE_FADE) {
                ...
                // fill top half
                for (uint16_t r = 0; r < FB_CURSOR_SZ / 2; r++)
                    for (uint16_t c = r / 2 + 1; c < 2 * r - 1; c++) setCursorIfVis(r, c, bgcolor);
                // fill bottom half
                for (uint16_t r = FB_CURSOR_SZ / 2; r < FB_CURSOR_SZ; r++)
                    for (uint16_t c = r / 2 + 1; c < 3 * FB_CURSOR_SZ / 2 - r - 1; c++) setCursorIfVis(r, c, bgcolor);
                // draw border
                for (uint16_t i = 0; i < FB_CURSOR_SZ / 2; i++) {
