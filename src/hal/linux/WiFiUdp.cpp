...
bool WiFiUDP::begin(int port) {
    ...
    if (debugLevel(DEBUG_NET, 1)) printf("UDP: new socket %d port %d\n", sockfd, port);
...
bool WiFiUDP::beginMulticast(IPAddress ifIP, IPAddress mcIP, int port) {
    ...
    if (debugLevel(DEBUG_NET, 1)) printf("UDP: new multicast socket %d\n", sockfd);
...
int WiFiUDP::parsePacket() {
    ...
    FD_ZERO(&rset);
    FD_SET(sockfd, &rset);
    if (debugLevel(DEBUG_NET, 1)) printf("UDP: checking for pending packet\n");
    ...
    if (s == 0) {
        if (debugLevel(DEBUG_NET, 1)) printf("UDP: socket %d read timed out\n", sockfd);
        return (0);
    }

    socklen_t rlen = sizeof(remoteip);
    r_n = ::recvfrom(sockfd, r_buf, sizeof(r_buf), 0, (struct sockaddr*)&remoteip, &rlen);
    if (debugLevel(DEBUG_NET, 1)) printf("UDP: socket %d read %d\n", sockfd, r_n);
    if (r_n < 0) {
...
void WiFiUDP::stop() {
    if (sockfd >= 0) {
        ::close(sockfd);
        if (debugLevel(DEBUG_NET, 1)) printf("UDP: closing socket %d\n", sockfd);
        sockfd = -1;
    }
}
