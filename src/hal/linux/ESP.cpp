...
void ESP::restart(bool minus_K, bool minus_0) {
    ...
    for (char** argv = our_argv; *argv != NULL; argv++) {
        ...
        if (s[0] == '-') {
            ...
            if ((*argv)[1] != '\0') addArgv(tmp_argv, tmp_argc, *argv);
        } else {
    ...
    if (minus_K) addArgv(tmp_argv, tmp_argc, "-K");
    if (minus_0) addArgv(tmp_argv, tmp_argc, "-0");
    ...
    for (int i = 0; tmp_argv[i] != NULL; i++) printf("  argv[%d]: %s\n", i, tmp_argv[i]);
