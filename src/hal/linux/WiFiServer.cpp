...
WiFiServer::WiFiServer(int newport) {
    port = newport;
    socket = -1;
    if (_trace_server) printf("WiFiServer: new instance on port %d\n", port);
}
...
bool WiFiServer::begin(char ynot[]) {
    ...
    /* ok */
    if (_trace_server) printf("WiFiServer: new server fd %d\n", socket);
    socket = sfd;
    return (true);
}
    ...
    if (socket >= 0) {
        ...
        if (s == 1 && FD_ISSET(socket, &fs)) {
            ...
            if (cli_fd < 0)
                ...
                if (_trace_server) printf("WiFiServer: available() found new client fd %d\n", cli_fd);
            }
        }
    }
...
void WiFiServer::stop() {
    if (socket >= 0) {
        if (_trace_server) printf("WiFiServer: closing fd %d\n", socket);
...
WiFiClient WiFiServer::next() {
    ...
    if (socket >= 0) {
        ...
        if (cli_fd < 0)
            ...
            if (_trace_server) printf("WiFiServer: next() found new client fd %d\n", cli_fd);
        }
    }
