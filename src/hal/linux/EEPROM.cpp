...
void EEPROM::begin(int s) {
    ...
    if (flock(fileno(fp), LOCK_EX | LOCK_NB) < 0) {
        fprintf(stderr,
                "Another instance of HamClock has been detected.\n"
...
bool EEPROM::commit(void) {
    ...
    for (unsigned a = 0; a < n_data_array; a++) fprintf(fp, "%08X %02X\n", a, data_array[a]);
    fflush(fp);
...
uint8_t EEPROM::read(uint32_t address) {
    ...
    if (data_array && address < n_data_array) return (data_array[address]);
