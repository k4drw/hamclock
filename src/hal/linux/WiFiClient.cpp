...
WiFiClient::WiFiClient(int fd) {
    if (fd >= 0 && debugLevel(DEBUG_NET, 1)) printf("WiFiCl: new WiFiClient inheriting fd %d\n", fd);
...
WiFiClient::operator bool() {
    ...
    if (active && debugLevel(DEBUG_NET, 2)) printf("WiFiCl: fd %d is active\n", socket);
    return (active);
}
...
int WiFiClient::connect_to(int sockfd, struct sockaddr* serv_addr, int addrlen, int to_ms) {
    ...
    if (fcntl(sockfd, F_SETFL, flags) < 0) printf("WiFiCl: fcntl fd %d: %s\n", sockfd, strerror(errno));
...
bool WiFiClient::connect(const char* host, int port) {
    ...
    /* ok start fresh */
    if (debugLevel(DEBUG_NET, 1)) printf("WiFiCl: new %s:%d fd %d\n", host, port, sockfd);
    freeaddrinfo(aip);
...
void WiFiClient::stop() {
    if (socket >= 0) {
        if (debugLevel(DEBUG_NET, 1)) printf("WiFiCl: stopping fd %d\n", socket);
...
bool WiFiClient::connected() { return (socket >= 0); }
...
bool WiFiClient::pending(int ms) {
    ...
    if (debugLevel(DEBUG_NET, 2)) printf("WiFiCl: %smore pending\n", more ? "" : "no ");
...
int WiFiClient::available(int pending_ms) {
    ...
    if (next_peek < n_peek) return (1);

    // wait as instructed
    if (!pending(pending_ms)) return (0);

    // read more
    int nr = ::read(socket, peek, sizeof(peek));
    if (nr > 0) {
        if (debugLevel(DEBUG_NET, 2)) printf("WiFiCl: available read(%d,%ld) %d\n", socket, (long)sizeof(peek), nr);
        if (debugLevel(DEBUG_NET, 3)) logBuffer(peek, nr);
        n_peek = nr;
        next_peek = 0;
        return (1);
    } else if (nr == 0) {
        if (debugLevel(DEBUG_NET, 1)) printf("WiFiCl: available read(%d) EOF\n", socket);
        stop();
        return (0);
    } else {
        if (debugLevel(DEBUG_NET, 1)) printf("WiFiCl: available read(%d): %s\n", socket, strerror(errno));
        stop();
        return (0);
    }
}
...
int WiFiClient::readArray(uint8_t* array, long count) {
    ...
    if (debugLevel(DEBUG_NET, 2)) printf("WiFiCl: readArray(%d,%ld) %d\n", socket, count, n_return);
    return (n_return);
}
...
int WiFiClient::write(const uint8_t* buf, int n) {
    ...
    if (debugLevel(DEBUG_NET, 2)) printf("WiFiCl: write(%d) %d\n", socket, n);
    if (debugLevel(DEBUG_NET, 3)) logBuffer(buf, n);
...
void WiFiClient::print(const char* str) { write((const uint8_t*)str, strlen(str)); }
...
void WiFiClient::println(void) { write((const uint8_t*)"\r\n", 2); }
