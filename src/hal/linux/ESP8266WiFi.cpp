...
static bool getCommand(const char cmd[], char line[], size_t line_len) {
    ...
    line[0] = '\0';
    if (debugLevel(DEBUG_NET, 2) || debugLevel(DEBUG_WIFI, 2)) printf("** run=%s\n", cmd);
    FILE* pp = popen(cmd, "r");
    if (!pp) return (false);
    ...
    if (fgets_ok && exited && exstatus == 0 && strlen(line) > 1) {
        ...
        if (debugLevel(DEBUG_NET, 2) || debugLevel(DEBUG_WIFI, 2)) printf("** back=%s\n", line);
        return (true);
    }
...
static bool crackIP(const char line[], IPAddress& a) {
    ...
    if (sscanf(line, "%d.%d.%d.%d", &i[0], &i[1], &i[2], &i[3]) != 4) return (false);
...
void WiFi::begin(char* ssid, char* pw) {
    ...
    if (!ssid || ssid[0] == '\0' || !pw || pw[0] == '\0') return;
...
IPAddress WiFi::gatewayIP(void) {
    ...
    if (getCommand(cmd, back, sizeof(back)) && crackIP(back, a)) return (a);
...
bool WiFi::RSSI(int& value, bool& is_dbm) {
    ...
    for (struct ifaddrs* ifa = ifaddr; ifa != NULL; ifa = ifa->ifa_next) {
        ...
        if (ioctl(sock, SIOCGIWNAME, &wrq) == 0) {
            if (debugLevel(DEBUG_WIFI, 1)) printf("found wireless connection %s\n", wrq.ifr_name);
    ...
    if (ok) {
        if (debugLevel(DEBUG_WIFI, 1)) {
            printf("Signal level%s is %d%s.\n", (iwstats.qual.updated & IW_QUAL_DBM ? " (in dBm)" : " (as percentage)"),
                   iwstats.qual.level, (iwstats.qual.updated & IW_QUAL_LEVEL_UPDATED ? " (updated)" : ""));
        }
    ...
    if (debugLevel(DEBUG_WIFI, 1) && !ok) printf("No RSSI from apple\n");
...
int WiFi::status(void) {
    ...
    // return result code
    if (!ok) printf("no net connections\n");
...
std::string WiFi::macAddress(void) {
    ...
    for (int i = 0; i < n_cmds; i++) {
        if (getCommand(cmds[i], line, sizeof(line))) {
            ...
            if (sscanf(line, "%x:%x:%x:%x:%x:%x", &m1, &m2, &m3, &m4, &m5, &m6) == 6) break;
        }
...
int WiFi::channel(void) {
    ...
    if (pf) {
        ...
        while (fgets(buf, sizeof(buf), pf))
            if (sscanf(buf, " channel %d", &channel) == 1) break;
        pclose(pf);
    }
#endif
