...
static void* blinkerThread(void* vp) {
    ...
    while (!tb.disable) {
        ...
        if (new_hz == BLINKER_ON) {
            // constant on
            if (new_hz != prev_hz) mcp.digitalWrite(tb.pin, !tb.on_is_low);
            n_delay = 0;
...
void startBinkerThread(int pin, bool on_is_low) {
    Message ynot;
    if (!checkUserPin(pin, ynot)) fatalError("startBinkerThread: %s", ynot.get());
    ...
    if (!tb.running && !tb.error) {
        ...
        if (e != 0) fatalError("LED blinker thread for pin %d failed: %s", tb.pin, strerror(e));
        if (debugLevel(DEBUG_IO, 1)) Serial.printf("MCP: started blinker thread for pin %d\n", pin);
    }
}
...
void setBlinkerRate(int pin, int hz) {
    Message ynot;
    if (!checkUserPin(pin, ynot)) fatalError("setBlinkerRate: %s", ynot.get());
    volatile ThreadBlinker& tb = blinkers[pin];

    if (debugLevel(DEBUG_IO, 1) && tb.hz != hz) printf("setBlinkerRate pin %d  %d -> %d\n", pin, tb.hz, hz);
...
void disableBlinker(int pin) {
    Message ynot;
    if (!checkUserPin(pin, ynot)) fatalError("disableBlinker: %s", ynot.get());
    volatile ThreadBlinker& tb = blinkers[pin];
...
void startMCPPoller(int pin) {
    Message ynot;
    if (!checkUserPin(pin, ynot)) fatalError("startMCPPoller: %s", ynot.get());
    ...
    if (!mp.running && !mp.error) {
        ...
        if (e != 0) fatalError("LED poller thread for pin %d failed: %s", mp.pin, strerror(e));
        if (debugLevel(DEBUG_IO, 1)) Serial.printf("MCP: started poller thread for pin %d\n", pin);
    }
}
...
bool readMCPPoller(int pin) {
    Message ynot;
    if (!checkUserPin(pin, ynot)) fatalError("readMCPPoller: %s", ynot.get());
    volatile MCPPoller& mp = pollers[pin];
...
bool readLatchedMCPPoller(int pin, bool& value) {
    Message ynot;
    if (!checkUserPin(pin, ynot)) fatalError("readLatchedMCPPoller: %s", ynot.get());
    volatile MCPPoller& mp = pollers[pin];
...
void disableMCPPoller(int pin) {
    Message ynot;
    if (!checkUserPin(pin, ynot)) fatalError("disableMCPPoller: %s", ynot.get());
    volatile MCPPoller& mp = pollers[pin];
...
bool setUserGPIO(int pin, int hz, Message& ynot) {
    // validate pin
    if (!checkUserPin(pin, ynot)) return (false);
    volatile ThreadBlinker& tb = blinkers[pin];
    ...
    if (pollers[pin].running) disableMCPPoller(pin);

    // start if new then engage
    if (!tb.running) startBinkerThread(pin, false);  // on-is-hi
...
bool getUserGPIO(int pin, bool latched, bool& state, Message& ynot) {
    ...
    if (blinkers[pin].running) disableBlinker(pin);

    // start if new
    if (!mp.running) startMCPPoller(pin);
