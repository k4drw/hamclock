...
static void logCPUTemp(float t) {
    ...
    if (prev_tr.t > 0 && now < prev_tr.t + SAMPLE_DT) return;
    ...
    if (++temps_head == N_CPUTEMPS) temps_head = 0;
}
...
bool getCPUTemp(float& t_C) {
    ...
    if (cmd_fp) {
        ...
        while (fgets(buf, sizeof(buf), cmd_fp)) {
            if (sscanf(buf, "CPU die temperature: %f", &t_C) == 1) ok = true;
        }
...
static bool plotCPUTempSince(time_t start_tm) {
    ...
    if (n_data < 2) return (found_older);
    ...
    for (int i = 0; i < n_data; i++) x[i] *= scale_factor;
...
void plotCPUTempHistory(void) {
    ...
    if (plotCPUTempSince(myNow() - 3600)) (void)plotCPUTempSince(0);
}
