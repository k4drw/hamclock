...
#define _TOT_DT 5000  // max millis for entire test
    ...
    if (getNBMEConnected() == 0) Serial.println("BME none found");
}
    ...
    for (int i = 0; i < MAX_N_BME; i++) {
        ...
        for (int j = 0; j < N_BME_READINGS; j++) {
            ...
            if (dp->u[qj] > 0) {                             // skip if time not set
                ...
                if (age_s > MAX_BME_AGE) continue;           // limit plot age
        ...
        for (int j = 0; j < nxy; j++) x[j] *= time_scale;
        ...
        if (getNBMEConnected() > 1) {
            ...
            if (i > 0) plbox.y += plbox.h;  // second sensor uses lower half
        }
...
const BMEData* getBMEData(BMEIndex device, bool fresh_read) {
    if (fresh_read) (void)readSensor(device);
...
void readBME280() {
    ...
    if (getNBMEConnected() == 0 || !clockTimeOk()) return;
...
void drawBME280Panes() {
    ...
    if (pp != PANE_NONE) drawOneBME280Pane(plot_b[pp], PLOT_CH_TEMPERATURE);
    pp = findPaneChoiceNow(PLOT_CH_PRESSURE);
    if (pp != PANE_NONE) drawOneBME280Pane(plot_b[pp], PLOT_CH_PRESSURE);
    pp = findPaneChoiceNow(PLOT_CH_HUMIDITY);
    if (pp != PANE_NONE) drawOneBME280Pane(plot_b[pp], PLOT_CH_HUMIDITY);
    pp = findPaneChoiceNow(PLOT_CH_DEWPOINT);
    if (pp != PANE_NONE) drawOneBME280Pane(plot_b[pp], PLOT_CH_DEWPOINT);
}

/* return whether new data has been read that has not been displayed
 */
bool newBME280data() { return (new_data); }

/* return number of connected BME sensors.
 * N.B. only valid after connectSensors()
 */
int getNBMEConnected(void) { return ((bme_data[BME_76] != NULL) + (bme_data[BME_77] != NULL)); }
...
void drawBMEStats() {
    ...
    if (i != NCDXF_B_NFIELDS) fatalError("drawBMEStats wrong count");
...
bool recalBMETemp(BMEIndex device, float new_corr) {
    ...
    if (dp) {
        ...
        for (int i = 0; i < N_BME_READINGS; i++)
            if (dp->u[i] > 0) dp->t[i] = dp->t[i] + del_corr;
...
bool recalBMEPres(BMEIndex device, float new_corr) {
    ...
    if (dp) {
        ...
        // apply to all existing data
        for (int i = 0; i < N_BME_READINGS; i++)
            if (dp->u[i] > 0) dp->p[i] = dp->p[i] + del_corr;
